...
export default class Item5e extends SystemDocumentMixin(Item) {
  ...
  _initializeSource(data, options={}) {
    ...
    if ( data.type === "backpack" ) {
      data.type = "container";
      foundry.utils.setProperty(data, "flags.aafo.persistSourceMigration", true);
    }

    /**
     * A hook event that fires before source data is initialized for an Item in a compendium.
     * @function aafo.initializeItemSource
     * @memberof hookEvents
     ...
     */
    if ( options.pack || options.parent?.pack ) Hooks.callAll("aafo.initializeItemSource", this, data, options);
  ...
  get usageScaling() {
    ...
    if ( isLeveled && CONFIG.AAFO.spellPreparationModes[preparation.mode]?.upcast ) return "slot";
   ...
   * @property {string} type              Spellcasting type as defined in ``CONFIG.AAFO.spellcastingTypes`.
  ...
  get spellcasting() {
    ...
    if ( CONFIG.AAFO.spellcastingTypes[finalSC.progression] ) finalSC.type = finalSC.progression;
    else finalSC.type = Object.entries(CONFIG.AAFO.spellcastingTypes).find(([type, data]) => {
  ...
  *allApplicableEffects() {
  ...
  prepareDerivedData() {
    ...
    if ( this.system.properties ) {
      this.labels.properties = this.system.properties.reduce((acc, prop) => {
        ...
        acc.push({
          abbr: prop,
          label: CONFIG.AAFO.itemProperties[prop]?.label,
          icon: CONFIG.AAFO.itemProperties[prop]?.icon
        });
        return acc;
      }, []);
    }
   ...
   */
  _prepareEquipment() {
    this.labels.armor = this.system.armor.value ? `${this.system.armor.value} ${game.i18n.localize("AAFO.AC")}` : "";
  }
  ...
  _prepareFeat() {
    ...
    if ( act?.type === "legendary" ) this.labels.featType = game.i18n.localize("AAFO.LegendaryActionLabel");
    else if ( act?.type === "lair" ) this.labels.featType = game.i18n.localize("AAFO.LairActionLabel");
    else if ( act?.type ) {
      const isAttack = /\w\wak$/.test(this.system.actionType);
      this.labels.featType = game.i18n.localize(isAttack ? "AAFO.Attack" : "AAFO.Action");
    }
    else this.labels.featType = game.i18n.localize("AAFO.Passive");
  }
  ...
  _prepareSpell() {
    const attributes = this.system?.validProperties.reduce((obj, k) => {
      obj[k] = CONFIG.AAFO.itemProperties[k];
      return obj;
    }, {});
    this.system.preparation.mode ||= "prepared";
    this.labels.level = CONFIG.AAFO.spellLevels[this.system.level];
    this.labels.school = CONFIG.AAFO.spellSchools[this.system.school]?.label;
  ...
  _prepareAction() {
    ...
    if ( dmg.parts ) {
      const types = CONFIG.AAFO.damageTypes;
  ...
  _prepareRecovery() {
    ...
    const config = CONFIG.AAFO.limitedUsePeriods[per] ?? {};
  ...
  _prepareAdvancement() {
    ...
    this.advancement = {
      ...
      byLevel: Object.fromEntries(
        Array.fromRange(CONFIG.AAFO.maxLevel, minAdvancementLevel).map(l => [l, []])
      ),
      byType: {},
  ...
  prepareFinalAttributes() {
    ...
    if ( "actionType" in this.system ) {
      this.labels.abilityCheck = game.i18n.format("AAFO.AbilityPromptTitle", {
        ability: CONFIG.AAFO.abilities[this.system.ability]?.label ?? ""
      });
  ...
  getDerivedDamageLabel() {
    ...
    const damageLabels = { ...CONFIG.AAFO.damageTypes, ...CONFIG.AAFO.healingTypes };
  ...
  getSaveDC() {
    ...
    // Update labels
    const abl = CONFIG.AAFO.abilities[save.ability]?.label ?? "";
    this.labels.save = game.i18n.format("AAFO.SaveDC", {dc: save.dc || "", ability: abl});
    return save.dc;
  }
   ...
   * @deprecated since aafo 3.2, available until aafo 3.4
   */
  replaceFormulaData(formula, data, { property }) {
    foundry.utils.logCompatibilityWarning(
      "Item5e#replaceFormulaData has been moved to aafo.utils.replaceFormulaData.",
      { since: "aafo 3.2", until: "aafo 3.4" }
    );
    return aafo.utils.replaceFormulaData(formula, data, { actor: this.actor, property });
  }
  ...
  async use(config={}, options={}) {
    if ( !this.isOwner ) {
      ui.notifications.error("AAFO.DocumentUseWarn", { localize: true });
      return null;
    }
    let item = this;
    ...
    options = foundry.utils.mergeObject({
      ...
      createMessage: true,
      "flags.aafo.use": {type: this.type, itemId: this.id, itemUuid: this.uuid}
    }, options);
     ...
     */
    if ( Hooks.call("aafo.preUseItem", item, config, options) === false ) return;
    ...
    if ( config.enchantmentProfile ) {
      foundry.utils.setProperty(options.flags, "aafo.use.enchantmentProfile", config.enchantmentProfile);
    }
    ...
    if ( item.type === "spell" ) foundry.utils.mergeObject(options.flags, {"aafo.use.spellLevel": item.system.level});
    ...
    if ( config.beginConcentrating ) {
      ...
      if ( effect ) {
        effects.push(effect);
        foundry.utils.setProperty(options.flags, "aafo.use.concentrationId", effect.id);
      }
    ...
    if ( config.createMeasuredTemplate ) {
      try {
        templates = await (aafo.canvas.AbilityTemplate.fromItem(item))?.drawPreview();
      } catch(err) {
        Hooks.onError("Item5e#use", err, {
          msg: game.i18n.localize("AAFO.PlaceTemplateError"),
          log: "error",
          notify: "error"
        });
      }
     ...
     */
    Hooks.callAll("aafo.useItem", item, config, options, templates ?? null, effects, summoned ?? null);
  ...
  async consume(item, config, options) {
     ...
     * @function aafo.preItemUsageConsumption
     * @memberof hookEvents
     * @param {Item5e} item                  Item being used.
     * @param {ItemUseConfiguration} config  Configuration data for the item usage being prepared.
     * @param {ItemUseOptions} options       Additional options used for configuring item usage.
     * @returns {boolean}                    Explicitly return `false` to prevent item from being used.
     */
    if ( Hooks.call("aafo.preItemUsageConsumption", item, config, options) === false ) return false;
    ...
    options.flags ??= {};
    if ( config.consumeUsage ) foundry.utils.setProperty(options.flags, "aafo.use.consumedUsage", true);
    if ( config.consumeResource ) foundry.utils.setProperty(options.flags, "aafo.use.consumedResource", true);
    if ( config.consumeSpellSlot ) foundry.utils.setProperty(options.flags, "aafo.use.consumedSpellSlot", true);

    /**
     * A hook event that fires after an item's resource consumption has been calculated but before any
     * changes have been made.
     * @function aafo.itemUsageConsumption
     * @memberof hookEvents
     * @param {Item5e} item                     Item being used.
     * @param {ItemUseConfiguration} config     Configuration data for the item usage being prepared.
     * @param {ItemUseOptions} options          Additional options used for configuring item usage.
     ...
     * @returns {boolean}                       Explicitly return `false` to prevent item from being used.
     */
    if ( Hooks.call("aafo.itemUsageConsumption", item, config, options, usage) === false ) return false;
  ...
  _getUsageConfig() {
    ...
    if ( this.requiresConcentration && !game.settings.get("aafo", "disableConcentration") ) {
      ...
      if ( limit && (limit <= effects.size) ) {
        const id = effects.find(e => {
          const data = e.flags.aafo?.itemData ?? {};
  ...
  _getUsageUpdates(config) {
    ...
    if ( config.consumeSpellSlot ) {
      ...
      if ( spells === 0 ) {
        ...
        const labelKey = isLeveled ? `AAFO.SpellLevel${this.system.level}`: `AAFO.SpellProg${config.slotLevel?.capitalize()}`;
        const label = game.i18n.localize(labelKey);
        ui.notifications.warn(game.i18n.format("AAFO.SpellCastNoSlots", {name: this.name, level: label}));
        return false;
      }
    ...
    if ( config.beginConcentrating ) {
      ...
      if ( config.endConcentration ) {
        ...
        if ( !replacedEffect ) {
          ui.notifications.warn("AAFO.ConcentratingMissingItem", {localize: true});
          return false;
        }
      }
      ...
      // Case 2: Starting concentration, but at limit.
        ...
        ui.notifications.warn("AAFO.ConcentratingLimited", {localize: true});
        return false;
      }
    }
  ...
  _handleConsumeUses(itemUpdates, actorUpdates, resourceUpdates, deleteIds) {
    ...
    if ( !used ) {
      ui.notifications.warn(game.i18n.format("AAFO.ItemNoUses", {name: this.name}));
      return false;
    }
  }
  ...
  _handleConsumeResource(usageConfig, itemUpdates, actorUpdates, resourceUpdates, deleteIds) {
    ...
    // No consumed target
    const typeLabel = CONFIG.AAFO.abilityConsumptionTypes[consume.type];
    if ( !consume.target ) {
      ui.notifications.warn(game.i18n.format("AAFO.ConsumeWarningNoResource", {name: this.name, type: typeLabel}));
      return false;
    }
    ...
    if ( resource === undefined ) {
      ui.notifications.warn(game.i18n.format("AAFO.ConsumeWarningNoSource", {name: this.name, type: typeLabel}));
      return false;
    }
    ...
    if ( remaining < 0 ) {
      ui.notifications.warn(game.i18n.format("AAFO.ConsumeWarningNoQuantity", {name: this.name, type: typeLabel}));
      return false;
    }
  ...
  async displayCard(options={}) {
    ...
    const consumeUsage = this.hasLimitedUses && !options.flags?.aafo?.use?.consumedUsage;
    const consumeResource = this.hasResource && !options.flags?.aafo?.use?.consumedResource;
    ...
    const templateData = {
      ...
      item: this,
      effects: this.effects.filter(e => (e.getFlag("aafo", "type") !== "enchantment") && !e.getFlag("aafo", "rider")),
    ...
    const html = await renderTemplate("systems/aafo/templates/chat/item-card.hbs", templateData);
    ...
    if ( (this.type === "consumable") && !this.actor.items.has(this.id) ) {
      chatData.flags["aafo.itemData"] = templateData.item.toObject();
    }
     ...
     * @param {ItemUseOptions} options  Options which configure the display of the item chat card.
     */
    Hooks.callAll("aafo.preDisplayCard", this, chatData, options);
     ...
     */
    Hooks.callAll("aafo.displayCard", this, card);
  ...
  async rollAttack(options={}) {
    const flags = this.actor.flags.aafo ?? {};
    if ( !this.hasAttack ) throw new Error("You may not place an Attack Roll with this Item.");
    let title = `${this.name} - ${game.i18n.localize("AAFO.AttackRoll")}`;
    ...
    const elvenAccuracy = (flags.elvenAccuracy
      && CONFIG.AAFO.characterFlags.elvenAccuracy.abilities.includes(this.abilityMod)) || undefined;
  ...
  async rollDamage({critical, event=null, spellLevel=null, versatile=false, options={}}={}) {
    ...
    const levelingFlag = this.getFlag("aafo", "spellLevel");
    ...
    const properties = Array.from(this.system.properties).filter(p => CONFIG.AAFO.itemProperties[p]?.isPhysical);
    const rollConfigs = dmg.parts.map(([formula, type]) => ({ parts: [formula], type, properties }));
    const rollData = this.getRollData();
    if ( spellLevel ) rollData.item.level = spellLevel;

    // Configure the damage roll
    const actionFlavor = game.i18n.localize(this.system.actionType === "heal" ? "AAFO.Healing" : "AAFO.DamageRoll");
    ...
    if ( versatile && dmg.versatile ) {
      ...
      rollConfig.messageData["flags.aafo"].roll.versatile = true;
    }
    ...
    if ( ammo ) {
      const properties = Array.from(ammo.system.properties).filter(p => CONFIG.AAFO.itemProperties[p]?.isPhysical);
    ...
    if ( this.system.actionType === "mwak" ) {
      rollConfig.criticalBonusDice = this.actor.getFlag("aafo", "meleeCriticalDamageDice") ?? 0;
    }
  ...
  async rollFormula({spellLevel}={}) {
     ...
     * @returns {boolean}                   Explicitly return false to prevent the roll from being performed.
     */
    if ( Hooks.call("aafo.preRollFormula", this, rollConfig) === false ) return;
    ...
    if ( rollConfig.chatMessage ) {
      roll.toMessage({
        speaker: ChatMessage.getSpeaker({actor: this.actor}),
        flavor: `${this.name} - ${game.i18n.localize("AAFO.OtherFormula")}`,
        rollMode: game.settings.get("core", "rollMode"),
        messageData: {"flags.aafo.roll": {type: "other", itemId: this.id, itemUuid: this.uuid}}
      });
    }

    /**
     * A hook event that fires after a formula has been rolled for an Item.
     * @function aafo.rollFormula
     * @memberof hookEvents
     * @param {Item5e} item  Item for which the roll was performed.
     * @param {Roll} roll    The resulting roll.
     */
    Hooks.callAll("aafo.rollFormula", this, roll);
  ...
  async rollRecharge() {
     ...
     * @returns {boolean}                   Explicitly return false to prevent the roll from being performed.
     */
    if ( Hooks.call("aafo.preRollRecharge", this, rollConfig) === false ) return;

    const roll = await new Roll(rollConfig.formula, rollConfig.data).roll({async: true});
    const success = roll.total >= rollConfig.target;

    if ( rollConfig.chatMessage ) {
      const resultMessage = game.i18n.localize(`AAFO.ItemRecharge${success ? "Success" : "Failure"}`);
      roll.toMessage({
        flavor: `${game.i18n.format("AAFO.ItemRechargeCheck", {name: this.name})} - ${resultMessage}`,
     ...
     * @function aafo.rollRecharge
     * @memberof hookEvents
     * @param {Item5e} item  Item for which the roll was performed.
     * @param {Roll} roll    The resulting roll.
     * @returns {boolean}    Explicitly return false to prevent the item from being recharged.
     */
    if ( Hooks.call("aafo.rollRecharge", this, roll) === false ) return roll;
  ...
  static async _onChatCardAction(event) {
    ...
    try {
      ...
      const storedData = message.getFlag("aafo", "itemData");
      let item = storedData ? new this(storedData, {parent: actor}) : actor.items.get(card.dataset.itemId);
      if ( !item ) {
        ui.notifications.error(game.i18n.format("AAFO.ActionWarningNoItem", {
      ...
      switch ( action ) {
        ...
        case "applyEffect":
          ...
          const concentration = actor.effects.get(message.getFlag("aafo", "use.concentrationId"));
          const effectData = { "flags.aafo.spellLevel": spellLevel };
        ...
        case "placeTemplate":
          try {
            await aafo.canvas.AbilityTemplate.fromItem(item, {"flags.aafo.spellLevel": spellLevel})?.drawPreview();
          } catch(err) {
  ...
  static async _applyEffectToToken(effect, token, { concentration, effectData={} }={}) {
    ...
    if ( !game.user.isGM && !token.actor?.isOwner ) {
      throw new Error(game.i18n.localize("AAFO.EffectApplyWarningOwnership"));
    }
    ...
    if ( !game.user.isGM && concentration && !concentration.actor?.isOwner ) {
      throw new Error(game.i18n.localize("AAFO.EffectApplyWarningConcentration"));
    }
    ...
    if ( needsConfiguration ) {
      let config = await AbilityUseDialog.create(item, {
        ...
        button: {
          ...
          label: game.i18n.localize("AAFO.Summoning.Action.Summon")
        },
        disableScaling: true
      });
  ...
  static _getChatCardTargets(card) {
    ...
    if ( !targets.length ) ui.notifications.warn("AAFO.ActionWarningNoToken", {localize: true});
    return targets;
  }
  ...
  createAdvancement(type, data={}, { showConfig=true, source=false }={}) {
    ...
    let config = CONFIG.AAFO.advancementTypes[type];
    if ( !config ) throw new Error(`${type} not found in CONFIG.AAFO.advancementTypes`);
    if ( config.prototype instanceof Advancement ) {
      foundry.utils.logCompatibilityWarning(
        ...
        { since: "aafo 3.1", until: "aafo 3.3", once: true }
      );
      config = {
  ...
  async _preUpdate(changed, options, user) {
    ...
    if ( changed.system.levels <= 0 ) {
      ui.notifications.warn("AAFO.MaxClassLevelMinimumWarn", {localize: true});
      changed.system.levels = 1;
    }

    // Check to make sure the updated class level doesn't exceed level cap
    if ( changed.system.levels > CONFIG.AAFO.maxLevel ) {
      ui.notifications.warn(game.i18n.format("AAFO.MaxClassLevelExceededWarn", {max: CONFIG.AAFO.maxLevel}));
      changed.system.levels = CONFIG.AAFO.maxLevel;
    }
    ...
    if ( newCharacterLevel > CONFIG.AAFO.maxLevel ) {
      ui.notifications.warn(game.i18n.format("AAFO.MaxCharacterLevelExceededWarn", {max: CONFIG.AAFO.maxLevel}));
      changed.system.levels -= newCharacterLevel - CONFIG.AAFO.maxLevel;
    }
  }
  ...
  async deleteDialog(options={}) {
    ...
    if ( this.actor?.system.metadata?.supportsAdvancement && !game.settings.get("aafo", "disableAdvancements") ) {
    ...
    if ( count ) {
      return Dialog.confirm({
        title: `${game.i18n.format("DOCUMENT.Delete", {type: game.i18n.localize("AAFO.Container")})}: ${this.name}`,
        content: `<h4>${game.i18n.localize("AreYouSure")}</h4>
          <p>${game.i18n.format("AAFO.ContainerDeleteMessage", {count})}</p>
          <label>
            <input type="checkbox" name="deleteContents">
            ${game.i18n.localize("AAFO.ContainerDeleteContents")}
          </label>`,
        yes: html => {
  ...
  static async createWithContents(items, { container, keepId=false, transformAll, transformFirst }={}) {
    ...
    if ( container ) {
      ...
      if ( depth > PhysicalItemTemplate.MAX_DEPTH ) {
        ui.notifications.warn(game.i18n.format("AAFO.ContainerMaxDepth", { depth: PhysicalItemTemplate.MAX_DEPTH }));
        return;
      }
    }
  ...
  static async createScrollFromSpell(spell, options={}, config={}) {
    config = foundry.utils.mergeObject({
      explanation: game.user.getFlag("aafo", "creation.scrollExplanation") ?? "reference",
    ...
    if ( config.dialog !== false ) {
      ...
      const result = await Dialog.prompt({
        title: game.i18n.format("AAFO.Scroll.CreateFrom", { spell: spell.name }),
        label: game.i18n.localize("AAFO.Scroll.CreateScroll"),
        content: await renderTemplate("systems/aafo/templates/apps/spell-scroll-dialog.hbs", {
          ...config, anchor, spellLevels: Object.entries(CONFIG.AAFO.spellLevels).reduce((obj, [k, v]) => {
      ...
      await game.user.setFlag("aafo", "creation.scrollExplanation", config.explanation);
    }
     ...
     * @function aafo.preCreateScrollFromSpell
     * @memberof hookEvents
     ...
     */
    if ( Hooks.call("aafo.preCreateScrollFromSpell", itemData, options, config) === false ) return;
    ...
    // Get scroll data
    let scrollUuid;
    const id = CONFIG.AAFO.spellScrollIds[level];
    if ( foundry.data.validators.isValidId(id) ) {
      scrollUuid = game.packs.get(CONFIG.AAFO.sourcePacks.ITEMS).index.get(id).uuid;
    } else {
      scrollUuid = id;
    }
    ...
    switch ( config.explanation ) {
      case "full":
        ...
        desc = [
          scrollIntro,
          "<hr>",
          `<h3>${itemData.name} (${game.i18n.format("AAFO.LevelNumber", {level})})</h3>`,
          isConc ? `<p><em>${game.i18n.localize("AAFO.Scroll.RequiresConcentration")}</em></p>` : null,
          "<hr>",
          description.value,
          "<hr>",
          `<h3>${game.i18n.localize("AAFO.Scroll.Details")}</h3>`,
          "<hr>",
          scrollDetails
        ].filterJoin("");
        break;
      case "reference":
        desc = [
          "<p><em>",
          CONFIG.AAFO.spellLevels[level] ?? level,
          "&Reference[Spell Scroll]",
          isConc ? `, ${game.i18n.localize("AAFO.Scroll.RequiresConcentration")}` : null,
          "</em></p>",
          description.value
        ].filterJoin("");
        break;
    ...
    const spellScrollData = foundry.utils.mergeObject(scrollData, {
      name: `${game.i18n.localize("AAFO.SpellScroll")}: ${itemData.name}`,
      img: itemData.img,
     ...
     * @function aafo.createScrollFromSpell
     * @memberof hookEvents
     * @param {Item5e|object} spell              The spell or item data to be made into a scroll.
     * @param {object} spellScrollData           The final item data used to make the scroll.
     * @param {SpellScrollConfiguration} config  Configuration options for scroll creation.
     */
    Hooks.callAll("aafo.createScrollFromSpell", spell, spellScrollData, config);
  ...
  static async createDialog(data={}, { parent=null, pack=null, types=null, ...options }={}) {
    ...
    const content = await renderTemplate("systems/aafo/templates/apps/document-create.hbs", {
      folders, name, type,
      folder: data.folder,
    ...
    return Dialog.prompt({
      ...
      rejectClose: false,
      options: { ...options, jQuery: false, width: 350, classes: ["aafo2", "create-document", "dialog"] }
    });
  }
  ...
  static getDefaultArtwork(itemData={}) {
    ...
    return { img: CONFIG.AAFO.defaultArtwork.Item[type] ?? img };
  }
