...
export default class TokenDocument5e extends SystemFlagsMixin(TokenDocument) {
   ...
   */
  get hasDynamicRing() {
    if ( game.release.generation < 12 ) return !!this.getFlag("aafo", "tokenRing.enabled");
   ...
   */
  get subjectPath() {
    const subject = this.getFlag("aafo", "tokenRing")?.textures?.subject;
  ...
  static getConsumedAttributes(data) {
    return CONFIG.AAFO.consumableResources;
  }
  ...
  static getTrackedAttributeChoices(attributes) {
    ...
    if ( abilities.length ) groups[game.i18n.localize("AAFO.AbilityScorePl")] = abilities;
    if ( movement.length ) groups[game.i18n.localize("AAFO.MovementSpeeds")] = movement;
    if ( senses.length ) groups[game.i18n.localize("AAFO.Senses")] = senses;
    if ( skills.length ) groups[game.i18n.localize("AAFO.SkillPassives")] = skills;
    if ( slots.length ) groups[game.i18n.localize("JOURNALENTRYPAGE.AAFO.Class.SpellSlots")] = slots;
    return groups;
  }
  ...
  prepareData() {
    ...
    if ( !size ) return;
    const dts = CONFIG.AAFO.actorSizes[size].dynamicTokenScale ?? 1;
  ...
  _onUpdate(data, options, userId) {
    ...
    const tokenRingChange = foundry.utils.hasProperty(data, "flags.aafo.tokenRings.enabled");
  ...
  getRingColors() {
    ...
    if ( this.hasStatusEffect(CONFIG.specialStatusEffects.DEFEATED) ) {
      colors.ring = CONFIG.AAFO.tokenRingColors.defeated;
    }
    return colors;
  }
  ...
  flashRing(type) {
    ...
    const color = CONFIG.AAFO.tokenRingColors[type];
    if ( !color ) return;
    const options = {};
    if ( type === "damage" ) {
  ...
  _onDelete(options, userId) {
    ...
    const origin = this.actor?.getFlag("aafo", "summon.origin");
