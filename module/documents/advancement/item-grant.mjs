...
export default class ItemGrantAdvancement extends Advancement {
  ...
  static get metadata() {
    return foundry.utils.mergeObject(super.metadata, {
      ...
      order: 40,
      icon: "systems/aafo/icons/svg/item-grant.svg",
      title: game.i18n.localize("AAFO.AdvancementItemGrantTitle"),
      hint: game.i18n.localize("AAFO.AdvancementItemGrantHint"),
      apps: {
   ...
   * @type {Set<string>}
   */
  static VALID_TYPES = new Set(["feat", "perk", "spell", "consumable", "container", "equipment", "loot", "tool", "weapon"]);
  ...
  summaryForLevel(level, { configMode=false }={}) {
    ...
    if ( !this.value.added || configMode ) return this.configuration.items.filter(i => fromUuidSync(i.uuid))
      .reduce((html, i) => html + aafo.utils.linkForUuid(i.uuid), "");
  ...
  restore(level, data) {
    ...
    for ( const item of data.items ) {
      ...
      updates[item._id] = item.flags.aafo.sourceId;
    }
    this.updateSource({
      "value.ability": data.ability,
      [this.storagePath(level)]: updates
  ...
  _validateItemType(item, { strict=true }={}) {
    ...
    if ( strict ) throw new Error(game.i18n.format("AAFO.AdvancementItemTypeInvalidWarning", {type}));
    return false;
  }
}
