...
export default class AbilityScoreImprovementAdvancement extends Advancement {
  ...
  static get metadata() {
    return foundry.utils.mergeObject(super.metadata, {
      ...
      order: 20,
      icon: "systems/aafo/icons/svg/ability-score-improvement.svg",
      title: game.i18n.localize("AAFO.AdvancementAbilityScoreImprovementTitle"),
      hint: game.i18n.localize("AAFO.AdvancementAbilityScoreImprovementHint"),
      apps: {
   ...
   */
  get allowFeat() {
    return (this.item.type === "class") && game.settings.get("aafo", "allowFeats");
  }
   ...
   */
  canImprove(ability) {
    return CONFIG.AAFO.abilities[ability]?.improvement !== false;
  }
  ...
  summaryForLevel(level, { configMode=false }={}) {
    ...
    if ( configMode ) {
      const entries = Object.entries(this.configuration.fixed).map(([key, value]) => {
        ...
        const name = CONFIG.AAFO.abilities[key]?.label ?? key;
