...
export default class TraitAdvancement extends Advancement {
  ...
  static get metadata() {
    return foundry.utils.mergeObject(super.metadata, {
      ...
      title: game.i18n.localize("AAFO.AdvancementTraitTitle"),
      hint: game.i18n.localize("AAFO.AdvancementTraitHint"),
      apps: {
        config: TraitConfig,
        flow: TraitFlow
      }
  ...
  prepareData() {
    ...
    const traitConfig = rep.size === 1 ? CONFIG.AAFO.traits[rep.first()] : null;
  ...
  sortingValueForLevel(levels) {
    const traitOrder = Object.keys(CONFIG.AAFO.traits).findIndex(k => k === this.representedTraits().first());
    const modeOrder = Object.keys(CONFIG.AAFO.traitModes).findIndex(k => k === this.configuration.mode);
  ...
  async actorSelected() {
    ...
    const traitTypes = this.configuration.mode === "default" ? Object.keys(CONFIG.AAFO.traits)
      : filteredKeys(CONFIG.AAFO.traits, t => t.expertise);
  ...
  async availableChoices(chosen) {
    ...
    if ( this.configuration.allowReplacements && (unfilteredLength > available.length) ) {
      ...
      return {
        ...
        label: game.i18n.format("AAFO.AdvancementTraitChoicesRemaining", {
