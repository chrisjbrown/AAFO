...
export default class ChatMessage5e extends ChatMessage {
  ...
  get shouldDisplayChallenge() {
    ...
    switch ( game.settings.get("aafo", "challengeVisibility") ) {
  ...
  async getHTML(...args) {
    ...
    if ( game.settings.get("aafo", "autoCollapseItemCards") ) {
  ...
  _collapseTrays(html) {
    let collapse;
    switch ( game.settings.get("aafo", "autoCollapseChatTrays") ) {
  ...
  _displayChatActionButtons(html) {
    const chatCard = html.find(".aafo.chat-card, .aafo2.chat-card");
    if ( chatCard.length > 0 ) {
      ...
      if ( game.user.isGM || actor?.isOwner || (this.user.id === game.user.id) ) {
        ...
        optionallyHide('button[data-action="consumeUsage"]', this.getFlag("aafo", "use.consumedUsage"));
        optionallyHide('button[data-action="consumeResource"]', this.getFlag("aafo", "use.consumedResource"));
        return;
      }
  ...
  _highlightCriticalSuccessFailure(html) {
    ...
    const originatingMessage = game.messages.get(this.getFlag("aafo", "originatingMessage")) ?? this;
  ...
  _enrichChatCard(html) {
    ...
    const roll = this.getFlag("aafo", "roll");
    ...
    if ( this.isContentVisible && item ) {
      ...
      const subtitle = roll.type === "damage"
        ? isCritical ? game.i18n.localize("AAFO.CriticalHit") : game.i18n.localize("AAFO.DamageRoll")
        : roll.type === "attack"
      ...
      flavor.classList.add("aafo2", "chat-card");
      flavor.innerHTML = `
  ...
  _enrichAttackTargets(html) {
    ...
    const targets = this.getFlag("aafo", "targets");
  ...
  _enrichEnchantmentTooltip(html) {
    const enchantmentProfile = this.getFlag("aafo", "use.enchantmentProfile");
    if ( !enchantmentProfile ) return;

    // Ensure concentration is still being maintained
    const concentrationId = this.getFlag("aafo", "use.concentrationId");
    ...
    enchantmentApplication.classList.add("aafo2");
  ...
  static addChatMessageContextOptions(html, options) {
    ...
    options.push(
      {
        name: game.i18n.localize("AAFO.ChatContextDamage"),
      ...
      {
        name: game.i18n.localize("AAFO.ChatContextHealing"),
      ...
      {
        name: game.i18n.localize("AAFO.ChatContextTempHP"),
      ...
      {
        name: game.i18n.localize("AAFO.ChatContextDoubleDamage"),
      ...
      {
        name: game.i18n.localize("AAFO.ChatContextHalfDamage"),
      ...
      {
        name: game.i18n.localize("AAFO.ChatContextSelectHit"),
      ...
      {
        name: game.i18n.localize("AAFO.ChatContextSelectMiss"),
        icon: '<i class="fas fa-bullseye"></i>',
        condition: canTarget,
  ...
  getAssociatedItem() {
    ...
    if ( !actor ) return;
    const storedData = this.getFlag("aafo", "itemData");
    return storedData
      ? new Item.implementation(storedData, { parent: actor })
      : actor.items.get(this.getFlag("aafo", "use.itemId"));
  }
}
