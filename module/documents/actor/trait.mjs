 ...
 * @param {string} [trait]  Trait as defined in `CONFIG.AAFO.traits`, only needed if key isn't prefixed.
 * @returns {string|void}
 */
...
export async function categories(trait) {
  const traitConfig = CONFIG.AAFO.traits[trait];
  const config = foundry.utils.deepClone(CONFIG.AAFO[traitConfig.configKey ?? trait]);
  ...
  for ( const key of Object.keys(config) ) {
    ...
    if ( traitConfig.children?.[key] ) {
      ...
      for ( const [childKey, value] of Object.entries(CONFIG.AAFO[traitConfig.children[key]]) ) {
  ...
  if ( traitConfig.subtypes ) {
    const map = CONFIG.AAFO[`${trait}ProficienciesMap`];

    // Merge all ID lists together
    const ids = traitConfig.subtypes.ids.reduce((obj, key) => {
      foundry.utils.mergeObject(obj, CONFIG.AAFO[key] ?? {});
      return obj;
    }, {});
      ...
      for ( const field of fields ) {
        ...
        if ( (field !== "system.type.value") && (val !== undefined) ) {
          ...
          foundry.utils.logCompatibilityWarning(
            ...
            { since: "aafo 3.0", until: "aafo 3.4", once: true }
          );
        }
      }
      obj[entry._id] = entry;
...
export function getBaseItemUUID(identifier) {
  ...
  let pack = CONFIG.AAFO.sourcePacks.ITEMS;
...
export function traitIndexFields() {
  ...
  for ( const traitConfig of Object.values(CONFIG.AAFO.traits) ) {
...
export function traitLabel(trait, count) {
  ...
  if ( !traitConfig ) return game.i18n.localize(`AAFO.TraitGenericPlural.${pluralRule}`);
 ...
 * @param {string} [config.trait]   Trait as defined in `CONFIG.AAFO.traits` if not using a prefixed key.
...
export function keyLabel(key, config={}) {
  ...
  const traitData = CONFIG.AAFO[traitConfig.configKey ?? trait] ?? {};
  ...
  if ( lastKey === "*" ) {
    ...
    const localization = `AAFO.TraitConfigChoose${final ? "Other" : `Any${count ? "Counted" : "Uncounted"}`}`;
    ...
    for ( const childrenKey of Object.values(traitConfig.children ?? {}) ) {
      const childLabel = CONFIG.AAFO[childrenKey]?.[lastKey];
      if ( childLabel ) return childLabel;
    }

    // Base item (e.g. "Shortsword")
    for ( const idsKey of traitConfig.subtypes?.ids ?? [] ) {
      const baseItemId = CONFIG.AAFO[idsKey]?.[lastKey];
...
export function choiceLabel(choice, { only=false, final=false }={}) {
  ...
  return game.i18n.format("AAFO.TraitConfigChooseList", {
    count: choice.count,
...
export function localizedList({ grants=new Set(), choices=[] }) {
  ...
  return game.i18n.format("AAFO.TraitConfigChooseWrapper", {
