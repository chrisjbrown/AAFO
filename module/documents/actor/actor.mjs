...
export default class Actor5e extends SystemDocumentMixin(Actor) {
   ...
   */
  get isPolymorphed() {
    return this.getFlag("aafo", "isPolymorphed") || false;
  }
  ...
  get concentration() {
    ...
    for ( const effect of this.effects ) {
      ...
      const data = effect.getFlag("aafo", "itemData");
  ...
  _initializeSource(source, options={}) {
    ...
    if ( !source._id || !options.pack || aafo.moduleArt.suppressArt ) return source;
    const uuid = `Compendium.${options.pack}.${source._id}`;
    const art = game.aafo.moduleArt.map.get(uuid);
  ...
  /** @inheritDoc */
  prepareData() {
    if ( this.system.modelProvider !== aafo ) return super.prepareData();
  ...
  /** @inheritDoc */
  prepareDerivedData() {
    const origin = this.getFlag("aafo", "summon.origin");
    ...
    if ( (this.system.modelProvider !== aafo) || (this.type === "group") ) return;
   ...
   */
  getPenaltiesTotal() {
    const penalties = this.system.attributes.penalties;
    return Object.values(penalties).reduce((total, penalty) => {
      total += penalty.value
      return total
    }, 0)
  }
   ...
   */
  getLevelExp(level) {
    const levels = CONFIG.AAFO.CHARACTER_EXP_LEVELS;
  ...
  getCRExp(cr) {
    ...
    return CONFIG.AAFO.CR_EXP_LEVELS[cr];
  }
   ...
   * @param {string} key      A key in `AAFO.conditionEffects`.
   * @returns {boolean}       Whether the actor is affected.
   */
  hasConditionEffect(key) {
    const props = CONFIG.AAFO.conditionEffects[key] ?? new Set();
  ...
  _prepareScaleValues() {
    this.system.scale = this.items.reduce((scale, item) => {
      if ( CONFIG.AAFO.advancementTypes.ScaleValue.validItemTypes.has(item.type) ) {
   ...
   *                                             `this.flags.aafo.originalActor` are used.
  ...
  _prepareSkill(skillId, {
    ...
    const feats = CONFIG.AAFO.characterFlags;
    ...
    skillData.total = skillData.mod + skillData.value;
    ...
    skillData.passive = skillData.mod + skillData.bonus + skillData.prof.flat + passive + passiveBonus;
  ...
  _prepareArmorClass() {
    ...
    let cfg = CONFIG.AAFO.armorClasses[ac.calc];
    if ( !cfg ) {
      ac.calc = "flat";
      if ( Number.isNumeric(ac.value) ) ac.flat = Number(ac.value);
      cfg = CONFIG.AAFO.armorClasses.flat;
    }

    // Identify Equipped Items
    const armorTypes = new Set(Object.keys(CONFIG.AAFO.armorTypes));
      ...
      default:
        ...
        if ( armors.length ) {
          if ( armors.length > 1 ) this._preparationWarnings.push({
            message: game.i18n.localize("AAFO.WarnMultipleArmor"), type: "warning"
          });
        ...
        try {
          ...
          this._preparationWarnings.push({
            message: game.i18n.localize("AAFO.WarnBadACFormula"), link: "armor", type: "error"
          });
          const replaced = Roll.replaceFormulaData(CONFIG.AAFO.armorClasses.default.formula, rollData);
          ac.base = Roll.safeEval(replaced);
        }
        break;
    }
    ...
    if ( shields.length ) {
      if ( shields.length > 1 ) this._preparationWarnings.push({
        message: game.i18n.localize("AAFO.WarnMultipleShields"), type: "warning"
      });
  ...
  _prepareInitiative(bonusData, globalCheckBonus=0) {
    ...
    const abilityId = init.ability || CONFIG.AAFO.defaultAbilities.initiative;
  ...
  _prepareSpellcasting() {
    ...
    for ( const type of Object.keys(CONFIG.AAFO.spellcastingTypes) ) {
  ...
  static computeClassProgression(progression, cls, {actor, spellcasting, count=1}={}) {
     ...
     * The actual hook names include the spellcasting type (e.g. `aafo.computeLeveledProgression`).
     * @param {object} progression                    Spellcasting progression data. *Will be mutated.*
     ...
     * @function aafo.computeSpellcastingProgression
     * @memberof hookEvents
     */
    const allowed = Hooks.call(
      `aafo.compute${type.capitalize()}Progression`, progression, actor, cls, spellcasting, count
    );
  ...
  static computeLeveledProgression(progression, actor, cls, spellcasting, count) {
    const prog = CONFIG.AAFO.spellcastingTypes.leveled.progression[spellcasting.progression];
    if ( !prog ) return;
  ...
  static prepareSpellcastingSlots(spells, type, progression, {actor}={}) {
     ...
     * The actual hook names include the spellcasting type (e.g. `aafo.prepareLeveledSlots`).
     * @param {object} spells        The `data.spells` object within actor's data. *Will be mutated.*
     * @param {Actor5e} actor        Actor for whom the data is being prepared.
     * @param {object} progression   Spellcasting progression data.
     * @returns {boolean}            Explicitly return false to prevent default preparation from being performed.
     * @function aafo.prepareSpellcastingSlots
     * @memberof hookEvents
     */
    const allowed = Hooks.call(`aafo.prepare${type.capitalize()}Slots`, spells, actor, progression);
  ...
  static prepareLeveledSlots(spells, actor, progression) {
    const levels = Math.clamp(progression.slot, 0, CONFIG.AAFO.maxLevel);
    const slots = CONFIG.AAFO.SPELL_SLOT_TABLE[Math.min(levels, CONFIG.AAFO.SPELL_SLOT_TABLE.length) - 1] ?? [];
    for ( const level of Array.fromRange(Object.keys(CONFIG.AAFO.spellLevels).length - 1, 1) ) {
  ...
  static prepareAltSlots(spells, actor, progression, key, table) {
    ...
    let keyLevel = Math.clamp(progression[key], 0, CONFIG.AAFO.maxLevel);
    spells[key] ??= {};
  ...
  static preparePactSlots(spells, actor, progression) {
    this.prepareAltSlots(spells, actor, progression, "pact", CONFIG.AAFO.pactCastingProgression);
  }
  ...
  async _preCreate(data, options, user) {
    ...
    if ( "size" in (this.system.traits || {}) ) {
      const size = CONFIG.AAFO.actorSizes[this.system.traits.size || "med"].token ?? 1;
  ...
  async _preUpdate(changed, options, user) {
    ...
    if ( "size" in (this.system.traits || {}) ) {
      ...
      if ( newSize && (newSize !== this.system.traits?.size) ) {
        let size = CONFIG.AAFO.actorSizes[newSize].token ?? 1;
    ...
    if ( "hp" in (this.system.attributes || {}) ) {
      ...
      foundry.utils.setProperty(options, "aafo.hp", { ...this.system.attributes.hp });
    }

    // Record previous exhaustion level.
    if ( Number.isFinite(foundry.utils.getProperty(changed, "system.attributes.exhaustion")) ) {
      foundry.utils.setProperty(options, "aafo.originalExhaustion", this.system.attributes.exhaustion);
    }
  }
  ...
  async beginConcentrating(item, effectData={}) {
     ...
     * @function aafo.preBeginConcentrating
     * @memberof hookEvents
     ...
     */
    if ( Hooks.call("aafo.preBeginConcentrating", this, item, effectData) === false ) return;
  ...
  async endConcentration(target) {
    ...
    if ( foundry.utils.getType(target) === "string" ) effect = effects.find(e => e.id === target);
      ...
      effect = effects.find(e => {
        const data = e.getFlag("aafo", "itemData") ?? {};
     ...
     * @function aafo.preEndConcentration
     * @memberof hookEvents
     ...
     */
    if ( Hooks.call("aafo.preEndConcentration", this, effect) === false) return [];

    await effect.delete();

    /**
     * A hook that is called after a concentration effect is deleted.
     * @function aafo.endConcentration
     * @memberof hookEvents
     * @param {Actor5e} actor             The actor ending concentration.
     * @param {ActiveEffect5e} effect     The ActiveEffect that was deleted.
     */
    Hooks.callAll("aafo.endConcentration", this, effect);
  ...
  async challengeConcentration({ dc=10, ability=null }={}) {
    ...
    if ( ability in CONFIG.AAFO.abilities ) dataset.ability = ability;
    ...
    return ChatMessage.implementation.create({
      content: await renderTemplate("systems/aafo/templates/chat/request-card.hbs", {
  ...
  _isRemarkableAthlete(ability) {
    return this.getFlag("aafo", "remarkableAthlete")
      && CONFIG.AAFO.characterFlags.remarkableAthlete.abilities.includes(ability);
  }
  ...
  async rollSkill(skillId, options={}) {
    ...
    const luc = this.system.abilities.luc
    // const globalBonuses = this.system.bonuses?.abilities ?? {};
    const parts = ["@skillVal", "@mod", "@lucMod"];
    const data = this.getRollData();

    // Skill value
    data.skillVal = skl?.value ?? 0;
    ...
    if (luc.mod) {
      parts.push("@lucMod");
      data.lucMod = Math.floor(luc.mod / 2);
    }

    // Penalties
    const penaltiesTotal = this.getPenaltiesTotal();
    if (penaltiesTotal) {
      parts.push("@penaltiesTotal");
      data.penaltiesTotal = `- ${penaltiesTotal}`;
    }
    ...
    const reliableTalent = (skl.value >= 1 && this.getFlag("aafo", "reliableTalent"));

    // Roll and return
    const flavor = game.i18n.format("AAFO.SkillPromptTitle", {skill: CONFIG.AAFO.skills[skillId]?.label ?? ""});
    const rollData = foundry.utils.mergeObject({
      data: data,
      title: `${flavor}: ${this.name}`,
      flavor,
      chooseModifier: true,
      halflingLucky: this.getFlag("aafo", "halflingLucky"),
      reliableTalent,
      messageData: {
        speaker: options.speaker || ChatMessage.getSpeaker({actor: this}),
        "flags.aafo.roll": {type: "skill", skillId }
      }
    }, options);
     ...
     * @function aafo.preRollSkill
     * @memberof hookEvents
     * @param {Actor5e} actor                Actor for which the skill check is being rolled.
     * @param {D20RollConfiguration} config  Configuration data for the pending roll.
     * @param {string} skillId               ID of the skill being rolled as defined in `AAFO.skills`.
     * @returns {boolean}                    Explicitly return `false` to prevent skill check from being rolled.
     */
    if ( Hooks.call("aafo.preRollSkill", this, rollData, skillId) === false ) return;

    const roll = await d20Roll(rollData);

    /**
     * A hook event that fires after a skill check has been rolled for an Actor.
     * @function aafo.rollSkill
     * @memberof hookEvents
     * @param {Actor5e} actor   Actor for which the skill check has been rolled.
     * @param {D20Roll} roll    The resulting roll.
     * @param {string} skillId  ID of the skill that was rolled as defined in `AAFO.skills`.
     */
    if ( roll ) Hooks.callAll("aafo.rollSkill", this, roll, skillId);
  ...
  async rollToolCheck(toolId, options={}) {
    ...
    const reliableTalent = (prof?.multiplier >= 1 && this.getFlag("aafo", "reliableTalent"));

    const flavor = game.i18n.format("AAFO.ToolPromptTitle", {tool: Trait.keyLabel(toolId, {trait: "tool"}) ?? ""});
    const rollData = foundry.utils.mergeObject({
      data, flavor,
      title: `${flavor}: ${this.name}`,
      chooseModifier: true,
      ...
      messageData: {
        ...
        "flags.aafo.roll": {type: "tool", toolId}
      }
    }, options);
    rollData.parts = parts.concat(options.parts ?? []);
     ...
     * @returns {boolean}                    Explicitly return `false` to prevent skill check from being rolled.
     */
    if ( Hooks.call("aafo.preRollToolCheck", this, rollData, toolId) === false ) return;

    const roll = await d20Roll(rollData);

    /**
     * A hook event that fires after a tool check has been rolled for an Actor.
     * @function aafo.rollTool
     * @memberof hookEvents
     ...
     */
    if ( roll ) Hooks.callAll("aafo.rollToolCheck", this, roll, toolId);
  ...
  rollAbility(abilityId, options={}) {
    const label = CONFIG.AAFO.abilities[abilityId]?.label ?? "";
    new Dialog({
      title: `${game.i18n.format("AAFO.AbilityPromptTitle", {ability: label})}: ${this.name}`,
      content: `<p>${game.i18n.format("AAFO.AbilityPromptText", {ability: label})}</p>`,
      buttons: {
        test: {
          label: game.i18n.localize("AAFO.ActionAbil"),
          callback: () => this.rollAbilityTest(abilityId, options)
        },
        save: {
          label: game.i18n.localize("AAFO.ActionSave"),
  ...
  async rollAbilityTest(abilityId, options={}) {
    ...
    // Roll and return
    const flavor = game.i18n.format("AAFO.AbilityPromptTitle", {ability: label});
    const rollData = foundry.utils.mergeObject({
      data,
      title: `${flavor}: ${this.name}`,
      flavor,
      ...
      messageData: {
        speaker: options.speaker || ChatMessage.getSpeaker({actor: this}),
        "flags.aafo.roll": {type: "ability", abilityId }
      }
    }, options);
    rollData.parts = parts.concat(options.parts ?? []);
     ...
     * @function aafo.preRollAbilityTest
     * @memberof hookEvents
     * @param {Actor5e} actor                Actor for which the ability test is being rolled.
     * @param {D20RollConfiguration} config  Configuration data for the pending roll.
     * @param {string} abilityId             ID of the ability being rolled as defined in `AAFO.abilities`.
     * @returns {boolean}                    Explicitly return `false` to prevent ability test from being rolled.
     */
    if ( Hooks.call("aafo.preRollAbilityTest", this, rollData, abilityId) === false ) return;

    const roll = await d20Roll(rollData);

    /**
     * A hook event that fires after an ability test has been rolled for an Actor.
     * @function aafo.rollAbilityTest
     * @memberof hookEvents
     * @param {Actor5e} actor     Actor for which the ability test has been rolled.
     * @param {D20Roll} roll      The resulting roll.
     * @param {string} abilityId  ID of the ability that was rolled as defined in `AAFO.abilities`.
     */
    if ( roll ) Hooks.callAll("aafo.rollAbilityTest", this, roll, abilityId);
  ...
  async rollAbilitySave(abilityId, options={}) {
    ...
    // Roll and return
    const flavor = game.i18n.format("AAFO.SavePromptTitle", {ability: label});
    const rollData = foundry.utils.mergeObject({
      data,
      title: `${flavor}: ${this.name}`,
      flavor,
      ...
      messageData: {
        speaker: options.speaker || ChatMessage.getSpeaker({actor: this}),
        "flags.aafo.roll": {type: "save", abilityId }
      }
    }, options);
    rollData.parts = parts.concat(options.parts ?? []);
     ...
     * @function aafo.preRollAbilitySave
     * @memberof hookEvents
     ...
     */
    if ( Hooks.call("aafo.preRollAbilitySave", this, rollData, abilityId) === false ) return;

    const roll = await d20Roll(rollData);

    /**
     * A hook event that fires after an ability save has been rolled for an Actor.
     * @function aafo.rollAbilitySave
     * @memberof hookEvents
     * @param {Actor5e} actor     Actor for which the ability save has been rolled.
     * @param {D20Roll} roll      The resulting roll.
     * @param {string} abilityId  ID of the ability that was rolled as defined in `AAFO.abilities`.
     */
    if ( roll ) Hooks.callAll("aafo.rollAbilitySave", this, roll, abilityId);
  ...
  async rollDeathSave(options={}) {
    ...
    if ( (this.system.attributes.hp.value > 0) || (death.failure >= 3) || (death.success >= 3) ) {
      ui.notifications.warn("AAFO.DeathSaveUnnecessary", {localize: true});
      return null;
    }
    ...
    if ( this.getFlag("aafo", "diamondSoul") ) {
      parts.push("@prof");
    ...
    // Evaluate the roll
    const flavor = game.i18n.localize("AAFO.DeathSavingThrow");
    const rollData = foundry.utils.mergeObject({
      data,
      title: `${flavor}: ${this.name}`,
      flavor,
     ...
     * @function aafo.preRollDeathSave
     * @memberof hookEvents
     ...
     */
    if ( Hooks.call("aafo.preRollDeathSave", this, rollData) === false ) return;
    ...
    if ( roll.total >= (roll.options.targetValue ?? 10) ) {
      ...
      if ( roll.isCritical ) {
        ...
        details.chatString = "AAFO.DeathSaveCriticalSuccess";
      }
      ...
      // 3 Successes = survive and reset checks
        ...
        details.chatString = "AAFO.DeathSaveSuccess";
      }
    ...
    // Save failure
      ...
      if ( failures >= 3 ) {  // 3 Failures = death
        details.chatString = "AAFO.DeathSaveFailure";
      }
    }
     ...
     */
    if ( Hooks.call("aafo.rollDeathSave", this, roll, details) === false ) return roll;
  ...
  async rollConcentration(options={}) {
     ...
     * @function aafo.preRollConcentration
     * @memberof hookEvents
     ...
     */
    if ( Hooks.call("aafo.preRollConcentration", this, options) === false ) return;
     ...
     * @function aafo.rollConcentration
     * @memberof hookEvents
     * @param {Actor5e} actor     Actor for which the saving throw has been rolled.
     * @param {D20Roll} roll      The resulting roll.
     */
    if ( roll ) Hooks.callAll("aafo.rollConcentration", this, roll);
  ...
  getInitiativeRoll(options={}) {
    ...
    const data = this.getRollData();
    const flags = this.flags.aafo || {};
    if ( flags.initiativeAdv ) options.advantageMode ??= aafo.dice.D20Roll.ADV_MODE.ADVANTAGE;
    ...
    const tiebreaker = game.settings.get("aafo", "initiativeDexTiebreaker");
    ...
    options = foundry.utils.mergeObject({
      flavor: options.flavor ?? game.i18n.localize("AAFO.Initiative"),
  ...
  async rollInitiativeDialog(rollOptions={}) {
    ...
    const choice = await roll.configureDialog({
      ...
      title: `${game.i18n.localize("AAFO.InitiativeRoll")}: ${this.name}`,
      chooseModifier: false,
      defaultAction: rollOptions.advantageMode ?? aafo.dice.D20Roll.ADV_MODE.NORMAL
    });
  ...
  async rollInitiative(options={}, rollOptions={}) {
     ...
     * @function aafo.preRollInitiative
     * @memberof hookEvents
     * @param {Actor5e} actor  The Actor that is rolling initiative.
     * @param {D20Roll} roll   The initiative roll.
     */
    if ( Hooks.call("aafo.preRollInitiative", this, this._cachedInitiativeRoll) === false ) {
  ...
  async rollHitDie(denomination, options={}) {
    ...
    if ( this.type === "npc" ) {
      ...
      if ( !this.system.attributes.hd.value ) {
        ui.notifications.error(game.i18n.format("AAFO.HitDiceNPCWarn", {name: this.name}));
        return null;
      }
    }
    ...
    // Otherwise check classes
      ...
      if ( !cls ) {
        ui.notifications.error(game.i18n.format("AAFO.HitDiceWarn", {name: this.name, formula: denomination}));
        return null;
      }
    }

    // Prepare roll data
    const flavor = game.i18n.localize("AAFO.HitDiceRoll");
     ...
     */
    if ( Hooks.call("aafo.preRollHitDie", this, rollConfig, denomination) === false ) return;
     ...
     * @returns {boolean}               Explicitly return `false` to prevent updates from being performed.
     */
    if ( Hooks.call("aafo.rollHitDie", this, roll, updates) === false ) return roll;
   ...
   * @see {@link aafo.preRollClassHitPoints}
   */
  async rollClassHitPoints(item, { chatMessage=true }={}) {
    ...
    const flavor = game.i18n.format("AAFO.AdvancementHitPointsRollMessage", { class: item.name });
    const messageData = {
      title: `${flavor}: ${this.name}`,
      flavor,
      speaker: ChatMessage.implementation.getSpeaker({ actor: this }),
      "flags.aafo.roll": { type: "hitPoints" }
    };

    /**
     * A hook event that fires before hit points are rolled for a character's class.
     * @function aafo.preRollClassHitPoints
     * @memberof hookEvents
     ...
     */
    Hooks.callAll("aafo.preRollClassHitPoints", this, item, rollData, messageData);
     ...
     * @function aafo.rollClassHitPoints
     * @memberof hookEvents
     * @param {Actor5e} actor  Actor for which the hit points have been rolled.
     * @param {Roll} roll      The resulting roll.
     */
    Hooks.callAll("aafo.rollClassHitPoints", this, roll);
   ...
   * @param {boolean} [options.chatMessage=true]  Display the chat message for this roll.
   * @returns {Promise<Roll>}                     The completed roll.
   * @see {@link aafo.preRollNPCHitPoints}
   */
  async rollNPCHitPoints({ chatMessage=true }={}) {
    ...
    const flavor = game.i18n.format("AAFO.HPFormulaRollMessage");
    const messageData = {
      title: `${flavor}: ${this.name}`,
      flavor,
     ...
     * @function aafo.preRollNPCHitPoints
     * @memberof hookEvents
     * @param {Actor5e} actor            Actor for which the hit points are being rolled.
     * @param {object} rollData
     * @param {string} rollData.formula  The string formula to parse.
     ...
     * @param {object} messageData       The data object to use when creating the message.
     */
    Hooks.callAll("aafo.preRollNPCHitPoints", this, rollData, messageData);
     ...
     * @function aafo.rollNPCHitPoints
     * @memberof hookEvents
     * @param {Actor5e} actor  Actor for which the hit points have been rolled.
     * @param {Roll} roll      The resulting roll.
     */
    Hooks.callAll("aafo.rollNPCHitPoints", this, roll);
   ...
   */
  async levelUp() {
    const uuid = "Compendium.aafo.character.Item.t1Z9BO9LJyCjoLUv"
    const source = await fromUuid(uuid);
    if ( !source ) return null;
    const itemData =  source.clone({
      _id: foundry.utils.randomID(),
      "flags.aafo.sourceId": uuid,
    }).toObject();

    const charLevel = this.system.details.level;
    itemData.system.levels = Math.min(itemData.system.levels, CONFIG.AAFO.maxLevel - charLevel);
    if ( itemData.system.levels <= 0 ) {
      const err = game.i18n.format("AAFO.MaxCharacterLevelExceededWarn", { max: CONFIG.AAFO.maxLevel });
      ui.notifications.error(err);
      return false;
    }

    // class exists on actor, level up class
    const cls = this.itemTypes.class.find(c => c.identifier === itemData.system.identifier);
    if ( cls ) { 
      const priorLevel = cls.system.levels;
      if ( !game.settings.get("aafo", "disableAdvancements") ) {
        const manager = AdvancementManager.forLevelChange(this, cls.id, itemData.system.levels);
    ...
    // class does not exist, add class to level up
    if ( this.system.metadata?.supportsAdvancement && itemData.system.advancement?.length
      && !game.settings.get("aafo", "disableAdvancements") ) {
      // Ensure that this item isn't violating the singleton rule
      const dataModel = CONFIG.Item.dataModels[itemData.type];
      const singleton = dataModel?.metadata.singleton ?? false;
      if ( singleton && this.itemTypes[itemData.type].length ) {
        ui.notifications.error(game.i18n.format("AAFO.ActorWarningSingleton", {
      ...
      const manager = AdvancementManager.forNewItem(this, itemData);
      if ( manager.steps.length ) {
        manager.render(true);
        return false;
      }
  ...
  async shortRest(config={}) {
    ...
    config = foundry.utils.mergeObject({
      ...
      duration: CONFIG.AAFO.restTypes.short.duration[game.settings.get("aafo", "restVariant")]
    }, config);

    /**
     * A hook event that fires before a short rest is started.
     * @function aafo.preShortRest
     * @memberof hookEvents
     * @param {Actor5e} actor             The actor that is being rested.
     * @param {RestConfiguration} config  Configuration options for the rest.
     * @returns {boolean}                 Explicitly return `false` to prevent the rest from being started.
     */
    if ( Hooks.call("aafo.preShortRest", this, config) === false ) return;
  ...
  async longRest(config={}) {
    ...
    config = foundry.utils.mergeObject({
      ...
      duration: CONFIG.AAFO.restTypes.long.duration[game.settings.get("aafo", "restVariant")]
    }, config);

    /**
     * A hook event that fires before a long rest is started.
     * @function aafo.preLongRest
     * @memberof hookEvents
     * @param {Actor5e} actor             The actor that is being rested.
     * @param {RestConfiguration} config  Configuration options for the rest.
     * @returns {boolean}                 Explicitly return `false` to prevent the rest from being started.
     */
    if ( Hooks.call("aafo.preLongRest", this, config) === false ) return;
     ...
     * @function aafo.longRest
     * @memberof hookEvents
     * @param {Actor5e} actor             The actor that is being rested.
     * @param {RestConfiguration} config  Configuration options for the rest.
     * @returns {boolean}                 Explicitly return `false` to prevent the rest from being continued.
     */
    if ( Hooks.call("aafo.longRest", this, config) === false ) return;
  ...
  async _rest(config, result={}, ...args) {
     ...
     * @function aafo.preRestCompleted
     * @memberof hookEvents
     * @param {Actor5e} actor             The actor that is being rested.
     ...
     */
    if ( Hooks.call("aafo.preRestCompleted", this, result, config) === false ) return result;
  ...
  async _displayRestResultMessage(result, longRest=false) {
    ...
    let restFlavor;
    switch (game.settings.get("aafo", "restVariant")) {
      case "normal":
        restFlavor = (longRest && newDay) ? "AAFO.LongRestOvernight" : `AAFO.${length}RestNormal`;
        break;
      case "gritty":
        restFlavor = (!longRest && newDay) ? "AAFO.ShortRestOvernight" : `AAFO.${length}RestGritty`;
        break;
      case "epic":
        restFlavor = `AAFO.${length}RestEpic`;
        break;
    }

    // Determine the chat message to display
    let message;
    if ( diceRestored && healthRestored ) message = `AAFO.${length}RestResult`;
    else if ( longRest && !diceRestored && healthRestored ) message = "AAFO.LongRestResultHitPoints";
    else if ( longRest && diceRestored && !healthRestored ) message = "AAFO.LongRestResultHitDice";
    else message = `AAFO.${length}RestResultShort`;
  ...
  _getRestSpellRecovery({recoverShort=true, recoverLong=true}={}) {
    ...
    Object.entries(CONFIG.AAFO.spellPreparationModes).forEach(([k, v]) => {
      const isSR = CONFIG.AAFO.spellcastingTypes[k === "prepared" ? "leveled" : k]?.shortRest;
  ...
  async _getRestItemUsesRecovery({recoverShortRestUses=true, recoverLongRestUses=true,
    ...
    for ( let item of this.items ) {
      ...
      if ( recoverDailyUses && uses.recovery && CONFIG.AAFO.limitedUsePeriods[uses.per]?.formula ) {
        const roll = new Roll(uses.recovery, item.getRollData());
        if ( recoverLongRestUses && (game.settings.get("aafo", "restVariant") === "gritty") ) {
        ...
        try {
          ...
          ui.notifications.warn(game.i18n.format("AAFO.ItemRecoveryFormulaWarning", {
            name: item.name,
        ...
        if ( newValue !== uses.value ) {
          ...
          const locKey = `AAFO.Item${diff < 0 ? "Loss" : "Recovery"}Roll${isMax ? "Max" : ""}`;
  ...
  _prepareMovementAttribution() {
    ...
    const units = movement.units || Object.keys(CONFIG.AAFO.movementUnits)[0];
    return Object.entries(CONFIG.AAFO.movementTypes).reduce((html, [k, label]) => {
  ...
  async _prepareArmorClassAttribution({ title }={}) {
    ...
    const cfg = CONFIG.AAFO.armorClasses[ac.calc];
    const attribution = [];

    if ( ac.calc === "flat" ) {
      attribution.push({
        label: game.i18n.localize("AAFO.ArmorClassFlat"),
    ...
    switch ( ac.calc ) {
      ...
      case "natural":
        attribution.push({
          label: game.i18n.localize("AAFO.ArmorClassNatural"),
          mode: CONST.ACTIVE_EFFECT_MODES.OVERRIDE,
          value: ac.flat
        });
        break;
      ...
      default:
        ...
        let label = game.i18n.localize("AAFO.PropertyBase");
        if ( armorInFormula ) label = this.armor?.name ?? game.i18n.localize("AAFO.ArmorClassUnarmored");
        attribution.unshift({
          label,
          mode: CONST.ACTIVE_EFFECT_MODES.OVERRIDE,
          value: base
    ...
    if ( ac.shield !== 0 ) attribution.push({
      label: this.shield?.name ?? game.i18n.localize("AAFO.EquipmentShield"),
      mode: CONST.ACTIVE_EFFECT_MODES.ADD,
      value: ac.shield
    });
  ...
  getOriginalStats() {
    ...
    if ( this.isPolymorphed ) {
      const transformOptions = this.flags.aafo?.transformOptions;
      const original = game.actors?.get(this.flags.aafo?.originalActor);
      if ( original ) {
  ...
  async transformInto(target, { keepPhysical=false, keepMental=false, keepSaves=false, keepSkills=false,
    ...
    const allowed = game.settings.get("aafo", "allowPolymorphing");
    if ( !allowed && !game.user.isGM ) {
      ui.notifications.warn("AAFO.PolymorphWarn", {localize: true});
      return null;
    }

    // Get the original Actor data and the new source data
    const o = this.toObject();
    o.flags.aafo = o.flags.aafo || {};
    o.flags.aafo.transformOptions = {mergeSkills, mergeSaves};
    const source = target.toObject();

    if ( keepSelf ) {
      o.img = source.img;
      o.name = `${o.name} (${game.i18n.localize("AAFO.PolymorphSelf")})`;
    }
    ...
    foundry.utils.setProperty(d.prototypeToken, "flags.aafo.tokenRing", foundry.utils.mergeObject(
      foundry.utils.getProperty(d.prototypeToken, "flags.aafo.tokenRing") ?? {},
      foundry.utils.getProperty(source.prototypeToken, "flags.aafo.tokenRing") ?? {},
      { inplace: false }
    ));
    ...
    if ( !keepSelf ) {
      ...
      for ( let k of Object.keys(abilities) ) {
        ...
        const type = CONFIG.AAFO.abilities[k]?.type;
      ...
      if ( !keepClass && d.system.details.cr ) {
        const cls = new aafo.dataModels.item.ClassData({levels: d.system.details.cr});
        d.items.push({
          type: "class",
          name: game.i18n.localize("AAFO.PolymorphTmpClass"),
          system: cls.toObject()
        });
      }
    ...
    // Set new data flags
    if ( !this.isPolymorphed || !d.flags.aafo.originalActor ) d.flags.aafo.originalActor = this.id;
    d.flags.aafo.isPolymorphed = true;

    // Gather previous actor data
    const previousActorIds = this.getFlag("aafo", "previousActorIds") || [];
    previousActorIds.push(this._id);
    foundry.utils.setProperty(d.flags, "aafo.previousActorIds", previousActorIds);
    ...
    if ( this.isToken ) {
      ...
      foundry.utils.setProperty(tokenData, "flags.aafo.previousActorData", previousActorData);
     ...
     * @param {object} [options]
     */
    Hooks.callAll("aafo.transformActor", this, target, d, {
    ...
    const updates = tokens.map(t => {
      ...
      const dOriginalActor = foundry.utils.getProperty(d, "flags.aafo.originalActor");
      foundry.utils.setProperty(newTokenData, "flags.aafo.originalActor", dOriginalActor);
      foundry.utils.setProperty(newTokenData, "flags.aafo.isPolymorphed", true);
      return newTokenData;
    });
  ...
  async revertOriginalForm({renderSheet=true}={}) {
    ...
    if ( !this.isOwner ) {
      ui.notifications.warn("AAFO.PolymorphRevertWarn", {localize: true});
      return null;
    }

    /**
     * A hook event that fires just before the actor is reverted to original form.
     * @function aafo.revertOriginalForm
     * @memberof hookEvents
     * @param {Actor} this                 The original actor before transformation.
     * @param {object} [options]
     */
    Hooks.callAll("aafo.revertOriginalForm", this, {renderSheet});
    const previousActorIds = this.getFlag("aafo", "previousActorIds") ?? [];
    const isOriginalActor = !previousActorIds.length;
    const isRendered = this.sheet.rendered;

    // Obtain a reference to the original actor
    const original = game.actors.get(this.getFlag("aafo", "originalActor"));
    ...
    if ( this.isToken ) {
      ...
      if ( !baseActor ) {
        ui.notifications.warn(game.i18n.format("AAFO.PolymorphRevertNoOriginalActorWarn", {
          reference: this.getFlag("aafo", "originalActor")
        }));
        return;
      }
      const prototypeTokenData = await baseActor.getTokenDocument();
      const actorData = this.token.getFlag("aafo", "previousActorData");
      ...
      foundry.utils.setProperty(tokenUpdate, "flags.aafo.tokenRing", foundry.utils.mergeObject(
        foundry.utils.getProperty(tokenUpdate, "flags.aafo.tokenRing") ?? {},
        foundry.utils.getProperty(prototypeTokenData, "flags.aafo.tokenRing") ?? {},
        { inplace: false }
      ));
      ...
      if ( isOriginalActor ) {
        await this.unsetFlag("aafo", "isPolymorphed");
        await this.unsetFlag("aafo", "previousActorIds");
        await this.token.unsetFlag("aafo", "previousActorData");
      }
    ...
    if ( canvas.ready ) {
      ...
      const tokenUpdates = tokens.map(t => {
        ...
        delete update.x;
        delete update.y;
        if ( !foundry.utils.getProperty(tokenData, "flags.aafo.tokenRing") ) {
          foundry.utils.setProperty(update, "flags.aafo.tokenRing", {});
        }
        return update;
      });
  ...
  static addDirectoryContextOptions(html, entryOptions) {
    entryOptions.push({
      name: "AAFO.PolymorphRestoreTransformation",
      ...
      name: "AAFO.Group.Primary.Set",
      icon: '<i class="fa-solid fa-star"></i>',
      callback: li => {
        game.settings.set("aafo", "primaryParty", { actor: game.actors.get(li[0].dataset.documentId) });
      },
      condition: li => {
        const actor = game.actors.get(li[0].dataset.documentId);
        const primary = game.settings.get("aafo", "primaryParty")?.actor;
        return game.user.isGM && (actor.type === "group")
          && (actor.system.type.value === "party") && (actor !== primary);
      },
      group: "system"
    }, {
      name: "AAFO.Group.Primary.Remove",
      icon: '<i class="fa-regular fa-star"></i>',
      callback: li => {
        game.settings.set("aafo", "primaryParty", { actor: null });
      },
      condition: li => {
        const actor = game.actors.get(li[0].dataset.documentId);
        const primary = game.settings.get("aafo", "primaryParty")?.actor;
  ...
  static onRenderActorDirectory(jQuery) {
    const primaryParty = game.settings.get("aafo", "primaryParty")?.actor;
    if ( primaryParty ) {
  ...
  static formatCreatureType(typeData) {
    ...
    } else if ( typeData.value in CONFIG.AAFO.creatureTypes ) {
      const code = CONFIG.AAFO.creatureTypes[typeData.value];
      localizedType = game.i18n.localize(typeData.swarm ? code.plural : code.label);
    }
    let type = localizedType;
    if ( typeData.swarm ) {
      type = game.i18n.format("AAFO.CreatureSwarmPhrase", {
        size: game.i18n.localize(CONFIG.AAFO.actorSizes[typeData.swarm].label),
        type: localizedType
      });
    }
  ...
  async _onUpdate(data, options, userId) {
    ...
    if ( hp && !options.isRest && !options.isAdvancement ) {
      ...
      if ( Number.isInteger(changes.total) && (changes.total !== 0) ) {
        ...
        if ( !game.settings.get("aafo", "disableConcentration") && (userId === game.userId) && (changes.total < 0) ) {
         ...
         */
        Hooks.callAll(`aafo.${changes.total > 0 ? "heal" : "damage"}Actor`, this, changes, data, userId);
      }
    }
  }
  ...
  _displayTokenEffect(changes) {
    ...
    const fill = CONFIG.AAFO.tokenHPColors[key];
  ...
  async _onUpdateExhaustion(data, options) {
    ...
    if ( level < 1 ) return effect?.delete();
      ...
      const originalExhaustion = foundry.utils.getProperty(options, "aafo.originalExhaustion");
      return effect.update({ "flags.aafo.exhaustionLevel": level }, { aafo: { originalExhaustion } });
    } else {
      effect = await ActiveEffect.implementation.fromStatusEffect("exhaustion", { parent: this });
      effect.updateSource({ "flags.aafo.exhaustionLevel": level });
  ...
  updateEncumbrance(options) {
    ...
    if ( !encumbrance || (game.settings.get("aafo", "encumbrance") === "none") ) return;
    const statuses = [];
    const variant = game.settings.get("aafo", "encumbrance") === "variant";
    ...
    const effectData = { ...CONFIG.AAFO.encumbrance.effects[statuses[0]], statuses };
    if ( effect ) {
      const originalEncumbrance = effect.statuses.first();
      return effect.update(effectData, { aafo: { originalEncumbrance } });
    }
