...
export default class ActiveEffect5e extends ActiveEffect {
   ...
   */
  static ID = {
    ENCUMBERED: staticID("aafoencumbered"),
    EXHAUSTION: staticID("aafoexhaustion")
  };
  ...
  get isAppliedEnchantment() {
    return (this.getFlag("aafo", "type") === "enchantment")
   ...
   */
  async getSource() {
    if ( (this.target instanceof aafo.documents.Actor5e) && (this.parent instanceof aafo.documents.Item5e) ) {
      return this.parent;
    }
  ...
  /** @inheritdoc */
  apply(actor, change) {
    if ( change.key.startsWith("flags.aafo.") ) change = this._prepareFlagChange(actor, change);
  ...
  _prepareFlagChange(actor, change) {
    ...
    const data = CONFIG.AAFO.characterFlags[key.replace("flags.aafo.", "")];
  ...
  determineSuppression() {
    ...
    if ( this.getFlag("aafo", "type") === "enchantment" ) return;
    if ( this.parent instanceof aafo.documents.Item5e ) this.isSuppressed = this.parent.areEffectsSuppressed;
  }
  ...
  prepareDerivedData() {
    ...
    if ( (this.getFlag("aafo", "type") === "enchantment") || this.getFlag("aafo", "rider") ) this.transfer = false;
  ...
  _prepareExhaustionLevel() {
    const config = CONFIG.AAFO.conditionTypes.exhaustion;
    let level = this.getFlag("aafo", "exhaustionLevel");
    ...
    this.name = `${game.i18n.localize("AAFO.Exhaustion")} ${level}`;
    if ( level >= config.levels ) {
      this.statuses.add("dead");
      CONFIG.AAFO.statusEffects.dead.statuses?.forEach(s => this.statuses.add(s));
    }
  }
  ...
  async createRiderConditions() {
    ...
    const createRider = async id => {
      const existing = this.parent.effects.get(staticID(`aafo${id}`));
      if ( existing ) return;
  ...
  async createRiderEnchantments() {
    ...
    // Create Effects
    const riderEffects = (this.getFlag("aafo", "enchantment.riders.effect") ?? []).map(id => {
      const effectData = origin.effects.get(id)?.toObject();
      if ( effectData ) {
        delete effectData._id;
        delete effectData.flags?.aafo?.rider;
    ...
    if ( this.parent.isEmbedded ) {
      const riderItems = await Promise.all((this.getFlag("aafo", "enchantment.riders.item") ?? []).map(async uuid => {
        const itemData = (await fromUuid(uuid))?.toObject();
        if ( itemData ) {
          delete itemData._id;
          foundry.utils.setProperty(itemData, "flags.aafo.enchantment", { origin: this.uuid });
        }
        return itemData;
      }));
  ...
  async _preCreate(data, options, user) {
    ...
    if ( (this.getFlag("aafo", "type") === "enchantment") && (this.parent instanceof Actor) ) {
      ui.notifications.error("AAFO.Enchantment.Warning.NotOnActor", { localize: true });
      return false;
    }
  ...
  _onUpdate(data, options, userId) {
    ...
    const originalLevel = foundry.utils.getProperty(options, "aafo.originalExhaustion");
    const newLevel = foundry.utils.getProperty(data, "flags.aafo.exhaustionLevel");
    const originalEncumbrance = foundry.utils.getProperty(options, "aafo.originalEncumbrance");
    ...
    // Display proper scrolling status effects for encumbrance
      ...
      if ( !increase ) this.name = CONFIG.AAFO.encumbrance.effects[originalEncumbrance].name;
  ...
  async _preDelete(options, user) {
    ...
    if ( dependents.length && !game.users.activeGM ) {
      ui.notifications.warn("AAFO.ConcentrationBreakWarning", { localize: true });
      return false;
    }
  ...
  static createConcentrationEffectData(item, data={}) {
    ...
    const effectData = foundry.utils.mergeObject({
      ...statusEffect,
      name: `${game.i18n.localize("EFFECT.AAFO.StatusConcentrating")}: ${item.name}`,
      description: game.i18n.format("AAFO.ConcentratingOn", {
        name: item.name,
        type: game.i18n.localize(`TYPES.Item.${item.type}`)
      }),
      duration: ActiveEffect5e.getEffectDurationFromItem(item),
      "flags.aafo.itemData": item.actor.items.has(item.id) ? item.id : item.toObject(),
      origin: item.uuid,
      statuses: [statusEffect.id].concat(statusEffect.statuses ?? [])
    }, data, {inplace: false});
    delete effectData.id;
    if ( item.type === "spell" ) effectData["flags.aafo.spellLevel"] = item.system.level;
  ...
  static _getExhaustionImage(level) {
    const split = CONFIG.AAFO.conditionTypes.exhaustion.icon.split(".");
    const ext = split.pop();
  ...
  static _manageConcentration(event, actor) {
    ...
    const choices = effects.reduce((acc, effect) => {
      ...
      acc[effect.id] = data?.name ?? actor.items.get(data)?.name ?? game.i18n.localize("AAFO.ConcentratingItemless");
      return acc;
    }, {});
    const options = HandlebarsHelpers.selectOptions(choices, { hash: { sort: true } });
    const content = `
    <form class="aafo">
      <p>${game.i18n.localize("AAFO.ConcentratingEndChoice")}</p>
      <div class="form-group">
        <label>${game.i18n.localize("AAFO.Source")}</label>
    ...
    Dialog.prompt({
      ...
      rejectClose: false,
      title: game.i18n.localize("AAFO.Concentration"),
      label: game.i18n.localize("AAFO.Confirm")
    });
  }
  ...
  addDependent(...dependent) {
    const dependents = this.getFlag("aafo", "dependents") ?? [];
    dependents.push(...dependent.map(d => ({ uuid: d.uuid })));
    return this.setFlag("aafo", "dependents", dependents);
  }
   ...
   */
  getDependents() {
    return (this.getFlag("aafo", "dependents") || []).reduce((arr, { uuid }) => {
  ...
  async richTooltip(enrichmentOptions={}) {
    const properties = [];
    if ( this.isSuppressed ) properties.push("AAFO.EffectType.Unavailable");
    else if ( this.disabled ) properties.push("AAFO.EffectType.Inactive");
    else if ( this.isTemporary ) properties.push("AAFO.EffectType.Temporary");
    else properties.push("AAFO.EffectType.Passive");
    if ( this.getFlag("aafo", "type") === "enchantment" ) properties.push("AAFO.Enchantment.Label");

    return {
      content: await renderTemplate(
        "systems/aafo/templates/effects/parts/effect-tooltip.hbs", {
          effect: this,
      ...
      classes: ["aafo2", "aafo-tooltip", "effect-tooltip"]
    };
  }
}
