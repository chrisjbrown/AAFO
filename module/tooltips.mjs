...
export default class Tooltips5e {
  ...
  async _onTooltipActivate() {
    ...
    if ( loading?.dataset.uuid ) {
      ...
      if ( doc instanceof aafo.documents.Actor5e ) return this._onHoverActor(doc);
      return this._onHoverContentLink(doc);
    }
  ...
  async _onHoverPassive(skill, ability, dc) {
    const skillConfig = CONFIG.AAFO.skills[skill];
    const abilityConfig = CONFIG.AAFO.abilities[ability ?? skillConfig.ability];

    let label;
    if ( skillConfig ) {
      label = game.i18n.format("AAFO.SkillPassiveSpecificHint", { skill: skillConfig.label, ability: abilityConfig.label });
    } else {
      ...
      label = game.i18n.format("AAFO.SkillPassiveHint", { skill: abilityConfig.label });
    }

    const party = game.settings.get("aafo", "primaryParty")?.actor;
    if ( !party ) {
    ...
    this.tooltip.classList.add("aafo-tooltip", "passive-tooltip");
    this.tooltip.innerHTML = await renderTemplate("systems/aafo/templates/journal/passive-tooltip.hbs", context);
