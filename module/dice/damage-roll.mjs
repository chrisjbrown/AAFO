...
export default class DamageRoll extends Roll {
      ...
      if ( term instanceof DiceTerm ) {
        ...
        if ( this.isCritical ) {
          ...
          if ( this.options.powerfulCritical ) {
            ...
            if ( bonus > 0 ) {
              const flavor = term.flavor?.toLowerCase().trim() ?? game.i18n.localize("AAFO.PowerfulCritical");
  ...
  static async toMessage(rolls, messageData={}, {rollMode, create=true}={}) {
    ...
    if ( isCritical ) {
      const label = game.i18n.localize("AAFO.CriticalHit");
  ...
  static async configureDialog(rolls, {
    ...
    const content = await renderTemplate(template ?? this.EVALUATION_TEMPLATE, {
      formulas: rolls.map((roll, index) => ({
        ...
        type: CONFIG.AAFO.damageTypes[roll.options.type]?.label
          ?? CONFIG.AAFO.healingTypes[roll.options.type]?.label ?? null
      })),
      defaultRollMode,
      ...
      new Dialog({
        ...
        buttons: {
          critical: {
            ...
            label: game.i18n.localize("AAFO.CriticalHit"),
            callback: html => resolve(rolls.map((r, i) => r._onDialogSubmit(html, true, i === 0)))
          },
          normal: {
            label: game.i18n.localize(allowCritical ? "AAFO.Normal" : "AAFO.Roll"),
