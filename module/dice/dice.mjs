...
export async function d20Roll({
  ...
  if ( messageId ) foundry.utils.setProperty(messageData, "flags.aafo.originatingMessage", messageId);
...
export async function damageRoll({
  ...
  const rolls = [];
  flavor ??= title;
  multiplyNumeric ??= game.settings.get("aafo", "criticalDamageModifiers");
  powerfulCritical ??= game.settings.get("aafo", "criticalDamageMaxDice");
