...
export function replaceFormulaData(formula, data, { actor, property }) {
  ...
  if ( (missingReferences.size > 0) && actor ) {
    ...
    const message = game.i18n.format("AAFO.FormulaMissingReferenceWarn", {
...
export function convertWeight(value, from, to) {
  ...
  const message = unit => `Weight unit ${unit} not defined in CONFIG.AAFO.weightUnits`;
  if ( !CONFIG.AAFO.weightUnits[from] ) throw new Error(message(from));
  if ( !CONFIG.AAFO.weightUnits[to] ) throw new Error(message(to));
  return value
    * CONFIG.AAFO.weightUnits[from].conversion
    / CONFIG.AAFO.weightUnits[to].conversion;
}
 ...
 * (e.g. "aafo.actor-traits").
 * @returns {Promise}
 */
export async function preloadHandlebarsTemplates() {
  const partials = [
    // Shared Partials
    "systems/aafo/templates/shared/active-effects.hbs",
    "systems/aafo/templates/shared/inventory.hbs",
    "systems/aafo/templates/shared/inventory2.hbs",
    "systems/aafo/templates/shared/active-effects2.hbs",
    "systems/aafo/templates/apps/parts/trait-list.hbs",

    // Actor Sheet Partials
    "systems/aafo/templates/actors/parts/actor-traits.hbs",
    "systems/aafo/templates/actors/parts/actor-inventory.hbs",
    "systems/aafo/templates/actors/parts/actor-features.hbs",
    "systems/aafo/templates/actors/parts/actor-spellbook.hbs",
    "systems/aafo/templates/actors/parts/actor-warnings.hbs",
    "systems/aafo/templates/actors/tabs/character-details.hbs",
    "systems/aafo/templates/actors/tabs/character-features.hbs",
    "systems/aafo/templates/actors/tabs/character-spells.hbs",
    "systems/aafo/templates/actors/tabs/character-biography.hbs",
    "systems/aafo/templates/actors/tabs/group-members.hbs",

    // Item Sheet Partials
    "systems/aafo/templates/items/parts/item-action.hbs",
    "systems/aafo/templates/items/parts/item-activation.hbs",
    "systems/aafo/templates/items/parts/item-advancement.hbs",
    "systems/aafo/templates/items/parts/item-description.hbs",
    "systems/aafo/templates/items/parts/item-mountable.hbs",
    "systems/aafo/templates/items/parts/item-spellcasting.hbs",
    "systems/aafo/templates/items/parts/item-source.hbs",
    "systems/aafo/templates/items/parts/item-summary.hbs",
    "systems/aafo/templates/items/parts/item-tooltip.hbs",

    // Journal Partials
    "systems/aafo/templates/journal/parts/journal-table.hbs",

    // Advancement Partials
    "systems/aafo/templates/advancement/parts/advancement-skill-control.hbs",
    "systems/aafo/templates/advancement/parts/advancement-ability-score-control.hbs",
    "systems/aafo/templates/advancement/parts/advancement-controls.hbs",
    "systems/aafo/templates/advancement/parts/advancement-spell-config.hbs"
  ];

  const paths = {};
  for ( const path of partials ) {
    paths[path.replace(".hbs", ".html")] = path;
    paths[`aafo.${path.split("/").pop().replace(".hbs", "")}`] = path;
  }
...
function itemContext(context, options) {
  if ( arguments.length !== 2 ) throw new Error("#aafo-itemContext requires exactly one argument");
...
function concealSection(conceal, options) {
  ...
  <div class="unidentified-notice">
      <div>
          <strong>${game.i18n.localize("AAFO.Unidentified.Title")}</strong>
          <p>${game.i18n.localize("AAFO.Unidentified.Notice")}</p>
      </div>
  </div>`;
  return content;
}
...
export function registerHandlebarsHelpers() {
  Handlebars.registerHelper({
    ...
    "aafo-concealSection": concealSection,
    "aafo-dataset": dataset,
    "aafo-groupedSelectOptions": groupedSelectOptions,
    "aafo-linkForUuid": (uuid, options) => linkForUuid(uuid, options.hash),
    "aafo-itemContext": itemContext,
    "aafo-numberFormat": (context, options) => formatNumber(context, options.hash),
    "aafo-textFormat": formatText
  });
}
 ...
 * @param {string} configKeyPath          Key path within `CONFIG.AAFO` to localize.
 * @param {object} [options={}]
 ...
 * @param {object} config  The `CONFIG.AAFO` object to localize and sort. *Will be mutated.*
 */
...
export function getHumanReadableAttributeLabel(attr, { actor }={}) {
  ...
  if ( (attr === "details.xp.value") && (actor?.type === "npc") ) {
    return game.i18n.localize("AAFO.ExperiencePointsValue");
  }
  ...
  // Derived fields.
  if ( attr === "attributes.init.total" ) label = "AAFO.InitiativeBonus";
  else if ( attr === "attributes.ac.value" ) label = "AAFO.ArmorClass";
  else if ( attr === "attributes.spelldc" ) label = "AAFO.SpellDC";

  // Abilities.
  else if ( attr.startsWith("abilities.") ) {
    const [, key] = attr.split(".");
    label = game.i18n.format("AAFO.AbilityScoreL", { ability: CONFIG.AAFO.abilities[key].label });
  }

  // Skills.
  else if ( attr.startsWith("skills.") ) {
    const [, key] = attr.split(".");
    label = game.i18n.format("AAFO.SkillPassiveScore", { skill: CONFIG.AAFO.skills[key].label });
  }

  // Spell slots.
  else if ( attr.startsWith("spells.") ) {
    const [, key] = attr.split(".");
    if ( !/spell\d+/.test(key) ) label = `AAFO.SpellSlots${key.capitalize()}`;
    else {
      const plurals = new Intl.PluralRules(game.i18n.lang, {type: "ordinal"});
      const level = Number(key.slice(5));
      label = game.i18n.format(`AAFO.SpellSlotsN.${plurals.select(level)}`, { n: level });
    }
  }

  // Attempt to find the attribute in a data model.
  if ( !label ) {
    const { CharacterData, NPCData, VehicleData, GroupData } = aafo.dataModels.actor;
