...
export default class Award extends DialogMixin(FormApplication) {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["aafo2", "award", "dialog"],
      template: "systems/aafo/templates/apps/award.hbs",
      title: "AAFO.Award.Title",
      width: 350,
      height: "auto",
      currency: null,
      xp: null,
  ...
  getData(options={}) {
    ...
    context.currency = Object.entries(CONFIG.AAFO.currencies).reduce((obj, [k, { label }]) => {
    ...
    context.hideXP = game.settings.get("aafo", "disableExperienceTracking");
    context.noPrimaryParty = !game.settings.get("aafo", "primaryParty")?.actor && !this.isPartyAward;
  ...
  static prepareDestinations(destinations, savedDestinations) {
    const icons = {
      container: '<aafo-icon class="fa-fw" src="systems/aafo/icons/svg/backpack.svg"></aafo-icon>',
  ...
  _saveDestinations(destinations) {
    ...
    target.setFlag("aafo", "awardDestinations", destinations);
  }
  ...
  static async displayAwardMessages(results) {
    ...
    for ( const [destination, result] of results ) {
      ...
      if ( result.xp ) entries.push(`
        <span class="award-entry">
          ${formatNumber(result.xp)} ${game.i18n.localize("AAFO.ExperiencePointsAbbr")}
        </span>
      `);
      if ( !entries.length ) continue;

      const content = game.i18n.format("AAFO.Award.Message", {
        name: destination.name, award: `<span class="aafo2">${game.i18n.getListFormatter().format(entries)}</span>`
      });
  ...
  static async handleAward(message) {
    if ( !game.user.isGM ) {
      ui.notifications.error("AAFO.Award.NotGMError", { localize: true });
      return;
    }
    ...
    try {
      ...
      // Otherwise show the UI with defaults
        ...
        const savedDestinations = game.user.getFlag("aafo", "awardDestinations");
  ...
  static parseAwardCommand(message) {
    ...
    for ( const part of command.split(" ") ) {
      ...
      try {
        new Roll(amount);
        if ( label in CONFIG.AAFO.currencies ) currency[label] = amount;
    ...
    if ( unrecognized.length ) throw new Error(game.i18n.format("AAFO.Award.UnrecognizedWarning", {
