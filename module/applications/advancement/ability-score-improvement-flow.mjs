...
export default class AbilityScoreImprovementFlow extends AdvancementFlow {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      ...
      template: "systems/aafo/templates/advancement/ability-score-improvement-flow.hbs"
    });
  }
  ...
  async getData() {
    const points = {
      assigned: Object.keys(CONFIG.AAFO.abilities).reduce((assigned, key) => {
    ...
    const abilities = Object.entries(CONFIG.AAFO.abilities).reduce((obj, [key, data]) => {
      ...
      obj[key] = {
        ...
        canIncrease: (value < points.cap) && !fixed && !this.feat && points.available > 0,
        canDecrease: (value > 0) && !fixed && !this.feat
      };
      return obj;
    }, {});
    ...
    return foundry.utils.mergeObject(super.getData(), {
      ...
      pointCap: game.i18n.format(
        `AAFO.AdvancementAbilityScoreImprovementCapDisplay.${pluralRules.select(points.cap)}`, {points: points.cap}
      ),
      pointsRemaining: game.i18n.format(
        `AAFO.AdvancementAbilityScoreImprovementPointsRemaining.${pluralRules.select(points.available)}`,
  ...
  async _onDrop(event) {
    ...
    if ( (item.type !== "feat") || (item.system.type.value !== "feat") ) {
      ui.notifications.error("AAFO.AdvancementAbilityScoreImprovementFeatWarning", {localize: true});
      return null;
    }
    ...
    if ( (item.system.prerequisites?.level ?? -Infinity) > this.advancement.actor.system.details.level ) {
      ui.notifications.error(game.i18n.format("AAFO.AdvancementAbilityScoreImprovementFeatLevelWarning", {
