...
export default class TraitConfig extends AdvancementConfig {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["aafo", "advancement", "traits", "two-column"],
      template: "systems/aafo/templates/advancement/trait-config.hbs",
      width: 640
    });
  }
  ...
  async getData() {
    ...
    context.validTraitTypes = Object.entries(CONFIG.AAFO.traits).reduce((obj, [key, config]) => {
    ...
    if ( traitConfig ) {
      ...
      context.default.title = game.i18n.localize("AAFO.TraitGenericPlural.other");
    ...
    context.selectedTraitHeader = `${CONFIG.AAFO.traits[this.trait].labels.localization}.other`;
  ...
  async _onChangeInput(event) {
    ...
    if ( (event.target.name === "configuration.mode")
      ...
      const validTraitTypes = filteredKeys(CONFIG.AAFO.traits, c => c.expertise);
