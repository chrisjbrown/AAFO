...
export default class ScaleValueConfig extends AdvancementConfig {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["aafo", "advancement", "scale-value", "two-column"],
      template: "systems/aafo/templates/advancement/scale-value-config.hbs",
      width: 540
    });
  }
  ...
  getData() {
    ...
    return foundry.utils.mergeObject(super.getData(), {
      ...
      movementUnits: CONFIG.AAFO.movementUnits
    });
  }
  ...
  _prepareLevelData() {
    let lastValue = null;
    let levels = Array.fromRange(CONFIG.AAFO.maxLevel + 1);
  ...
  _onIdentifierHintCopy(event) {
    ...
    game.tooltip.activate(event.target, {text: game.i18n.localize("AAFO.IdentifierCopied"), direction: "UP"});
  }
  ...
  async _updateObject(event, formData) {
    ...
    if ( typeChange && (updates.configuration.type !== this.advancement.configuration.type) ) {
      ...
      await this.advancement.update(Array.fromRange(CONFIG.AAFO.maxLevel, 1).reduce((obj, lvl) => {
