...
export default class AdvancementConfig extends FormApplication {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["aafo", "advancement", "dialog"],
      template: "systems/aafo/templates/advancement/advancement-config.hbs",
      width: 400,
      height: "auto",
      submitOnChange: true,
      closeOnSubmit: false,
  ...
  get title() {
    ...
    return `${game.i18n.format("AAFO.AdvancementConfigureTitle", { item: this.item.name })}: ${type}`;
  }
  ...
  /** @inheritdoc */
  getData() {
    const levels = Object.fromEntries(Array.fromRange(CONFIG.AAFO.maxLevel + 1).map(l => [l, l]));
    if ( ["class", "subclass"].includes(this.item.type) ) delete levels[0];
    else levels[0] = game.i18n.localize("AAFO.AdvancementLevelAnyHeader");
    const context = {
      appId: this.id,
      CONFIG: CONFIG.AAFO,
  ...
  async _onDrop(event) {
    ...
    if ( item.uuid === this.item.uuid ) {
      ui.notifications.error("AAFO.AdvancementItemGrantRecursiveWarning", {localize: true});
      return null;
    }

    // Abort if this uuid exists already
    if ( existingItems.find(i => i.uuid === item.uuid) ) {
      ui.notifications.warn("AAFO.AdvancementItemGrantDuplicateWarning", {localize: true});
      return null;
    }
