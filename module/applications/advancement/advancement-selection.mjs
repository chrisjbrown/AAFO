...
export default class AdvancementSelection extends Dialog {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["aafo", "sheet", "advancement"],
      template: "systems/aafo/templates/advancement/advancement-selection.hbs",
      title: "AAFO.AdvancementSelectionTitle",
      width: 500,
      height: "auto"
    });
  }
  ...
  getData() {
    const context = { types: {} };
    for ( let [name, config] of Object.entries(CONFIG.AAFO.advancementTypes) ) {
    ...
    context.types = aafo.utils.sortObjectEntries(context.types, "label");
    return context;
  }
  ...
  static async createDialog(item, { rejectClose=false, options={} }={}) {
    return new Promise((resolve, reject) => {
      const dialog = new this(item, {
        title: `${game.i18n.localize("AAFO.AdvancementSelectionTitle")}: ${item.name}`,
        buttons: {
          submit: {
            callback: html => {
