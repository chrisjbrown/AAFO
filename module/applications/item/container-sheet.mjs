...
export default class ContainerSheet extends ItemSheet5e {
  ...
  /** @inheritdoc */
  get template() {
    return "systems/aafo/templates/items/container.hbs";
  }
  ...
  _onDrop(event) {
    ...
    if ( Hooks.call("aafo.dropItemSheetData", this.item, this, data) === false ) return;
  ...
  async _onDropFolder(event, data) {
    ...
    if ( recursiveWarning ) ui.notifications.warn("AAFO.ContainerRecursiveError", { localize: true });
  ...
  async _onDropItem(event, data) {
    ...
    const parentContainers = await this.item.system.allContainers();
    if ( (this.item.uuid === item.uuid) || parentContainers.includes(item) ) {
      ui.notifications.error("AAFO.ContainerRecursiveError", { localize: true });
      return;
    }
