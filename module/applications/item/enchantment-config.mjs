...
export default class EnchantmentConfig extends DocumentSheet {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["aafo", "enchantment-config"],
      dragDrop: [{ dropSelector: "form" }],
      template: "systems/aafo/templates/apps/enchantment-config.hbs",
      width: 500,
      height: "auto",
      sheetConfig: false,
      closeOnSubmit: false,
  ...
  /** @inheritDoc */
  get title() {
    return `${game.i18n.localize("AAFO.Enchantment.Configuration")}: ${this.document.name}`;
  }
  ...
  async getData(options={}) {
    ...
    for ( const effect of this.document.effects ) {
      if ( effect.getFlag("aafo", "type") !== "enchantment" ) effects.push(effect);
    ...
    context.enchantments = context.enchantments.map(effect => ({
      ...
      riderEffects: effects.map(({ id, name }) => ({
        id, name, selected: effect.flags.aafo?.enchantment?.riders?.effect?.includes(id) ? "selected" : ""
      })),
      riderItems: effect.flags.aafo?.enchantment?.riders?.item?.join(",") ?? ""
    }));
  ...
  async _updateObject(event, formData) {
    ...
    const effectsChanges = Object.entries(effects ?? {}).map(([_id, changes]) => {
      ...
      if ( !foundry.utils.hasProperty(updates, "flags.aafo.enchantment.riders.effect") ) {
        foundry.utils.setProperty(updates, "flags.aafo.enchantment.riders.effect", []);
      }
      // End bug fix
      riderIds.add(...(foundry.utils.getProperty(updates, "flags.aafo.enchantment.riders.effect") ?? []));
      return updates;
    });
    for ( const effect of this.document.effects ) {
      if ( effect.getFlag("aafo", "type") === "enchantment" ) continue;
      if ( riderIds.has(effect.id) ) effectsChanges.push({ _id: effect.id, "flags.aafo.rider": true });
      else effectsChanges.push({ _id: effect.id, "flags.aafo.-=rider": null });
    }
