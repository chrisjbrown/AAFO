...
export default class SummoningConfig extends DocumentSheet {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["aafo", "summoning-config"],
      dragDrop: [{ dropSelector: "form" }],
      template: "systems/aafo/templates/apps/summoning-config.hbs",
      width: 500,
      height: "auto",
      sheetConfig: false,
      closeOnSubmit: false,
  ...
  /** @inheritDoc */
  get title() {
    return `${game.i18n.localize("AAFO.Summoning.Configuration")}: ${this.document.name}`;
  }
  ...
  async getData(options={}) {
    ...
    context.creatureSizes = Object.entries(CONFIG.AAFO.actorSizes).reduce((obj, [k, c]) => {
      obj[k] = { label: c.label, selected: context.summons?.creatureSizes.has(k) ? "selected" : "" };
      return obj;
    }, {});
    context.creatureTypes = Object.entries(CONFIG.AAFO.creatureTypes).reduce((obj, [k, c]) => {
