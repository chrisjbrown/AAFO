...
export default class AbilityUseDialog extends Dialog {
  constructor(item, dialogData={}, options={}) {
    ...
    this.options.classes = ["aafo", "dialog"];
  ...
  static async create(item, config, options={}) {
    ...
    const data = {
      ...
      title: game.i18n.format("AAFO.AbilityUseHint", {
    ...
    const html = await renderTemplate("systems/aafo/templates/apps/ability-use.hbs", data);

    // Create the Dialog and return data as a Promise
    const isSpell = item.type === "spell";
    const label = game.i18n.localize(`AAFO.AbilityUse${isSpell ? "Cast" : "Use"}`);
    return new Promise(resolve => {
      const dlg = new this(item, {
        title: `${item.name}: ${game.i18n.localize("AAFO.AbilityUseConfig")}`,
        content: html,
        buttons: {
          use: {
  ...
  static _createConcentrationOptions(item) {
    ...
    return effects.reduce((acc, effect) => {
      ...
      acc.push({
        name: effect.id,
        label: data?.name ?? item.actor.items.get(data)?.name ?? game.i18n.localize("AAFO.ConcentratingItemless")
      });
      return acc;
    }, []);
  }
  ...
  static _createSpellSlotOptions(actor, level) {
    ...
    let lmax = 0;
    const options = Array.fromRange(Object.keys(CONFIG.AAFO.spellLevels).length).reduce((arr, i) => {
      if ( i < level ) return arr;
      const label = CONFIG.AAFO.spellLevels[i];
      ...
      arr.push({
        key: `spell${i}`,
        level: i,
        label: i > 0 ? game.i18n.format("AAFO.SpellLevelSlot", {level: label, n: slots}) : label,
        canCast: max > 0,
        hasSlots: slots > 0
      });
      return arr;
    ...
    for ( const k of Object.keys(CONFIG.AAFO.spellcastingTypes) ) {
      ...
      if ( spellData.level >= level ) {
        options.push({
          ...
          label: `${game.i18n.format(`AAFO.SpellLevel${k.capitalize()}`, {level: spellData.level, n: spellData.value})}`,
          canCast: true,
  ...
  static _createResourceOptions(item) {
    ...
    const spellLevels = Object.keys(CONFIG.AAFO.spellLevels).length - 1;
    ...
    switch ( consume.type ) {
      ...
      case "hitDice": {
        ...
        if ( ["smallest", "largest"].includes(consume.target) ) {
          label = game.i18n.localize(`AAFO.ConsumeHitDice${consume.target.capitalize()}Long`);
          value = target.system.attributes.hd.value;
        } else {
          value = item.actor.system.attributes.hd.bySize[consume.target] ?? 0;
          label = `${game.i18n.localize("AAFO.HitDice")} (${consume.target})`;
        }
        break;
      }
    }
  ...
  static _getAbilityUseNote(item, config) {
    ...
    // Zero quantity
    if ( quantity <= 0 ) return game.i18n.localize("AAFO.AbilityUseUnavailableHint");

    // Abilities which use Recharge
    if ( config.consumeUsage && recharge?.value ) {
      return game.i18n.format(recharge.charged ? "AAFO.AbilityUseChargedHint" : "AAFO.AbilityUseRechargeHint", {
        type: game.i18n.localize(CONFIG.Item.typeLabels[item.type])
      });
    }
    ...
    // Consumables
    if ( uses.autoDestroy ) {
      let str = "AAFO.AbilityUseNormalHint";
      if ( uses.value > 1 ) str = "AAFO.AbilityUseConsumableChargeHint";
      else if ( quantity > 1 ) str = "AAFO.AbilityUseConsumableQuantityHint";
      return game.i18n.format(str, {
        type: game.i18n.localize(`AAFO.Consumable${item.system.type.value.capitalize()}`),
        value: uses.value,
        quantity: quantity,
        max: uses.max,
        per: CONFIG.AAFO.limitedUsePeriods[uses.per]?.label
      });
    }

    // Other Items
    else {
      return game.i18n.format(`AAFO.AbilityUse${uses.value ? "Normal" : "Unavailable"}Hint`, {
        type: game.i18n.localize(CONFIG.Item.typeLabels[item.type]),
        value: uses.value,
        max: uses.max,
        per: CONFIG.AAFO.limitedUsePeriods[uses.per]?.label
  ...
  static _getAbilityUseWarnings(data, options={}) {
    ...
    if ( (scale === "slot") && data.slotOptions.every(o => !o.hasSlots) ) {
      ...
      warnings.push(game.i18n.format("AAFO.SpellCastNoSlotsLeft", {
        name: item.name
      }));
      ...
      warnings.push(game.i18n.format("AAFO.SpellCastNoSlots", {
        level: CONFIG.AAFO.spellLevels[level],
        name: item.name
      }));
    } else if ( (scale === "resource") && foundry.utils.isEmpty(data.resourceOptions) ) {
      // Warn that the resource does not have enough left.
      warnings.push(game.i18n.format("AAFO.ConsumeWarningNoQuantity", {
        name: item.name,
        type: CONFIG.AAFO.abilityConsumptionTypes[consume.type]
      }));
    }
    ...
    if ( item.hasResource ) {
      ...
      if ( isItem && !item.actor.items.get(consume.target) ) {
        warnings.push(game.i18n.format("AAFO.ConsumeWarningNoSource", {
          name: item.name, type: CONFIG.AAFO.abilityConsumptionTypes[consume.type]
        }));
      }
    }
    ...
    if ( item.type === "consumable" ) {
      const type = game.i18n.localize(`AAFO.Consumable${item.system.type.value.capitalize()}`);
      if ( this._willLowerQuantity(item) && (quantity === 1) ) {
        warnings.push(game.i18n.format("AAFO.AbilityUseConsumableDestroyHint", {type}));
      }
      ...
      if ( resource && (resource.system.quantity === 1) && this._willLowerQuantity(resource, qty) ) {
        warnings.push(game.i18n.format("AAFO.AbilityUseConsumableDestroyResourceHint", {type, name: resource.name}));
      }
    }
    ...
    if ( data.concentration.show ) {
      const locale = `AAFO.ConcentratingWarnLimit${data.concentration.optional ? "Optional" : ""}`;
      warnings.push(game.i18n.localize(locale));
    } else if ( data.beginConcentrating && !item.actor.system.attributes?.concentration?.limit ) {
      const locale = "AAFO.ConcentratingWarnLimitZero";
      warnings.push(game.i18n.localize(locale));
    }
  ...
  _onChangeSlotLevel(event) {
    ...
    this._updateProfilesInput(
      "enchantmentProfile", "AAFO.Enchantment.Label", this.constructor._createEnchantmentOptions(item)
    );
    this._updateProfilesInput(
      "summonsProfile", "AAFO.Summoning.Profile.Label", this.constructor._createSummoningOptions(item)
    );
  }
