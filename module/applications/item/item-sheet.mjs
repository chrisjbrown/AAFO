...
export default class ItemSheet5e extends ItemSheet {
  ...
  /** @inheritdoc */
  get template() {
    return `systems/aafo/templates/items/${this.item.type}.hbs`;
  }
  ...
  async getData(options) {
    ...
    foundry.utils.mergeObject(context, {
      ...
      isFormulaRecharge: !!CONFIG.AAFO.limitedUsePeriods[item.system.uses?.per]?.formula,
      isCostlessAction: item.system.activation?.type in CONFIG.AAFO.staticAbilityActivationTypes,
    ...
    if ( !item.isEmbedded && foundry.utils.isEmpty(context.abilityConsumptionTargets) ) {
      context.abilityConsumptionHint = (this.item.system.consume?.type === "attribute")
        ? "AAFO.ConsumeHint.Attribute" : "AAFO.ConsumeHint.Item";
    }

    if ( ("properties" in item.system) && (item.type in CONFIG.AAFO.validProperties) ) {
      context.properties = item.system.validProperties.reduce((obj, k) => {
        const v = CONFIG.AAFO.itemProperties[k];
        obj[k] = {
          label: v.label,
    ...
    if ( ["feat", "loot", "consumable"].includes(item.type) ) {
      ...
      const itemTypes = CONFIG.AAFO[`${name}Types`][item.system.type.value];
      if ( itemTypes ) {
  ...
  async _getItemBaseTypes() {
    const baseIds = this.item.type === "equipment" ? {
      ...CONFIG.AAFO.armorIds,
      ...CONFIG.AAFO.shieldIds
    } : CONFIG.AAFO[`${this.item.type}Ids`];
  ...
  _getItemConsumptionTargets() {
    ...
    // Hit Dice
      ...
      return {
        smallest: game.i18n.localize("AAFO.ConsumeHitDiceSmallest"),
        ...CONFIG.AAFO.hitDieTypes.reduce((obj, hd) => { obj[hd] = hd; return obj; }, {}),
        largest: game.i18n.localize("AAFO.ConsumeHitDiceLargest")
      };
    }
    ...
    // Charges
      ...
      return actor.items.reduce((obj, i) => {
        ...
        if ( uses.per && uses.max ) {
          const label = CONFIG.AAFO.limitedUsePeriods[uses.per]?.formula
            ? ` (${game.i18n.format("AAFO.AbilityUseChargesLabel", {value: uses.value})})`
            : ` (${game.i18n.format("AAFO.AbilityUseConsumableLabel", {max: uses.max, per: uses.per})})`;
          obj[i.id] = i.name + label;
        }

        // Recharging items
        const recharge = i.system.recharge || {};
        if ( recharge.value ) obj[i.id] = `${i.name} (${game.i18n.format("AAFO.Recharge")})`;
        return obj;
      }, {});
    }
    else return {};
  ...
  _getItemStatus() {
    switch ( this.item.type ) {
      case "class":
        return game.i18n.format("AAFO.LevelCount", {ordinal: this.item.system.levels.ordinalString()});
      case "equipment":
      case "weapon":
        return game.i18n.localize(this.item.system.equipped ? "AAFO.Equipped" : "AAFO.Unequipped");
      case "feat":
      case "consumable":
        return this.item.system.type.label;
      case "spell":
        return CONFIG.AAFO.spellPreparationModes[this.item.system.preparation.mode]?.label;
      case "tool":
        return CONFIG.AAFO.proficiencyLevels[this.item.system.prof?.multiplier || 0];
    }
    return null;
  }
  ...
  _getItemProperties() {
    ...
    switch ( this.item.type ) {
      ...
      case "weapon":
        ...
        const ip = CONFIG.AAFO.itemProperties;
        const vp = CONFIG.AAFO.validProperties[this.item.type];
        this.item.system.properties.forEach(k => {
          if ( vp.has(k) ) props.push(ip[k].label);
        });
        break;
      case "equipment":
        props.push(CONFIG.AAFO.equipmentTypes[this.item.system.type.value]);
    ...
    if ( this.item.system.actionType ) {
      props.push(CONFIG.AAFO.itemActionTypes[this.item.system.actionType]);
    }
  ...
  _getSubmitData(updateData={}) {
    ...
    if ( uses?.max ) {
      ...
      if ( !maxRoll.isDeterministic ) {
        ...
        ui.notifications.error(game.i18n.format("AAFO.FormulaCannotContainDiceError", {
          name: game.i18n.localize("AAFO.LimitedUses")
        }));
        return null;
      }
    }
    ...
    if ( duration?.value ) {
      ...
      if ( !durationRoll.isDeterministic ) {
        ...
        ui.notifications.error(game.i18n.format("AAFO.FormulaCannotContainDiceError", {
          name: game.i18n.localize("AAFO.Duration")
        }));
        return null;
      }
    }
    ...
    if ( formData.system?.identifier && !aafo.utils.validators.isValidIdentifier(formData.system.identifier) ) {
      ...
      ui.notifications.error("AAFO.IdentifierError", {localize: true});
      return null;
    }
  ...
  activateListeners(html) {
    ...
    if ( this.isEditable ) {
      ...
      for ( const override of this._getItemOverrides() ) {
        for ( const element of html[0].querySelectorAll(`[name="${override}"]`) ) {
          ...
          element.dataset.tooltip = "AAFO.Enchantment.Warning.Override";
        }
     ...
     * @function aafo.getItemAdvancementContext
     * @memberof hookEvents
     * @param {jQuery} html                      The HTML element to which the context options are attached.
     * @param {ContextMenuEntry[]} entryOptions  The context menu entries.
     */
    Hooks.call("aafo.getItemAdvancementContext", html, contextOptions);
  ...
  _getAdvancementContextMenuOptions() {
    ...
    return [
      {
        name: "AAFO.AdvancementControlEdit",
        icon: "<i class='fas fa-edit fa-fw'></i>",
        condition,
        callback: li => this._onAdvancementAction(li[0], "edit")
      },
      {
        name: "AAFO.AdvancementControlDuplicate",
      ...
      {
        name: "AAFO.AdvancementControlDelete",
  ...
  _onDrop(event) {
     ...
     * @function aafo.dropItemSheetData
     * @memberof hookEvents
     ...
     */
    const allowed = Hooks.call("aafo.dropItemSheetData", item, this, data);
  ...
  async _onDropActiveEffect(event, data) {
    ...
    if ( effect.getFlag("aafo", "type") === "enchantment" ) {
  ...
  async _onDropAdvancement(event, data) {
    ...
    advancements = advancements.filter(a => {
      const validItemTypes = CONFIG.AAFO.advancementTypes[a.constructor.typeName]?.validItemTypes
    ...
    if ( this.item.actor?.system.metadata?.supportsAdvancement && !game.settings.get("aafo", "disableAdvancements") ) {
  ...
  _onAdvancementAction(target, action) {
    ...
    switch (action) {
      case "add": return game.aafo.applications.advancement.AdvancementSelection.createDialog(this.item);
