...
export default class JournalClassPageSheet extends JournalPageSheet {
  ...
  /** @inheritdoc */
  get template() {
    return `systems/aafo/templates/journal/page-${this.document.type}-${this.isEditable ? "edit" : "view"}.hbs`;
  }
  ...
  async _getTable(item, initialLevel=1) {
    ...
    const headers = [[{content: game.i18n.localize("AAFO.Level")}]];
    if ( item.type === "class" ) headers[0].push({content: game.i18n.localize("AAFO.ProficiencyBonus")});
    if ( hasFeatures ) headers[0].push({content: game.i18n.localize("AAFO.Features")});
    ...
    const rows = [];
    for ( const level of Array.fromRange((CONFIG.AAFO.maxLevel - (initialLevel - 1)), initialLevel) ) {
      const features = [];
      for ( const advancement of item.advancement.byLevel[level] ) {
        switch ( advancement.constructor.typeName ) {
          case "AbilityScoreImprovement":
            features.push(game.i18n.localize("AAFO.AdvancementAbilityScoreImprovementTitle"));
            continue;
          case "ItemGrant":
  ...
  async _getSpellProgression(item) {
    ...
    if ( spellcasting.type === "leveled" ) {
      const spells = {};
      const maxSpellLevel = CONFIG.AAFO.SPELL_SLOT_TABLE[CONFIG.AAFO.SPELL_SLOT_TABLE.length - 1].length;
      Array.fromRange(maxSpellLevel, 1).forEach(l => spells[`spell${l}`] = {});

      let largestSlot;
      for ( const level of Array.fromRange(CONFIG.AAFO.maxLevel, 1).reverse() ) {
      ...
      table.headers = [
        [{content: game.i18n.localize("JOURNALENTRYPAGE.AAFO.Class.SpellSlotsPerSpellLevel"), colSpan: largestSlot}],
      ...
      table.headers = [[
        { content: game.i18n.localize("JOURNALENTRYPAGE.AAFO.Class.SpellSlots") },
        { content: game.i18n.localize("JOURNALENTRYPAGE.AAFO.Class.SpellSlotLevel") }
      ]];
      table.cols = [{class: "spellcasting", span: 2}];

      // Loop through each level, gathering "Spell Slots" & "Slot Level" for each one
      for ( const level of Array.fromRange(CONFIG.AAFO.maxLevel, 1) ) {
       ...
       * The actual hook names include the spellcasting type (e.g. `aafo.buildPsionicSpellcastingTable`).
       ...
       * @function aafo.buildSpellcastingTable
       * @memberof hookEvents
       */
      Hooks.callAll(
        `aafo.build${spellcasting.type.capitalize()}SpellcastingTable`, table, item, spellcasting
      );
    }
  ...
  async _getOptionalTable(item) {
    const headers = [[
      { content: game.i18n.localize("AAFO.Level") },
      { content: game.i18n.localize("AAFO.Features") }
    ]];
