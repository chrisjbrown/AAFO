...
export default class ShortRestDialog extends Dialog {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      template: "systems/aafo/templates/apps/short-rest.hbs",
      classes: ["aafo", "dialog"],
      height: "auto"
    });
  }
  ...
  getData() {
    ...
    // Determine rest type
    const variant = game.settings.get("aafo", "restVariant");
  ...
  static async shortRestDialog({ actor }={}) {
    return new Promise((resolve, reject) => {
      const dlg = new this(actor, {
        title: `${game.i18n.localize("AAFO.ShortRest")}: ${actor.name}`,
        buttons: {
          rest: {
            icon: '<i class="fas fa-bed"></i>',
            label: game.i18n.localize("AAFO.Rest"),
            callback: html => {
