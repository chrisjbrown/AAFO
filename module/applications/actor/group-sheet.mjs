...
export default class GroupActorSheet extends ActorSheetMixin(ActorSheet) {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["aafo", "sheet", "actor", "group"],
      template: "systems/aafo/templates/actors/group-sheet.hbs",
      tabs: [{navSelector: ".tabs", contentSelector: ".sheet-body", initial: "members"}],
      scrollY: ["aafo-inventory .inventory-list"],
      width: 620,
      height: 620,
      elements: {
  ...
  async getData(options={}) {
    ...
    // XP
    if ( !game.settings.get("aafo", "disableExperienceTracking") ) context.xp = context.system.details.xp;
    ...
    // Text labels
    context.labels = {
      currencies: Object.entries(CONFIG.AAFO.currencies).reduce((obj, [k, c]) => {
        obj[k] = c.label;
        return obj;
      }, {})
    };
  ...
  #getSummary(stats) {
    ...
    const members = [];
    if ( stats.nMembers ) {
      members.push(`${stats.nMembers} ${game.i18n.localize(`AAFO.Group.Member.${rule.select(stats.nMembers)}`)}`);
    }
    if ( stats.nVehicles ) {
      members.push(`${stats.nVehicles} ${game.i18n.localize(`AAFO.Group.Vehicle.${rule.select(stats.nVehicles)}`)}`);
    }
    if ( !members.length ) return game.i18n.localize("AAFO.GroupSummaryEmpty");
    return game.i18n.format("AAFO.GroupSummary", {members: formatter.format(members)});
  }
  ...
  #prepareMembers() {
    ...
    const displayXP = !game.settings.get("aafo", "disableExperienceTracking");
    for ( const [index, memberData] of this.object.system.members.entries() ) {
      ...
      m.hp.color = aafo.documents.Actor5e.getHPColor(m.hp.current, m.hp.max).css;
  ...
  #prepareMovementSpeed() {
    ...
    let speeds = [
      [movement.land, `${game.i18n.localize("AAFO.MovementLand")} ${movement.land}`],
      [movement.water, `${game.i18n.localize("AAFO.MovementWater")} ${movement.water}`],
      [movement.air, `${game.i18n.localize("AAFO.MovementAir")} ${movement.air}`]
    ];
  ...
  _onClickActionButton(event) {
    ...
    switch ( button.dataset.action ) {
      case "award":
        const award = new Award(this.object, { savedDestinations: this.actor.getFlag("aafo", "awardDestinations") });
        award.render(true);
        break;
      case "longRest":
