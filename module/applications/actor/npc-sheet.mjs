...
export default class ActorSheet5eNPC extends ActorSheet5e {
  ...
  _prepareItems(context) {
    ...
    const features = {
      weapons: { label: game.i18n.localize("AAFO.AttackPl"), items: [], hasActions: true,
        dataset: {type: "weapon", "weapon-type": "natural"} },
      actions: { label: game.i18n.localize("AAFO.ActionPl"), items: [], hasActions: true,
        dataset: {type: "feat", "activation.type": "action"} },
      passive: { label: game.i18n.localize("AAFO.Features"), items: [], dataset: {type: "feat"} },
      equipment: { label: game.i18n.localize("AAFO.Inventory"), items: [], dataset: {type: "loot"}}
    };

    // Start by classifying items into groups for rendering
    const maxLevelDelta = CONFIG.AAFO.maxLevel - (this.actor.system.details.level ?? 0);
    let [spells, other] = context.items.reduce((arr, item) => {
      ...
      ctx.canToggle = false;
      if ( item.type === "class" ) ctx.availableLevels = Array.fromRange(CONFIG.AAFO.maxLevel, 1).map(level => ({
  ...
  getArmorLabel() {
    ...
    const label = [];
    if ( ac.calc === "default" ) label.push(this.actor.armor?.name || game.i18n.localize("AAFO.ArmorClassUnarmored"));
    else label.push(game.i18n.localize(CONFIG.AAFO.armorClasses[ac.calc].label));
