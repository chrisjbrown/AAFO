...
export default class ActorSheet5eCharacter extends ActorSheet5e {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["aafo", "sheet", "actor", "character"]
    });
  }
  ...
  async getData(options={}) {
    ...
    context.resources = ["primary", "secondary", "tertiary"].reduce((arr, r) => {
      const res = foundry.utils.mergeObject(context.actor.system.resources[r] || {}, {
        name: r,
        placeholder: game.i18n.localize(`AAFO.Resource${r.titleCase()}`)
      }, {inplace: false});
    ...
    return foundry.utils.mergeObject(context, {
      disableExperience: game.settings.get("aafo", "disableExperienceTracking"),
      ...
      weightUnit: game.i18n.localize(`AAFO.Abbreviation${
        game.settings.get("aafo", "metricWeightUnits") ? "Kg" : "Lbs"}`),
  ...
  _prepareItems(context) {
    ...
    let {items, spells, feats, perks, races, backgrounds, classes, subclasses} = context.items.reduce((obj, item) => {
      ...
      if ( item.system.attunement ) ctx.attunement = item.system.attuned ? {
        icon: "fa-sun",
        cls: "attuned",
        title: "AAFO.AttunementAttuned"
      } : {
        icon: "fa-sun",
        cls: "not-attuned",
        title: CONFIG.AAFO.attunementTypes[item.system.attunement]
      };
      ...
      // Item grouping
      const [originId] = item.getFlag("aafo", "advancementOrigin")?.split(".") ?? [];
      ...
      else if ( item.type === "perk" ) obj.perks.push(item);
      ...
      return obj;
    }, { items: [], spells: [], feats: [], perks: [], races: [], backgrounds: [], classes: [], subclasses: [] });
    ...
    const maxLevelDelta = CONFIG.AAFO.maxLevel - this.actor.system.details.level;
    classes = classes.reduce((arr, cls) => {
      const ctx = context.itemContext[cls.id] ??= {};
      ctx.availableLevels = Array.fromRange(CONFIG.AAFO.maxLevel + 1).slice(1).map(level => {
    ...
    for ( const subclass of subclasses ) {
      feats.push(subclass);
      const message = game.i18n.format("AAFO.SubclassMismatchWarn", {
    ...
    const features = {
      perks: {
        label: CONFIG.Item.typeLabels.perk, items: perks,
        hasActions: false, dataset: {type: "perk"},
      },
      race: {
      ...
      active: {
        label: "AAFO.FeatureActive", items: [],
        hasActions: true, dataset: {type: "feat", "activation.type": "action"}
      },
      passive: {
        label: "AAFO.FeaturePassive", items: [],
  ...
  _prepareItem(item, context) {
    if ( item.type === "spell" ) {
      ...
      if ( isAlways ) context.toggleTitle = CONFIG.AAFO.spellPreparationModes.always.label;
      else if ( isPrepared ) context.toggleTitle = CONFIG.AAFO.spellPreparationModes.prepared.label;
      else context.toggleTitle = game.i18n.localize("AAFO.SpellUnprepared");
    }
    else {
      const isActive = !!item.system.equipped;
      context.toggleClass = isActive ? "active" : "";
      context.toggleTitle = game.i18n.localize(isActive ? "AAFO.Equipped" : "AAFO.Unequipped");
    ...
    switch ( button.dataset.action ) {
      case "convertCurrency":
        return Dialog.confirm({
          title: `${game.i18n.localize("AAFO.CurrencyConvert")}`,
          content: `<p>${game.i18n.localize("AAFO.CurrencyConvertHint")}</p>`,
  ...
  /**
   * Level up character, calling the relevant function on the Actor instance.
  ...
  async _onDropSingleItem(itemData) {
    ...
    // If a subclass is dropped, ensure it doesn't match another subclass with the same identifier
      ...
      if ( other ) {
        const err = game.i18n.format("AAFO.SubclassDuplicateError", {identifier: other.identifier});
        ui.notifications.error(err);
        return false;
      }
      const cls = this.actor.itemTypes.class.find(i => i.identifier === itemData.system.classIdentifier);
      if ( cls && cls.subclass ) {
        const err = game.i18n.format("AAFO.SubclassAssignmentError", {class: cls.name, subclass: cls.subclass.name});
        ui.notifications.error(err);
        return false;
      }
    }
