...
export default class ProficiencyConfig extends BaseConfigSheet {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["aafo"],
      template: "systems/aafo/templates/apps/proficiency-config.hbs",
      width: 500,
      height: "auto"
    });
  }
  ...
  /** @inheritdoc */
  get title() {
    const label = this.isSkill ? CONFIG.AAFO.skills[this.options.key].label
      : Trait.keyLabel(this.options.key, { trait: "tool" });
    return `${game.i18n.format("AAFO.ProficiencyConfigureTitle", {label})}: ${this.document.name}`;
  }
  ...
  getData(options={}) {
    return {
      ...
      proficiencyLevels: CONFIG.AAFO.proficiencyLevels,
  ...
  async _updateObject(event, formData) {
    ...
    if ( !passiveRoll.isDeterministic ) {
      const message = game.i18n.format("AAFO.FormulaCannotContainDiceError", {
        name: game.i18n.localize("AAFO.SkillBonusPassive")
      });
