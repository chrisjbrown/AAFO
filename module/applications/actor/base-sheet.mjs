...
import ActorStaminaPointsConfig from "./stamina-points-config.mjs";
import ActorActionPointsConfig from "./action-points-config.mjs";
...
export default class ActorSheet5e extends ActorSheetMixin(ActorSheet) {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      scrollY: [
        "aafo-inventory .inventory-list",
        "aafo-effects .effects-list",
        ".center-pane"
      ],
      tabs: [{navSelector: ".tabs", contentSelector: ".sheet-body", initial: "description"}],
      width: 720,
      height: Math.max(680, Math.max(
        237 + (Object.keys(CONFIG.AAFO.abilities).length * 70),
        240 + (Object.keys(CONFIG.AAFO.skills).length * 24)
      )),
      elements: {
        effects: "aafo-effects",
        inventory: "aafo-inventory"
      }
    });
  }
  ...
  /** @override */
  get template() {
    if ( !game.user.isGM && this.actor.limited ) return "systems/aafo/templates/actors/limited-sheet.hbs";
    return `systems/aafo/templates/actors/${this.actor.type}-sheet.hbs`;
  }
  ...
  async getData(options) {
    ...
    for ( const [a, abl] of Object.entries(context.abilities) ) {
      ...
      abl.hover = CONFIG.AAFO.proficiencyLevels[abl.proficient];
      abl.label = CONFIG.AAFO.abilities[a]?.label;
    ...
    const baseAbility = (prop, key) => {
      ...
      if ( src ) return src;
      if ( prop === "skills" ) src = CONFIG.AAFO.skills[key]?.ability;
      return src ?? "int";
    };
    ["skills", "tools"].forEach(prop => {
      for ( const [key, entry] of Object.entries(context[prop]) ) {
        entry.abbreviation = CONFIG.AAFO.abilities[entry.ability]?.abbreviation;
        entry.icon = this._getProficiencyIcon(entry.value);
        entry.hover = CONFIG.AAFO.proficiencyLevels[entry.value];
        entry.label = (prop === "skills") ? CONFIG.AAFO.skills[key]?.label : Trait.keyLabel(key, {trait: "tool"});
  ...
  _getLabels() {
    ...
    // Currency Labels
    labels.currencies = Object.entries(CONFIG.AAFO.currencies).reduce((obj, [k, c]) => {
      obj[k] = c.label;
      return obj;
    }, {});

    // Proficiency
    labels.proficiency = game.settings.get("aafo", "proficiencyModifier") === "dice"
  ...
  _getMovementSpeed(systemData, largestPrimary=false) {
    ...
    let speeds = [
      [movement.burrow, `${game.i18n.localize("AAFO.MovementBurrow")} ${movement.burrow}`],
      [movement.climb, `${game.i18n.localize("AAFO.MovementClimb")} ${movement.climb}`],
      [movement.fly, `${game.i18n.localize("AAFO.MovementFly")} ${movement.fly}${movement.hover ? ` (${game.i18n.localize("AAFO.MovementHover")})` : ""}`],
      [movement.swim, `${game.i18n.localize("AAFO.MovementSwim")} ${movement.swim}`]
    ];
    if ( largestPrimary ) {
      speeds.push([movement.walk, `${game.i18n.localize("AAFO.MovementWalk")} ${movement.walk}`]);
    }
    ...
    if ( largestPrimary ) {
      ...
      return {
        primary: `${primary ? primary[1] : "0"} ${movement.units || Object.keys(CONFIG.AAFO.movementUnits)[0]}`,
    ...
    // Case 2: Walk as primary
      ...
      return {
        primary: `${movement.walk || 0} ${movement.units || Object.keys(CONFIG.AAFO.movementUnits)[0]}`,
  ...
  _getSenses(systemData) {
    ...
    const tags = {};
    for ( let [k, label] of Object.entries(CONFIG.AAFO.senses) ) {
      const v = senses[k] ?? 0;
      if ( v === 0 ) continue;
      tags[k] = `${game.i18n.localize(label)} ${v} ${senses.units ?? Object.keys(CONFIG.AAFO.movementUnits)[0]}`;
    }
  ...
  _prepareTraits(systemData) {
    const traits = {};
    for ( const [trait, traitConfig] of Object.entries(CONFIG.AAFO.traits) ) {
      ...
      if ( data.bypasses?.size ) {
        values = values.filter(t => {
          if ( !CONFIG.AAFO.damageTypes[t]?.isPhysical ) return true;
          physical.push(t);
          return false;
        });
      }
      ...
      if ( physical.length ) {
        ...
        data.selected.physical = game.i18n.format("AAFO.DamagePhysicalBypasses", {
          ...
          bypassTypes: bypassFormatter.format(data.bypasses.reduce((acc, t) => {
            const v = CONFIG.AAFO.itemProperties[t];
      ...
      if ( (trait === "dr") && this.document.hasConditionEffect("petrification") ) {
        data.selected = { custom1: game.i18n.localize("AAFO.DamageAll") };
        data.cssClass = "";
      }
    }
    return traits;
  ...
  _prepareSpellbook(context, spells) {
    ...
    const sections = Object.entries(CONFIG.AAFO.spellPreparationModes).reduce((acc, [k, {order}]) => {
    ...
    const maxLevel = Array.fromRange(Object.keys(CONFIG.AAFO.spellLevels).length - 1, 1).reduce((max, i) => {
    ...
    if ( maxLevel > 0 ) {
      registerSection("spell0", 0, CONFIG.AAFO.spellLevels[0]);
      for (let lvl = 1; lvl <= maxLevel; lvl++) {
        const sl = `spell${lvl}`;
        registerSection(sl, lvl, CONFIG.AAFO.spellLevels[lvl], levels[sl]);
      }
    }

    // Create spellbook sections for all alternative spell preparation modes that have spell slots.
    for ( const [k, v] of Object.entries(CONFIG.AAFO.spellPreparationModes) ) {
      if ( !(k in levels) || !v.upcast || !levels[k].max ) continue;

      if ( !spellbook["0"] && v.cantrips ) registerSection("spell0", 0, CONFIG.AAFO.spellLevels[0]);
      const l = levels[k];
      const level = game.i18n.localize(`AAFO.SpellLevel${l.level}`);
    ...
    spells.forEach(spell => {
      ...
      // Sections for higher-level spells which the caster "should not" have, but spell items exist for
        ...
        registerSection(sl, s, CONFIG.AAFO.spellLevels[s], {levels: levels[sl]});
      }
  ...
  _filterItems(items, filters) {
    const spellSchools = new Set(Object.keys(CONFIG.AAFO.spellSchools));
   ...
   * @param {number} level  A proficiency mode defined in `CONFIG.AAFO.proficiencyLevels`.
  ...
  activateListeners(html) {
    ...
    if ( this.isEditable ) {
      ...
      html.find(".config-adjust").click(this._onAttributeAdjust.bind(this));
  ...
  _disableOverriddenFields(html) {
    ...
    for ( const override of Object.keys(foundry.utils.flattenObject(this.actor.overrides)) ) {
      html.find(`input[name="${override}"],select[name="${override}"]`).each((i, el) => {
        el.disabled = true;
        el.dataset.tooltip = "AAFO.ActiveEffectOverrideWarning";
      });
      ...
      for ( const [key, regex] of Object.entries(proficiencyToggles) ) {
        ...
        if ( match ) {
          ...
          toggle.attr("data-tooltip", "AAFO.ActiveEffectOverrideWarning");
        }
      }

      const [, spell] = override.match(/system\.spells\.(spell\d)\.override/) || [];
      if ( spell ) {
        html.find(`.spell-max[data-level="${spell}"]`).attr("data-tooltip", "AAFO.ActiveEffectOverrideWarning");
      }
    }
  }
  ...
  _onConfigMenu(event) {
    ...
    switch ( button.dataset.action ) {
      ...
      case "hitPoints":
        ...
        break;
      case "staminaPoints":
        app = new ActorStaminaPointsConfig(this.actor);
        break;
      case "actionPoints":
        app = new ActorActionPointsConfig(this.actor);
        break;
      case "initiative":
   ...
   * @private
   */
  _onAttributeAdjust(event) {
    event.preventDefault();
    event.stopPropagation();
    const button = event.currentTarget;
    const {action, adjustment} = button.dataset
    switch ( action ) {
      case "skill":
        const skill = event.currentTarget.closest("[data-key]").dataset.key;
        const skillPath = `system.skills.${skill}.value`;
        const skillValue = this.actor.system.skills[skill].value;

        if (adjustment === 'increment') this._incrementAttribute(skillPath, skillValue);
        else if (adjustment === 'decrement') this._decrementAttribute(skillPath, skillValue);
        break;
      case "ability":
        const ability = event.currentTarget.closest("[data-ability]").dataset.ability;
        const abiPath = `system.abilities.${ability}.value`;
        const abiValue = this.actor.system.abilities[ability].value;

        if (adjustment === 'increment') this._incrementAttribute(abiPath, abiValue);
        else if (adjustment === 'decrement') this._decrementAttribute(abiPath, abiValue);
        break;
    }
  }

  _incrementAttribute(path, value) {
    this.actor.update({ [path]: value + 1 })
  }

  _decrementAttribute(path, value) {
    this.actor.update({ [path]: value - 1 })
  }
  ...
  async _onDropActor(event, data) {
    const canPolymorph = game.user.isGM || (this.actor.isOwner && game.settings.get("aafo", "allowPolymorphing"));
    ...
    const rememberOptions = html => {
      ...
      const settings = foundry.utils.mergeObject(game.settings.get("aafo", "polymorphSettings") ?? {}, options);
      game.settings.set("aafo", "polymorphSettings", settings);
      return settings;
    };

    // Create and render the Dialog
    return new Dialog({
      title: game.i18n.localize("AAFO.PolymorphPromptTitle"),
      content: {
        options: game.settings.get("aafo", "polymorphSettings"),
        settings: CONFIG.AAFO.polymorphSettings,
        effectSettings: CONFIG.AAFO.polymorphEffectSettings,
      ...
      buttons: {
        accept: {
          ...
          label: game.i18n.localize("AAFO.PolymorphAcceptSettings"),
          callback: html => this.actor.transformInto(sourceActor, rememberOptions(html))
        },
        wildshape: {
          icon: CONFIG.AAFO.transformationPresets.wildshape.icon,
          label: CONFIG.AAFO.transformationPresets.wildshape.label,
          callback: html => this.actor.transformInto(sourceActor, foundry.utils.mergeObject(
            CONFIG.AAFO.transformationPresets.wildshape.options,
            { transformTokens: rememberOptions(html).transformTokens }
          ))
        },
        polymorph: {
          icon: CONFIG.AAFO.transformationPresets.polymorph.icon,
          label: CONFIG.AAFO.transformationPresets.polymorph.label,
          callback: html => this.actor.transformInto(sourceActor, foundry.utils.mergeObject(
            CONFIG.AAFO.transformationPresets.polymorph.options,
            { transformTokens: rememberOptions(html).transformTokens }
          ))
        },
        self: {
          icon: CONFIG.AAFO.transformationPresets.polymorphSelf.icon,
          label: CONFIG.AAFO.transformationPresets.polymorphSelf.label,
          callback: html => this.actor.transformInto(sourceActor, foundry.utils.mergeObject(
            CONFIG.AAFO.transformationPresets.polymorphSelf.options,
            { transformTokens: rememberOptions(html).transformTokens }
          ))
        },
        cancel: {
      ...
      classes: ["dialog", "aafo", "polymorph"],
      width: 900,
      template: "systems/aafo/templates/apps/polymorph-prompt.hbs"
    }).render(true);
  }
  ...
  async _onDropItemCreate(itemData) {
    ...
    if ( multipleAdvancements && !game.settings.get("aafo", "disableAdvancements") ) {
      ui.notifications.warn(game.i18n.format("AAFO.WarnCantAddMultipleAdvancements"));
  ...
  async _onDropSingleItem(itemData) {
    ...
    if ( this.constructor.unsupportedItemTypes.has(itemData.type) ) {
      ui.notifications.warn(game.i18n.format("AAFO.ActorWarningInvalidItem", {
  ...
  _onDropSpell(itemData) {
    ...
    const spellcastKeys = Object.keys(CONFIG.AAFO.spellcastingTypes);
  ...
  _getHeaderButtons() {
    ...
    if ( this.actor.isPolymorphed ) {
      buttons.unshift({
        label: "AAFO.PolymorphRestoreTransformation",
