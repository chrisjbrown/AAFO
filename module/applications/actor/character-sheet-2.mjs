...
export default class ActorSheet5eCharacter2 extends ActorSheet5eCharacter {
  constructor(object, options={}) {
    ...
    const { width, height } = game.user.getFlag("aafo", `sheetPrefs.${key}`) ?? {};
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["aafo2", "sheet", "actor", "character"],
  ...
  /** @override */
  get template() {
    if ( !game.user.isGM && this.actor.limited ) return "systems/aafo/templates/actors/limited-sheet-2.hbs";
    return "systems/aafo/templates/actors/character-sheet-2.hbs";
  }
  ...
  async _renderOuter() {
    ...
    if ( this.isEditable ) {
      ...
      toggle.dataset.tooltip = "AAFO.SheetModeEdit";
      toggle.setAttribute("aria-label", game.i18n.localize("AAFO.SheetModeEdit"));
    ...
    nav.append(...this.constructor.TABS.map(({ tab, label, icon, svg }) => {
      ...
      else if ( svg ) item.innerHTML = `<aafo-icon src="systems/aafo/icons/svg/${svg}.svg"></aafo-icon>`;
      return item;
    }));
    ...
    // setTheme(html[0], this.actor.getFlag("aafo", "theme"));
  ...
  async getData(options) {
    ...
    const sidebarCollapsed = game.user.getFlag("aafo", `sheetPrefs.character.tabs.${activeTab}.collapseSidebar`);
    if ( sidebarCollapsed ) {
    ...
    // Portrait
    const showTokenPortrait = this.actor.getFlag("aafo", "showTokenPortrait") === true;
    ...
    // Exhaustion
    // const max = CONFIG.AAFO.conditionTypes.exhaustion.levels;
    // context.exhaustion = Array.fromRange(max, 1).reduce((acc, n) => {
    //   const label = game.i18n.format("AAFO.ExhaustionLevel", { n });
    //   const classes = ["pip"];
    ...
    // Speed
    context.speed = Object.entries(CONFIG.AAFO.movementTypes).reduce((obj, [k, label]) => {
      const value = attributes.movement[k];
      if ( value > obj.value ) Object.assign(obj, { value, label });
      return obj;
    }, { value: 0, label: CONFIG.AAFO.movementTypes.walk });
    ...
    ["success", "failure"].forEach(deathSave => {
      ...
      for ( let i = 1; i < 4; i++ ) {
        ...
        const i18nKey = `AAFO.DeathSave${deathSave.titleCase()}Label`;
    ...
    // Ability Scores
    context.abilities = Object.entries(context.abilities).reduce((obj, [k, ability]) => {
      ability.key = k;
      ability.abbr = CONFIG.AAFO.abilities[k]?.abbreviation ?? "";
      ability.sign = Math.sign(ability.mod) < 0 ? "-" : "+";
      ability.mod = Math.abs(ability.mod);
      ability.baseValue = context.source.abilities[k]?.value ?? 0;
      ability.reference = CONFIG.AAFO.abilities[k]?.reference ?? "";
      obj.push(ability);
      return obj;
    }, []);

    // Penalties
    context.penalties = {...this.actor.system.attributes.penalties};
    for ( const [key, entry] of Object.entries(context.penalties) ) {
      entry.label = CONFIG.AAFO.penalties[key].label;
      entry.reference = CONFIG.AAFO.penalties[key].label;
      entry.pct = (entry.value / entry.max) * 100;
    }
    ...
    //   ability.hover = CONFIG.AAFO.proficiencyLevels[ability.proficient];
    ...
    //     label: game.i18n.localize("AAFO.Concentration"),
    //     abbr: game.i18n.localize("AAFO.Concentration"),
    ...
    // Size
    context.size = {
      label: CONFIG.AAFO.actorSizes[traits.size]?.label ?? traits.size,
      abbr: CONFIG.AAFO.actorSizes[traits.size]?.abbreviation ?? "—",
    ...
    for ( const [key, entry] of Object.entries(context.skills).concat(Object.entries(context.tools)) ) {
      ...
      entry.mod = entry.total;
      if ( key in CONFIG.AAFO.skills ) entry.reference = CONFIG.AAFO.skills[key].reference;
      else if ( key in CONFIG.AAFO.toolIds ) entry.reference = Trait.getBaseItemUUID(CONFIG.AAFO.toolIds[key]);
    }

    // Character Background
    context.creatureType = {
      class: details.type.value === "custom" ? "none" : "",
      icon: CONFIG.AAFO.creatureTypes[details.type.value]?.icon ?? "icons/svg/mystery-man.svg",
      title: details.type.value === "custom"
        ? details.type.custom
        : CONFIG.AAFO.creatureTypes[details.type.value]?.label,
      reference: CONFIG.AAFO.creatureTypes[details.type.value]?.reference,
      subtitle: details.type.subtype
    };

    if ( details.race instanceof aafo.documents.Item5e ) context.race = details.race;
    if ( details.background instanceof aafo.documents.Item5e ) context.background = details.background;

    // Senses
    context.senses = Object.entries(CONFIG.AAFO.senses).reduce((obj, [k, label]) => {
    ...
    for ( const item of Object.values(this.actor.classes).sort((a, b) => b.system.levels - a.system.levels) ) {
      ...
      context.spellcasting.push({
        label: game.i18n.format("AAFO.SpellcastingClass", { class: name }),
    ...
    context.conditions = Object.entries(CONFIG.AAFO.conditionTypes).reduce((arr, [k, c]) => {
      if ( c.pseudo ) return arr; // Filter out pseudo-conditions.
      const { label: name, icon, reference } = c;
      const id = staticID(`aafo${k}`);
      conditionIds.add(id);
    ...
    for ( const category of Object.values(context.effects) ) {
      category.effects = await category.effects.reduce(async (arr, effect) => {
        ...
        if ( (source instanceof aafo.documents.ActiveEffect5e) && (source.target !== this.object) ) {
        ...
        arr.push({
          ...
          hasTooltip: source instanceof aafo.documents.Item5e
        });
        return arr;
      }, []);
    }
  ...
  _getLabels() {
    ...
    labels.damageAndHealing = { ...CONFIG.AAFO.damageTypes, ...CONFIG.AAFO.healingTypes };
    return labels;
  }
  ...
  _prepareTraits() {
    const traits = {};
    for ( const [trait, config] of Object.entries(CONFIG.AAFO.traits) ) {
      ...
      values = values.map(key => {
        ...
        if ( data.bypasses?.size && CONFIG.AAFO.damageTypes[key]?.isPhysical ) icons.push(...data.bypasses);
        return value;
      });
    ...
    if ( this.document.hasConditionEffect("petrification") ) {
      traits.dr = [{ label: game.i18n.localize("AAFO.DamageAll") }];
    }
    ...
    if ( dm ) {
      ...
      const values = Object.entries(dm.amount).map(([k, v]) => {
        ...
        const value = {
          label: `${CONFIG.AAFO.damageTypes[k]?.label ?? k} ${formatNumber(total, { signDisplay: "always" })}`,
          color: total > 0 ? "maroon" : "green"
        };
        const icons = value.icons = [];
        if ( dm.bypasses.size && CONFIG.AAFO.damageTypes[k]?.isPhysical ) icons.push(...dm.bypasses);
        return value;
      }).filter(f => f);
  ...
  _prepareItems(context) {
    ...
    context.inventory.push({ label: "AAFO.Contents", items: [], dataset: { type: "all" } });
    ...
    if ( this.actor.system.details.race instanceof aafo.documents.Item5e ) {
      context.features.push({ label: "AAFO.FeaturesRace", items: [], dataset: { type: "race" } });
    }

    if ( this.actor.system.details.background instanceof aafo.documents.Item5e ) {
      context.features.push({ label: "AAFO.FeaturesBackground", items: [], dataset: { type: "background" } });
    }

    context.features.push({ label: "AAFO.FeaturesPerks", items: [], dataset: { type: "other" } });
    ...
    context.spellbook.forEach(section => {
      ...
      section.pips = Array.fromRange(max, 1).map(n => {
        ...
        const label = filled
          ? game.i18n.format(`AAFO.SpellSlotN.${plurals.select(n)}`, { n })
          : game.i18n.localize("AAFO.SpellSlotExpended");
        const classes = ["pip"];
        if ( filled ) classes.push("filled");
  ...
  _prepareItem(item, ctx) {
    ...
    if ( item.type === "spell" ) {
      ...
      const abbr = {
        action: "AAFO.ActionAbbr",
        bonus: "AAFO.BonusActionAbbr",
        reaction: "AAFO.ReactionAbbr",
        minute: "AAFO.TimeMinuteAbbr",
        hour: "AAFO.TimeHourAbbr",
        day: "AAFO.TimeDayAbbr"
      ...
      if ( units && (units !== "none") ) {
        if ( units in CONFIG.AAFO.movementUnits ) {
          ctx.range = {
            distance: true,
            value: system.range.value,
            unit: game.i18n.localize(`AAFO.Dist${units.capitalize()}Abbr`)
          };
        }
      ...
      const config = CONFIG.AAFO.spellPreparationModes[mode] ?? {};
      if ( config.prepares ) {
        ...
        ctx.preparation = {
          ...
          title: isAlways
            ? CONFIG.AAFO.spellPreparationModes.always.label
            : prepared
              ? CONFIG.AAFO.spellPreparationModes.prepared.label
              : game.i18n.localize("AAFO.SpellUnprepared")
        };
      }
    ...
    // Gear
      ...
      if ( "equipped" in system ) {
        ctx.equip = {
          ...
          title: `AAFO.${system.equipped ? "Equipped" : "Unequipped"}`,
  ...
  activateListeners(html) {
    ...
    if ( this.isEditable ) {
      [".hit-points", ".stamina-points", ".action-points"].forEach(selector => {
        html.find(`.meter > ${selector}`).on("click", event => this._toggleEditRange(event, selector, true));
        html.find(`.meter > ${selector} > input`).on("blur", event => this._toggleEditRange(event, selector, false));
  ...
  _onChangeTab(event, tabs, active) {
    ...
    const sidebarCollapsed = game.user.getFlag("aafo", `sheetPrefs.character.tabs.${active}.collapseSidebar`);
  ...
  _onDragStart(event) {
    ...
    const modes = CONFIG.AAFO.spellPreparationModes;
    ...
    let type;
    if ( key in CONFIG.AAFO.skills ) type = "skill";
    else if ( key in CONFIG.AAFO.toolIds ) type = "tool";
    else if ( modes[preparationMode]?.upcast && (level !== "0") && isSlots ) type = "slots";
    if ( !type ) return super._onDragStart(event);
    const dragData = { aafo: { action: "favorite", type } };
    if ( type === "slots" ) dragData.aafo.id = (preparationMode === "prepared") ? `spell${level}` : preparationMode;
    else dragData.aafo.id = key;
  ...
  async _onChangeSheetMode(event) {
    ...
    const label = game.i18n.localize(`AAFO.SheetMode${toggle.checked ? "Play" : "Edit"}`);
   ...
   * @param {PointerEvent} event  The triggering event.
   * @param {selector} string  The triggering element selector.
   * @param {boolean} edit  Whether to toggle to the edit state.
   * @protected
   */
  _toggleEditRange(event, selector, edit) {
  ...
  _toggleDeathTray(open) {
    ...
    tab.dataset.tooltip = `AAFO.DeathSave${this._deathTrayOpen ? "Hide" : "Show"}`;
  ...
  _onToggleSidebar() {
    ...
    game.user.setFlag("aafo", `sheetPrefs.character.tabs.${activeTab}.collapseSidebar`, collapsed);
  }
  ...
  _onCreateChild() {
    ...
    if ( activeTab === "effects" ) return ActiveEffect.implementation.create({
      name: game.i18n.localize("AAFO.EffectNew"),
  ...
  _onFindItem(type) {
    switch ( type ) {
      case "class": game.packs.get(CONFIG.AAFO.sourcePacks.CLASSES)?.render(true); break;
      case "race": game.packs.get(CONFIG.AAFO.sourcePacks.RACES)?.render(true); break;
      case "background": game.packs.get(CONFIG.AAFO.sourcePacks.BACKGROUNDS)?.render(true); break;
    }
  }
  ...
  _applyItemTooltips(element) {
    ...
    `;
    element.dataset.tooltipClass = "aafo2 aafo-tooltip item-tooltip";
  ...
  _onResize(event) {
    ...
    game.user.setFlag("aafo", `sheetPrefs.${key}`, { width, height });
  }
  ...
  async _onDrop(event) {
    ...
    const { action, type, id } = data.aafo ?? {};
  ...
  _onUseFavorite(event) {
    ...
    if ( favorite instanceof aafo.documents.Item5e ) return favorite.use({}, { event });
    if ( favorite instanceof aafo.documents.ActiveEffect5e ) return favorite.update({ disabled: !favorite.disabled });
  }
  ...
  async _prepareFavorites() {
    ...
    const resources = Object.entries(this.actor.system.resources).reduce((arr, [k, r]) => {
      ...
      if ( label && max ) arr.push({
        ...
        title: label,
        subtitle: [
          sr ? game.i18n.localize("AAFO.AbbreviationSR") : null,
          lr ? game.i18n.localize("AAFO.AbbreviationLR") : null
        ].filterJoin(" &bull; ")
      });
      return arr;
    }, []);
    ...
    return resources.concat(await this.actor.system.favorites.reduce(async (arr, f) => {
      ...
      if ( suppressed ) subtitle = game.i18n.localize("AAFO.Suppressed");
      arr.push({
  ...
  async _getFavoriteData(type, id) {
    ...
    if ( type === "slots" ) {
      ...
      if ( !/spell\d+/.test(id) ) return {
        uses, level,
        title: game.i18n.localize(`AAFO.SpellSlots${id.capitalize()}`),
        subtitle: [
          game.i18n.localize(`AAFO.SpellLevel${level}`),
          game.i18n.localize(`AAFO.Abbreviation${CONFIG.AAFO.spellcastingTypes[id]?.shortRest ? "SR" : "LR"}`)
        ],
        img: CONFIG.AAFO.spellcastingTypes[id]?.img || CONFIG.AAFO.spellcastingTypes.pact.img
      };

      const plurals = new Intl.PluralRules(game.i18n.lang, { type: "ordinal" });
      const isSR = CONFIG.AAFO.spellcastingTypes.leveled.shortRest;
      return {
        uses, level,
        title: game.i18n.format(`AAFO.SpellSlotsN.${plurals.select(level)}`, { n: level }),
        subtitle: game.i18n.localize(`AAFO.Abbreviation${isSR ? "SR" : "LR"}`),
        img: CONFIG.AAFO.spellcastingTypes.leveled.img.replace("{id}", id)
      };
    }
    ...
    // Skills & Tools
      ...
      const subtitle = game.i18n.format("AAFO.AbilityPromptTitle", {
        ability: CONFIG.AAFO.abilities[ability].label
      });
      let img;
      let title;
      let reference;
      if ( type === "tool" ) {
        reference = Trait.getBaseItemUUID(CONFIG.AAFO.toolIds[id]);
        ({ img, name: title } = Trait.getBaseItem(reference, { indexOnly: true }));
      }
      else if ( type === "skill" ) ({ icon: img, label: title, reference } = CONFIG.AAFO.skills[id]);
