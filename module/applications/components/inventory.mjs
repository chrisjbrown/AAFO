...
export default class InventoryElement extends HTMLElement {
  ...
  /**
   * TODO: Remove filtering code from aafo-inventory when all sheets use item-list-controls.
  ...
  _getContextOptions(item) {
    ...
    if ( item.system.attunement ) {
      options.push({
        name: item.system.attuned ? "AAFO.ContextMenuActionUnattune" : "AAFO.ContextMenuActionAttune",
    ...
    if ( "equipped" in item.system ) options.push({
      name: item.system.equipped ? "AAFO.ContextMenuActionUnequip" : "AAFO.ContextMenuActionEquip",
    ...
    // Toggle Prepared State
      ...
      name: item.system?.preparation?.prepared ? "AAFO.ContextMenuActionUnprepare" : "AAFO.ContextMenuActionPrepare",
      icon: "<i class='fas fa-sun fa-fw'></i>",
      condition: () => item.isOwner,
  ...
  async _onCreate(target) {
    ...
    if ( type === "class" && (this.actor.system.details.level + 1 > CONFIG.AAFO.maxLevel) ) {
    ...
    const itemData = {
      name: game.i18n.format("AAFO.ItemNew", {type: game.i18n.localize(CONFIG.Item.typeLabels[type])}),
      type,
  ...
  async _onExpand(target, item) {
    ...
    if ( this._app._expanded.has(item.id) ) {
      ...
      const summary = $(await renderTemplate("systems/aafo/templates/items/parts/item-summary.hbs", await chatData));
  ...
  _onOpenContextMenu(element) {
    ...
    Hooks.call("aafo.getItemContextOptions", item, ui.context.menuItems);
  }
}
