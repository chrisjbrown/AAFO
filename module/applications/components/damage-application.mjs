...
export default class DamageApplicationElement extends ChatTrayElement {
    ...
    if ( !this.targetList ) {
      ...
      div.innerHTML = `
        <label class="roboto-upper">
          ...
          <span>${game.i18n.localize("AAFO.Apply")}</span>
          ...
          <div class="wrapper">
            <div class="target-source-control">
              <button type="button" class="unbutton" data-mode="targeted" aria-pressed="false">
                <i class="fa-solid fa-bullseye" inert></i> ${game.i18n.localize("AAFO.Tokens.Targeted")}
              </button>
              <button type="button" class="unbutton" data-mode="selected" aria-pressed="false">
                <i class="fa-solid fa-expand" inert></i> ${game.i18n.localize("AAFO.Tokens.Selected")}
              </button>
            </div>
      ...
      if ( !this.chatMessage.getFlag("aafo", "targets")?.length ) this.targetSourceControl.hidden = true;
  ...
  buildTargetsList() {
    ...
    switch ( this.targetingMode ) {
      case "targeted":
        targetedTokens = (this.chatMessage.getFlag("aafo", "targets") ?? []).map(t => t.uuid);
        break;
      case "selected":
    ...
    if ( targets.length ) this.targetList.replaceChildren(...targets);
      ...
      li.innerText = game.i18n.localize(`AAFO.Tokens.None${this.targetingMode.capitalize()}`);
  ...
  getChangeSourceOptions(type, change, options) {
    ...
    if ( mode === "ignore" ) label = game.i18n.format("AAFO.DamageApplication.Ignoring", { source: label });
    if ( mode === "downgrade" ) label = game.i18n.format("AAFO.DamageApplication.Downgrading", { source: label });
