...
export default class ItemListControlsElement extends HTMLElement {
   ...
   */
  get prefs() {
    return game.user.getFlag("aafo", `sheetPrefs.${this.app.object.type}.tabs.${this.tab}`);
  }
  ...
  #buildHTML() {
    ...
    search.innerHTML = `
      ...
      <ul class="unlist controls">
        <li>
          <button type="button" class="unbutton filter-control" data-action="clear" data-tooltip="AAFO.FilterClear"
                  aria-label="${game.i18n.localize("AAFO.FilterClear")}">
    ...
    if ( options.length ) {
      ...
      item.innerHTML = `
        <button type="button" class="unbutton filter-control filter" data-action="filter"
                aria-label="${game.i18n.localize("AAFO.Filter")}">
  ...
  async _onToggleMode(event) {
    ...
    const current = game.user.getFlag("aafo", flag);
    let value;
    ...
    await game.user.setFlag("aafo", flag, value);
    if ( action === "group" ) {
      this._initGrouping();
      this._applyGrouping();
    } else if ( action === "sort" ) {
