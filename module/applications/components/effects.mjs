...
export default class EffectsElement extends HTMLElement {
  connectedCallback() {
    ...
    new MenuCls(this, "[data-effect-id]", [], {onOpen: element => {
      ...
      Hooks.call("aafo.getActiveEffectContextOptions", effect, ui.context.menuItems);
    }});
  }
  ...
  static prepareCategories(effects, { parent }={}) {
    ...
    const categories = {
      enchantment: {
        type: "enchantment",
        label: game.i18n.localize("AAFO.Enchantment.Category.General"),
        effects: [],
        isEnchantment: true
      },
      temporary: {
        type: "temporary",
        label: game.i18n.localize("AAFO.EffectTemporary"),
        effects: []
      },
      enchantmentActive: {
        type: "activeEnchantment",
        label: game.i18n.localize("AAFO.Enchantment.Category.Active"),
        effects: [],
        isEnchantment: true
      },
      passive: {
        type: "passive",
        label: game.i18n.localize("AAFO.EffectPassive"),
        effects: []
      },
      enchantmentInactive: {
        type: "inactiveEnchantment",
        label: game.i18n.localize("AAFO.Enchantment.Category.Inactive"),
        effects: [],
        isEnchantment: true
      },
      inactive: {
        type: "inactive",
        label: game.i18n.localize("AAFO.EffectInactive"),
        effects: []
      },
      suppressed: {
        type: "suppressed",
        label: game.i18n.localize("AAFO.EffectUnavailable"),
        effects: [],
        disabled: true,
        info: [game.i18n.localize("AAFO.EffectUnavailableInfo")]
      }
    };
    ...
    for ( const e of effects ) {
      ...
      else if ( e.getFlag("aafo", "type") === "enchantment" ) categories.enchantment.effects.push(e);
    ...
    for ( const category of Object.values(categories) ) {
      category.localizationPrefix = category.isEnchantment ? "AAFO.Enchantment.Action." : "AAFO.Effect";
    }
  ...
  _getContextOptions(effect) {
    ...
    const options = [
      {
        name: "AAFO.ContextMenuActionEdit",
        icon: "<i class='fas fa-edit fa-fw'></i>",
        condition: () => effect.isOwner,
        callback: li => this._onAction(li[0], "edit")
      },
      {
        name: "AAFO.ContextMenuActionDuplicate",
        icon: "<i class='fas fa-copy fa-fw'></i>",
        condition: () => effect.isOwner,
        callback: li => this._onAction(li[0], "duplicate")
      },
      {
        name: "AAFO.ContextMenuActionDelete",
        icon: "<i class='fas fa-trash fa-fw'></i>",
        condition: () => effect.isOwner && !isConcentrationEffect,
        callback: li => this._onAction(li[0], "delete")
      },
      {
        name: effect.disabled ? "AAFO.ContextMenuActionEnable" : "AAFO.ContextMenuActionDisable",
      ...
      {
        name: "AAFO.ConcentrationBreak",
        icon: '<aafo-icon src="systems/aafo/icons/svg/break-concentration.svg"></aafo-icon>',
    ...
    if ( (this.document instanceof Actor5e) && ("favorites" in this.document.system) ) {
      ...
      options.push({
        name: isFavorited ? "AAFO.FavoriteRemove" : "AAFO.Favorite",
  ...
  async _onToggleCondition(conditionId) {
    const existing = this.document.effects.get(staticID(`aafo${conditionId}`));
  ...
  async _onCreate(target) {
    ...
    return this.document.createEmbeddedDocuments("ActiveEffect", [{
      name: isActor ? game.i18n.localize("AAFO.EffectNew") : this.document.name,
      ...
      "flags.aafo.type": isEnchantment ? "enchantment" : undefined
    }]);
  }
  ...
  async _onClickEffectSource(event) {
    ...
    if ( !doc.testUserPermission(game.user, "LIMITED") ) {
      ui.notifications.warn("AAFO.DocumentViewWarn", { localize: true });
      return;
    }
