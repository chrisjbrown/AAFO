...
export default class EnchantmentApplicationElement extends HTMLElement {
  ...
  async buildItemList() {
    const enchantedItems = (await EnchantmentData.appliedEnchantments(this.enchantmentItem.uuid)).map(enchantment => {
      ...
      if ( item.isOwner ) {
        ...
        control.ariaLabel = game.i18n.localize("AAFO.Enchantment.Action.Remove");
        control.dataset.action = "removeEnchantment";
        control.dataset.tooltip = "AAFO.Enchantment.Action.Remove";
    ...
    if ( enchantedItems.length ) {
      ...
      this.dropArea.innerHTML = `<p>${game.i18n.localize("AAFO.Enchantment.DropArea")}</p>`;
    }
  }
  ...
  async _onDrop(event) {
    ...
    const effect = this.enchantmentItem.effects.get(this.chatMessage.getFlag("aafo", "use.enchantmentProfile"));
    ...
    const concentrationId = this.chatMessage.getFlag("aafo", "use.concentrationId");
    const concentration = effect.parent.actor.effects.get(concentrationId);
    if ( concentrationId && !concentration ) {
      ui.notifications.error("AAFO.Enchantment.Warning.ConcentrationEnded", { localize: true });
      return;
    }
    if ( !game.user.isGM && concentration && !concentration.actor?.isOwner ) {
      ui.notifications.error("AAFO.EffectApplyWarningConcentration", { localize: true });
      return;
    }
