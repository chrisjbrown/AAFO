...
export default class TokenConfig5e extends TokenConfig {
   ...
   * @type {string}
   */
  static dynamicRingTemplate = "systems/aafo/templates/apps/parts/dynamic-ring.hbs";
  ...
  async _addTokenRingConfiguration(html) {
    ...
    const flags = this.document.getFlag("aafo", "tokenRing") ?? {};
    ringTab.innerHTML = await renderTemplate(this.constructor.dynamicRingTemplate, {
      ...
      effects: Object.entries(CONFIG.AAFO.tokenRings.effects).reduce((obj, [key, label]) => {
    ...
    tab.insertAdjacentHTML("afterbegin", `
      <nav class="tabs sheet-tabs secondary-tabs" data-group="appearance">
        ...
        <a class="item" data-tab="ring" data-group="appearance">
          <i class="fa-solid fa-ring"></i> ${game.i18n.localize("AAFO.TokenRings.Title")}
        </a>
      </nav>
    `);
  ...
  _addItemAttributes(attributes) {
    ...
    if ( items.length ) {
      ...
      attributes[game.i18n.localize("AAFO.ConsumeCharges")] = items.map(i => i[0]);
    }
  }
  ...
  _getSubmitData(updateData={}) {
    ...
    formData["flags.aafo.tokenRing.effects"] = Object.keys(CONFIG.AAFO.tokenRings.effects).reduce((number, key) => {
      const checked = formData[`flags.aafo.tokenRing.effects.${key}`];
      delete formData[`flags.aafo.tokenRing.effects.${key}`];
  ...
  _previewChanges(change) {
    if ( change && (this.preview instanceof TokenDocument5e) && (game.release.generation < 12) ) {
      const flags = foundry.utils.getProperty(foundry.utils.expandObject(change), "flags.aafo.tokenRing") ?? {};
