...
async function enrichAward(config, label, options) {
  ...
  if ( parsed.xp ) entries.push(`
    <span class="award-entry">
      ${formatNumber(parsed.xp)} ${game.i18n.localize("AAFO.ExperiencePointsAbbr")}
    </span>
  `);

  let award = game.i18n.getListFormatter({ type: "unit" }).format(entries);
  if ( parsed.each ) award = game.i18n.format("EDITOR.AAFO.Inline.AwardEach", { award });

  block.innerHTML += `
    ${award}
    <a class="award-link" data-action="awardRequest">
      <i class="fa-solid fa-trophy"></i> ${label ?? game.i18n.localize("AAFO.Award.Action")}
    </a>
  `;
...
async function enrichCheck(config, label, options) {
  for ( let value of config.values ) {
    ...
    if ( value in CONFIG.AAFO.enrichmentLookup.abilities ) config.ability = value;
    else if ( value in CONFIG.AAFO.enrichmentLookup.skills ) config.skill = value;
    else if ( value in CONFIG.AAFO.enrichmentLookup.tools ) config.tool = value;
  ...
  const skillConfig = CONFIG.AAFO.enrichmentLookup.skills[slugify(config.skill)];
  ...
  const toolUUID = CONFIG.AAFO.enrichmentLookup.tools[slugify(config.tool)];
  ...
  let abilityConfig = CONFIG.AAFO.enrichmentLookup.abilities[slugify(config.ability)];
...
async function enrichSave(config, label, options) {
  ...
  const abilityConfig = CONFIG.AAFO.enrichmentLookup.abilities[config.ability];
  if ( !abilityConfig ) {
    console.warn(`Ability ${config.ability} not found while enriching ${config._input}.`);
    return null;
  }
...
async function enrichDamage(config, label, options) {
  ...
  for ( const value of config.values ) {
    if ( value in CONFIG.AAFO.damageTypes ) config.type = value;
    else if ( value in CONFIG.AAFO.healingTypes ) config.type = value;
  ...
  const typeConfig = CONFIG.AAFO.damageTypes[config.damageType] ?? CONFIG.AAFO.healingTypes[config.damageType];
  ...
  span.innerHTML = game.i18n.format(`EDITOR.AAFO.Inline.Damage${localizationType}`, localizationData);
  return span;
}
 ...
 *   <a class="content-link" draggable="true"
 *      data-uuid="Compendium.aafo.rules.JournalEntry.w7eitkpD7QQTB6j0.JournalEntryPage.UWw13ISmMxDzmwbd"
 ...
 *      data-tooltip="EDITOR.AAFO.Inline.ApplyStatus" aria-label="Apply Status to Selected Tokens">
...
async function enrichReference(config, label, options) {
  ...
  const type = Object.keys(config).find(k => k in CONFIG.AAFO.ruleTypes);
  if ( type ) {
    key = slugify(config[type]);
    source = foundry.utils.getProperty(CONFIG.AAFO, CONFIG.AAFO.ruleTypes[type].references)?.[key];
  } else if ( config.values.length ) {
    key = slugify(config.values.join(""));
    for ( const [type, { references }] of Object.entries(CONFIG.AAFO.ruleTypes) ) {
      source = foundry.utils.getProperty(CONFIG.AAFO, references)[key];
      if ( source ) {
  ...
  if ( isCondition && (config.apply !== false) ) {
    ...
    apply.dataset.tooltip = "EDITOR.AAFO.Inline.ApplyStatus";
...
export function createRollLabel(config) {
  const { label: ability, abbreviation } = CONFIG.AAFO.abilities[config.ability] ?? {};
  const skill = CONFIG.AAFO.skills[config.skill]?.label;
  const toolUUID = CONFIG.AAFO.enrichmentLookup.tools[config.tool];
  ...
  switch ( config.type ) {
    ...
    case "tool":
      if ( ability && (skill || tool) ) {
        label = game.i18n.format("EDITOR.AAFO.Inline.SpecificCheck", { ability, type: skill ?? tool });
      } else {
        label = ability;
      }
      if ( config.passive ) {
        label = game.i18n.format(`EDITOR.AAFO.Inline.DCPassive${longSuffix}`, { dc: config.dc, check: label });
      } else {
        if ( showDC ) label = game.i18n.format("EDITOR.AAFO.Inline.DC", { dc: config.dc, check: label });
        label = game.i18n.format(`EDITOR.AAFO.Inline.Check${longSuffix}`, { check: label });
      }
      break;
    case "concentration":
    case "save":
      if ( config.type === "save" ) label = ability;
      else label = `${game.i18n.localize("AAFO.Concentration")} ${ability ? `(${abbreviation})` : ""}`;
      if ( showDC ) label = game.i18n.format("EDITOR.AAFO.Inline.DC", { dc: config.dc, check: label });
      label = game.i18n.format(`EDITOR.AAFO.Inline.Save${longSuffix}`, { save: label });
      break;
    default:
      return "";
  }
  ...
  if ( config.icon ) {
    switch ( config.type ) {
      case "check":
      case "skill":
        label = `<i class="aafo-icon" data-src="systems/aafo/icons/svg/ability-score-improvement.svg"></i>${label}`;
        break;
      case "tool":
...
function createRollLink(label, dataset) {
  ...
  if ( game.user.isGM && (dataset.type !== "damage") && (dataset.type !== "item") ) {
    ...
    gmLink.dataset.tooltip = "EDITOR.AAFO.Inline.RequestRoll";
...
async function rollAction(event) {
  ...
  if ( (action === "roll") || !game.user.isGM ) {
    ...
    try {
      ...
      if ( !tokens.length ) {
        ui.notifications.warn("EDITOR.AAFO.Inline.Warning.NoActor", { localize: true });
        return;
      }
      ...
      for ( const token of tokens ) {
        ...
        switch ( type ) {
          case "check":
            ...
            break;
          case "concentration":
            if ( ability in CONFIG.AAFO.abilities ) options.ability = ability;
  ...
  // Roll request
    ...
    const chatData = {
      ...
      flavor: game.i18n.localize("EDITOR.AAFO.Inline.RollRequest"),
...
async function rollDamage(event) {
  ...
  const isHealing = damageType in CONFIG.AAFO.healingTypes;
  const title = game.i18n.localize(`AAFO.${isHealing ? "Healing" : "Damage"}Roll`);
  const rollConfig = {
    rollConfigs: [{
      parts: [formula],
      type: damageType
  ...
  if ( Hooks.call("aafo.preRollDamage", undefined, rollConfig) === false ) return;
  const roll = await damageRoll(rollConfig);
  if ( roll ) Hooks.callAll("aafo.rollDamage", undefined, roll);
}
...
async function useItem({ rollItemUuid, rollItemName, rollItemActor }={}) {
  ...
  ui.notifications.warn(game.i18n.format("EDITOR.AAFO.Inline.Warning.NoItemOnActor", {
