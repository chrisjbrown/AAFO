...
export default class AbilityTemplate extends MeasuredTemplate {
  ...
  static fromItem(item, options={}) {
    ...
    const templateShape = aafo.config.areaTargetTypes[target.type]?.template;
    ...
    const templateData = foundry.utils.mergeObject({
      ...
      flags: { aafo: { origin: item.uuid, spellLevel: item.system.level } }
    }, options);
    ...
    switch ( templateShape ) {
      ...
      case "rect": // 5e rectangular AoEs are always cubes
        ...
        if ( game.settings.get("aafo", "gridAlignedSquareTemplates") ) {
     ...
     * @function aafo.preCreateItemTemplate
     * @memberof hookEvents
     ...
     */
    if ( Hooks.call("aafo.preCreateItemTemplate", item, templateData) === false ) return null;
     ...
     * @function aafo.createItemTemplate
     * @memberof hookEvents
     * @param {Item5e} item                Item for which the template is being placed.
     * @param {AbilityTemplate} template   The template being placed.
     */
    Hooks.callAll("aafo.createItemTemplate", item, object);
