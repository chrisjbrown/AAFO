...
export default class Token5e extends Token {
  ...
  _drawHPBar(number, bar, data) {
    ...
    const hpColor = aafo.documents.Actor5e.getHPColor(value, effectiveMax);

    // Determine colors to use
    const blk = 0x000000;
    const c = CONFIG.AAFO.tokenHPColors;
  ...
  _onUpdate(data, options, userId) {
    ...
    if ( !foundry.utils.hasProperty(data, "flags.aafo.tokenRing") ) return;

    // Do we need to trigger a full redraw? We need to do so if a token ring texture has been updated
    const dataFlag = data.flags.aafo.tokenRing;
    ...
    const tokenRingFlag = this.document.getFlag("aafo", "tokenRing") || {};
