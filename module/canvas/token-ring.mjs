...
export default class TokenRing {
  ...
  configureMesh(mesh) {
    ...
    if ( this.enabled ) {
      const tokenRingFlag = this.token.document.getFlag("aafo", "tokenRing");
  ...
  async flashColor(color, animationOptions={}) {
    ...
    const originalColor = Color.from(foundry.utils.mergeObject(
      this.token.document.getFlag("aafo", "tokenRing.colors") ?? {},
  ...
  static initialize() {
    ...
    this.tokenRingSamplerShader = CONFIG.AAFO.tokenRings.shaderClass;
  ...
  static pushToLoad(additionalSources) {
    additionalSources.push(CONFIG.AAFO.tokenRings.spriteSheet);
  ...
  static createAssetsUVs() {
    ...
    const spritesheet = TextureLoader.loader.getCache(CONFIG.AAFO.tokenRings.spriteSheet);
