...
export class ItemChoiceConfigurationData extends foundry.abstract.DataModel {
  ...
  static defineSchema() {
    return {
      hint: new StringField({label: "AAFO.AdvancementHint"}),
      choices: new MappingField(new SchemaField({
        count: new NumberField({integer: true, min: 0}),
        replacement: new BooleanField({label: "AAFO.AdvancementItemChoiceReplacement"})
      }), {
        hint: "AAFO.AdvancementItemChoiceLevelsHint"
      }),
      allowDrops: new BooleanField({
        initial: true, label: "AAFO.AdvancementConfigureAllowDrops",
        hint: "AAFO.AdvancementConfigureAllowDropsHint"
      }),
      type: new StringField({
        blank: false, nullable: true, initial: null,
        label: "AAFO.AdvancementItemChoiceType", hint: "AAFO.AdvancementItemChoiceTypeHint"
      }),
      ...
      restriction: new SchemaField({
        type: new StringField({label: "AAFO.Type"}),
        subtype: new StringField({label: "AAFO.Subtype"}),
        level: new StringField({label: "AAFO.SpellLevel"})
      })
    };
  }
