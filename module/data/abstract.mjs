...
export default class SystemDataModel extends foundry.abstract.TypeDataModel {
   ...
   * @property {typeof DataModel} [systemFlagsModel]  Model that represents flags data within the aafo namespace.
   */
  ...
  static defineSchema() {
    ...
    for ( const template of this._schemaTemplates ) {
      if ( !template.defineSchema ) {
        throw new Error(`Invalid aafo template mixin ${template} defined on class ${this.constructor}`);
      }
...
export class ItemDataModel extends SystemDataModel {
   ...
   * @type {string}
   */
  static ITEM_TOOLTIP_TEMPLATE = "systems/aafo/templates/items/parts/item-tooltip.hbs";
  ...
  prepareBaseData() {
    if ( this.parent.isEmbedded ) {
      const sourceId = this.parent.flags.aafo?.sourceId ?? this.parent.flags.core?.sourceId;
  ...
  async richTooltip(enrichmentOptions={}) {
    return {
      ...
      classes: ["aafo2", "aafo-tooltip", "item-tooltip"]
    };
  }
  ...
  async getCardData(enrichmentOptions={}) {
    ...
    const context = {
      ...
      config: CONFIG.AAFO,
      controlHints: game.settings.get("aafo", "controlHints"),
    ...
    if ( context.labels.duration ) {
      context.labels.concentrationDuration = properties?.has("concentration")
        ? game.i18n.format("AAFO.ConcentrationDuration", {
