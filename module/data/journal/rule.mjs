 ...
 * @property {string} type     Type of rule represented. Should match an entry defined in `CONFIG.AAFO.ruleTypes`.
 */
export default class RuleJournalPageData extends foundry.abstract.DataModel {
  ...
  static defineSchema() {
    return {
      tooltip: new HTMLField({label: "AAFO.Rule.Tooltip"}),
      type: new StringField({blank: false, initial: "rule", label: "AAFO.Rule.Type.Label"})
    };
  }
  ...
  async richTooltip(enrichmentOptions={}) {
    const context = {
      page: this.parent,
      type: CONFIG.AAFO.ruleTypes[this.type].label,
    ...
    return {
      content: await renderTemplate("systems/aafo/templates/journal/page-rule-tooltip.hbs", context),
      classes: ["aafo-tooltip", "rule-tooltip"]
    };
  }
}
