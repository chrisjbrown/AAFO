  ...
  static defineSchema() {
    return {
      ...
      tokenRing: new SchemaField({
        enabled: new BooleanField({label: "AAFO.TokenRings.Enabled"}),
        colors: new SchemaField({
          ring: new ColorField({required: false, label: "AAFO.TokenRings.RingColor"}),
          background: new ColorField({required: false, label: "AAFO.TokenRings.RingColor"})
        }, {required: false, initial: undefined}),
        effects: new NumberField({
          initial: 1, min: 0, max: 8388607, integer: true, label: "AAFO.TokenRings.Effects.Label"
        }),
        scaleCorrection: new NumberField({
          required: false, initial: 1, min: 0, label: "AAFO.TokenRings.ScaleCorrection"
        }),
        textures: new SchemaField({
          subject: new FilePathField({
            required: false, categories: ["IMAGE"], label: "AAFO.TokenRings.Subject.Label",
            hint: "AAFO.TokenRings.Subject.Hint"
          })
        }, {required: false, initial: undefined})
      }, {required: false, initial: undefined, label: "AAFO.TokenRings.Title"})
    };
  }
}
