...
export default class RaceData extends ItemDataModel.mixin(ItemDescriptionTemplate) {
   ...
   */
  get movementLabels() {
    const units = CONFIG.AAFO.movementUnits[this.movement.units || Object.keys(CONFIG.AAFO.movementUnits)[0]];
    return Object.entries(CONFIG.AAFO.movementTypes).reduce((obj, [k, label]) => {
   ...
   */
  get sensesLabels() {
    const units = CONFIG.AAFO.movementUnits[this.senses.units || Object.keys(CONFIG.AAFO.movementUnits)[0]];
    return Object.entries(CONFIG.AAFO.senses).reduce((arr, [k, label]) => {
  ...
  async _preCreate(data, options, user) {
    ...
    this.parent.updateSource({"system.advancement": toCreate.map(c => {
      const config = CONFIG.AAFO.advancementTypes[c.type];
