 ...
 * @property {string} preparation.mode           Spell preparation mode as defined in `AAFO.spellPreparationModes`.
 ...
 * @property {string} scaling.mode               Spell scaling mode as defined in `AAFO.spellScalingModes`.
...
export default class SpellData extends ItemDataModel.mixin(
  ...
  static defineSchema() {
    return this.mergeSchema(super.defineSchema(), {
      level: new foundry.data.fields.NumberField({
        required: true, integer: true, initial: 1, min: 0, label: "AAFO.SpellLevel"
      }),
      school: new foundry.data.fields.StringField({required: true, label: "AAFO.SpellSchool"}),
      properties: new foundry.data.fields.SetField(new foundry.data.fields.StringField(), {
        label: "AAFO.SpellComponents"
      }),
      materials: new foundry.data.fields.SchemaField({
        value: new foundry.data.fields.StringField({required: true, label: "AAFO.SpellMaterialsDescription"}),
        consumed: new foundry.data.fields.BooleanField({required: true, label: "AAFO.SpellMaterialsConsumed"}),
        cost: new foundry.data.fields.NumberField({
          required: true, initial: 0, min: 0, label: "AAFO.SpellMaterialsCost"
        }),
        supply: new foundry.data.fields.NumberField({
          required: true, initial: 0, min: 0, label: "AAFO.SpellMaterialsSupply"
        })
      }, {label: "AAFO.SpellMaterials"}),
      preparation: new foundry.data.fields.SchemaField({
        mode: new foundry.data.fields.StringField({
          required: true, initial: "prepared", label: "AAFO.SpellPreparationMode"
        }),
        prepared: new foundry.data.fields.BooleanField({required: true, label: "AAFO.SpellPrepared"})
      }, {label: "AAFO.SpellPreparation"}),
      scaling: new foundry.data.fields.SchemaField({
        mode: new foundry.data.fields.StringField({required: true, initial: "none", label: "AAFO.ScalingMode"}),
        formula: new FormulaField({required: true, nullable: true, initial: null, label: "AAFO.ScalingFormula"})
      }, {label: "AAFO.LevelScaling"})
    });
  }
  ...
  static _migrateComponentData(source) {
    ...
    if ( components.length ) {
      foundry.utils.setProperty(source, "flags.aafo.migratedProperties", components);
    }
  }
  ...
  async getCardData(enrichmentOptions={}) {
    ...
    context.subtitle = [this.parent.labels.level, CONFIG.AAFO.spellSchools[this.school]?.label].filterJoin(" &bull; ");
  ...
  get _typeCriticalThreshold() {
    return this.parent?.actor?.flags.aafo?.spellCriticalThreshold ?? Infinity;
  }
