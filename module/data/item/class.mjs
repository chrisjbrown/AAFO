 ...
 * @property {string} hitDice           Denomination of hit dice available as defined in `AAFO.hitDieTypes`.
 ...
 * @property {string} spellcasting.progression  Spell progression granted by class as from `AAFO.spellProgression`.
...
export default class ClassData extends ItemDataModel.mixin(ItemDescriptionTemplate, StartingEquipmentTemplate) {
  ...
  static defineSchema() {
    return this.mergeSchema(super.defineSchema(), {
      ...
      levels: new NumberField({
        required: true, nullable: false, integer: true, min: 0, initial: 1, label: "AAFO.ClassLevels"
      }),
      hitDice: new StringField({
        required: true, initial: "d6", blank: false, label: "AAFO.HitDice",
        validate: v => /d\d+/.test(v), validationError: "must be a dice value in the format d#"
      }),
      hitDiceUsed: new NumberField({
        required: true, nullable: false, integer: true, initial: 0, min: 0, label: "AAFO.HitDiceUsed"
      }),
      ...
      spellcasting: new SchemaField({
        progression: new StringField({
          required: true, initial: "none", blank: false, label: "AAFO.SpellProgression"
        }),
        ability: new StringField({required: true, label: "AAFO.SpellAbility"})
      }, {label: "AAFO.Spellcasting"}),
      wealth: new FormulaField({label: "AAFO.StartingEquipment.Wealth.Label"})
    });
  }
  ...
  static _migrateTraitAdvancement(source) {
    ...
    if ( needsMigration ) foundry.utils.setProperty(source, "flags.aafo.persistSourceMigration", true);
  }
}
