...
export class SummonsData extends foundry.abstract.DataModel {
  ...
  static defineSchema() {
    return {
      bonuses: new SchemaField({
        ac: new FormulaField({
          label: "AAFO.Summoning.Bonuses.ArmorClass.Label", hint: "AAFO.Summoning.Bonuses.ArmorClass.hint"
        }),
        hd: new FormulaField({
          label: "AAFO.Summoning.Bonuses.HitDice.Label", hint: "AAFO.Summoning.Bonuses.HitDice.hint"
        }),
        hp: new FormulaField({
          label: "AAFO.Summoning.Bonuses.HitPoints.Label", hint: "AAFO.Summoning.Bonuses.HitPoints.hint"
        }),
        attackDamage: new FormulaField({
          label: "AAFO.Summoning.Bonuses.Attack.Label", hint: "AAFO.Summoning.Bonuses.Attack.Hint"
        }),
        saveDamage: new FormulaField({
          label: "AAFO.Summoning.Bonuses.Saves.Label", hint: "AAFO.Summoning.Bonuses.Saves.Hint"
        }),
        healing: new FormulaField({
          label: "AAFO.Summoning.Bonuses.Healing.Label", hint: "AAFO.Summoning.Bonuses.Healing.Hint"
        })
      }),
      classIdentifier: new IdentifierField(),
      creatureSizes: new SetField(new StringField(), {
        label: "AAFO.Summoning.CreatureSizes.Label", hint: "AAFO.Summoning.CreatureSizes.Hint"
      }),
      creatureTypes: new SetField(new StringField(), {
        label: "AAFO.Summoning.CreatureTypes.Label", hint: "AAFO.Summoning.CreatureTypes.Hint"
      }),
      match: new SchemaField({
        attacks: new BooleanField({
          label: "AAFO.Summoning.Match.Attacks.Label", hint: "AAFO.Summoning.Match.Attacks.Hint"
        }),
        proficiency: new BooleanField({
          label: "AAFO.Summoning.Match.Proficiency.Label", hint: "AAFO.Summoning.Match.Proficiency.Hint"
        }),
        saves: new BooleanField({
          label: "AAFO.Summoning.Match.Saves.Label", hint: "AAFO.Summoning.Match.Saves.Hint"
        })
      }),
      ...
      prompt: new BooleanField({
        initial: true, label: "AAFO.Summoning.Prompt.Label", hint: "AAFO.Summoning.Prompt.Hint"
      })
    };
  }
  ...
  static get canSummon() {
    return game.user.can("TOKEN_CREATE") && (game.user.isGM || game.settings.get("aafo", "allowSummoning"));
  }
  ...
  get summonedCreatures() {
    ...
    return SummonsData.summonedCreatures(this.item.actor)
      .filter(i => i?.getFlag("aafo", "summon.origin") === this.item.uuid);
  }
  ...
  async summon(profileId, options={}) {
    ...
    if ( !profile ) {
      throw new Error(game.i18n.format("AAFO.Summoning.Warning.NoProfile", { profileId, item: this.item.name }));
    }

    /**
     * A hook event that fires before summoning is performed.
     * @function aafo.preSummon
     * @memberof hookEvents
     ...
     */
    if ( Hooks.call("aafo.preSummon", this.item, profile, options) === false ) return;
    ...
    if ( !actor.isOwner ) {
      throw new Error(game.i18n.format("AAFO.Summoning.Warning.NoOwnership", { actor: actor.name }));
    }
    ...
    try {
      ...
      for ( const placement of placements ) {
         ...
         */
        if ( Hooks.call("aafo.preSummonToken", this.item, profile, tokenUpdateData, options) === false ) continue;
         ...
         * @param {SummoningOptions} options  Additional summoning options.
         */
        Hooks.callAll("aafo.summonToken", this.item, profile, tokenData, options);
     ...
     * @param {SummoningOptions} options  Additional summoning options.
     */
    Hooks.callAll("aafo.postSummon", this.item, profile, createdTokens, options);
  }
  ...
  async fetchActor(uuid) {
    ...
    if ( !actor ) throw new Error(game.i18n.format("AAFO.Summoning.Warning.NoActor", { uuid }));

    const actorLink = actor.prototypeToken.actorLink;
    if ( !actor.pack && (!actorLink || actor.getFlag("aafo", "summon.origin") === this.item.uuid )) return actor;
    ...
    const localActor = game.actors.find(a =>
      ...
      a.getFlag("aafo", "summonedCopy")
      // Sourced from the desired actor UUID
      && (a.getFlag("core", "sourceId") === uuid)
      // Unlinked or created from this item specifically
      && ((a.getFlag("aafo", "summon.origin") === this.item.uuid) || !a.prototypeToken.actorLink)
    );
    if ( localActor ) return localActor;

    // Check permissions to create actors before importing
    if ( !game.user.can("ACTOR_CREATE") ) throw new Error(game.i18n.localize("AAFO.Summoning.Warning.CreateActor"));
    ...
    if ( actor.pack ) {
      ...
      return game.actors.importFromCompendium(game.packs.get(actor.pack), actor.id, {
        "flags.aafo.summonedCopy": true
      });
    } else {
      // Template actor (linked) found in world, create a copy for this user's item.
      return actor.clone({"flags.aafo.summonedCopy": true, "flags.core.sourceId": actor.uuid}, {save: true});
    }
  }
  ...
  async getChanges(actor, profile, options) {
    ...
    // Add flags
    actorUpdates["flags.aafo.summon"] = {
    ...
    if ( this.match.proficiency ) {
      const proficiencyEffect = new ActiveEffect({
        _id: staticID("aafoMatchProficiency"),
        changes: [{
        ...
        name: game.i18n.localize("AAFO.Summoning.Match.Proficiency.Label")
      });
      ...
      if ( acBonus.total ) {
        if ( actor.system.attributes.ac.calc === "flat" ) {
          ...
          actorUpdates.effects.push((new ActiveEffect({
            ...
            name: game.i18n.localize("AAFO.Summoning.Bonuses.ArmorClass.Label")
          })).toObject());
        }
      }
    }
    ...
    if ( this.bonuses.hd && (actor.type === "npc") ) {
      ...
      if ( hdBonus.total ) {
        actorUpdates.effects.push((new ActiveEffect({
          ...
          name: game.i18n.localize("AAFO.Summoning.Bonuses.HitDice.Label")
        })).toObject());
      }
    }
      ...
      if ( hpBonus.total ) {
        ...
        const maxHpEffect = hpField => {
          return (new ActiveEffect({
            ...
            name: game.i18n.localize("AAFO.Summoning.Bonuses.HitPoints.Label")
          })).toObject();
        };
    ...
    if ( this.creatureSizes.size ) {
      ...
      const config = CONFIG.AAFO.actorSizes[size];
      if ( config ) {
    ...
    for ( const item of actor.items ) {
      ...
      if ( changes.length ) {
        const effect = (new ActiveEffect({
          _id: staticID("aafoItemChanges"),
          changes,
          disabled: false,
          icon: "icons/skills/melee/strike-slashes-orange.webp",
          name: game.i18n.localize("AAFO.Summoning.ItemChanges.Label"),
          origin: this.item.uuid,
          flags: {
            aafo: { type: "enchantment" }
          }
        })).toObject();
  ...
  async getTokenData({ actor, placement, tokenUpdates, actorUpdates }) {
    if ( actor.prototypeToken.randomImg && !game.user.can("FILES_BROWSE") ) {
      ...
      ui.notifications.warn("AAFO.Summoning.Warning.Wildcard", { localize: true });
    }
