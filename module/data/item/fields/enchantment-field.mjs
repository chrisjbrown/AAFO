...
export class EnchantmentData extends foundry.abstract.DataModel {
   ...
   */
  get enchantments() {
    return this.item.effects.filter(ae => ae.getFlag("aafo", "type") === "enchantment");
  }
  ...
  static isEnchantmentSource(data) {
    return !this.isEnchantment(data)
      && CONFIG.AAFO.featureTypes[data.type?.value]?.subtypes?.[data.type?.subtype]
      && (data.type?.subtype in CONFIG.AAFO.featureTypes.enchantment.subtypes);
  }
  ...
  static availableEnchantments(item) {
    ...
    return item.effects.filter(e => {
      if ( (e.getFlag("aafo", "type") !== "enchantment") || e.isAppliedEnchantment ) return false;
      const { min, max } = e.getFlag("aafo", "enchantment.level") ?? {};
  ...
  canEnchant(item) {
    ...
    if ( !this.restrictions.allowMagical && item.system.properties?.has("mgc") ) {
      errors.push(new EnchantmentError(game.i18n.localize("AAFO.Enchantment.Warning.NoMagicalItems")));
    }

    if ( this.restrictions.type && (item.type !== this.restrictions.type) ) {
      errors.push(new EnchantmentError(game.i18n.format("AAFO.Enchantment.Warning.WrongType", {
