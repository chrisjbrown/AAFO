...
export default class WeaponData extends ItemDataModel.mixin(
  ...
  static defineSchema() {
    return this.mergeSchema(super.defineSchema(), {
      type: new ItemTypeField({value: "simpleM", subtype: false}, {label: "AAFO.ItemWeaponType"}),
      magicalBonus: new NumberField({min: 0, integer: true, label: "AAFO.MagicalBonus"}),
      properties: new SetField(new StringField(), {label: "AAFO.ItemWeaponProperties"}),
  ...
  prepareDerivedData() {
    ...
    this.type.label = CONFIG.AAFO.weaponTypes[this.type.value] ?? game.i18n.localize(CONFIG.Item.typeLabels.weapon);
  }
  ...
  async getFavoriteData() {
    return foundry.utils.mergeObject(await super.getFavoriteData(), {
      subtitle: CONFIG.AAFO.itemActionTypes[this.actionType],
  ...
  get _typeCriticalThreshold() {
    return this.parent?.actor?.flags.aafo?.weaponCriticalThreshold ?? Infinity;
  }
  ...
  get proficiencyMultiplier() {
    ...
    const config = CONFIG.AAFO.weaponProficienciesMap;
    ...
    const improvised = (this.type.value === "improv") && !!actor.getFlag("aafo", "tavernBrawlerFeat");
