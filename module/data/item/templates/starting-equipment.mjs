...
export class EquipmentEntryData extends foundry.abstract.DataModel {
  ...
  static GROUPING_TYPES = {
    OR: "AAFO.StartingEquipment.Operator.OR",
    AND: "AAFO.StartingEquipment.Operator.AND"
  };
  ...
  static OPTION_TYPES = {
    // Category types
    armor: "AAFO.StartingEquipment.Choice.Armor",
    tool: "AAFO.StartingEquipment.Choice.Tool",
    weapon: "AAFO.StartingEquipment.Choice.Weapon",
    focus: "AAFO.StartingEquipment.Choice.Focus",

    // Generic item type
    linked: "AAFO.StartingEquipment.SpecificItem"
  };
  ...
  /**
   * Where in `CONFIG.AAFO` to find the type category labels.
  ...
  static defineSchema() {
    return {
      ...
      requiresProficiency: new BooleanField({label: "AAFO.StartingEquipment.Proficient.Label"})
    };
  }
  ...
  get label() {
    ...
    else if ( this.type !== "linked" ) label = game.i18n.format("AAFO.TraitConfigChooseAnyUncounted", { type: label });
    if ( (this.type === "linked") && this.requiresProficiency ) {
      label += ` (${game.i18n.localize("AAFO.StartingEquipment.IfProficient").toLowerCase()})`;
    }
    return label;
  }
  ...
  get categoryLabel() {
    ...
    if ( this.type === "weapon" ) label = game.i18n.format("AAFO.WeaponCategory", { category: label });
   ...
   */
  get keyOptions() {
    const config = foundry.utils.deepClone(CONFIG.AAFO[this.constructor.CATEGORIES[this.type]?.config]);
    if ( this.type === "weapon" ) foundry.utils.mergeObject(config, CONFIG.AAFO.weaponTypes);
