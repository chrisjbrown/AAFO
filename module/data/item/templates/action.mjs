 ...
 * @property {string} actionType            Action type as defined in `AAFO.itemActionTypes`.
...
export default class ActionTemplate extends ItemDataModel {
  ...
  static defineSchema() {
    return {
      ability: new StringField({required: true, nullable: true, initial: null, label: "AAFO.AbilityModifier"}),
      actionType: new StringField({required: true, nullable: true, initial: null, label: "AAFO.ItemActionType"}),
      attack: new SchemaField({
        bonus: new FormulaField({required: true, label: "AAFO.ItemAttackBonus"}),
        flat: new BooleanField({label: "AAFO.ItemAttackFlat"})
      }),
      chatFlavor: new StringField({required: true, label: "AAFO.ChatFlavor"}),
      critical: new SchemaField({
        threshold: new NumberField({
          required: true, integer: true, initial: null, positive: true, label: "AAFO.ItemCritThreshold"
        }),
        damage: new FormulaField({required: true, label: "AAFO.ItemCritExtraDamage"})
      }),
      damage: new SchemaField({
        parts: new ArrayField(new ArrayField(new StringField({nullable: true})), {required: true}),
        versatile: new FormulaField({required: true, label: "AAFO.VersatileDamage"})
      }, {label: "AAFO.Damage"}),
      enchantment: new EnchantmentField(),
      formula: new FormulaField({required: true, label: "AAFO.OtherFormula"}),
      save: new SchemaField({
        ability: new StringField({required: true, blank: true, label: "AAFO.Ability"}),
        dc: new NumberField({required: true, min: 0, integer: true, label: "AAFO.AbbreviationDC"}),
        scaling: new StringField({required: true, blank: false, initial: "spell", label: "AAFO.ScalingFormula"})
      }, {label: "AAFO.SavingThrow"}),
