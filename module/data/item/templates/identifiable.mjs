...
export default class IdentifiableTemplate extends SystemDataModel {
  ...
  static defineSchema() {
    return {
      identified: new BooleanField({required: true, initial: true, label: "AAFO.Identified"}),
      unidentified: new SchemaField({
        name: new StringField({label: "AAFO.NameUnidentified"}),
        description: new HTMLField({label: "AAFO.DescriptionUnidentified"})
      })
    };
  }
  ...
  async _preUpdate(changed, options, user) {
    ...
    if ( this.parent.type === "weapon" ) baseItemIdentifier = CONFIG.AAFO.weaponIds[this.type.baseItem];
    else if ( this.parent.type === "tool" ) baseItemIdentifier = CONFIG.AAFO.toolIds[this.type.baseItem];
    else if ( this.parent.type === "equipment" ) {
      if ( this.type.value === "shield" ) baseItemIdentifier = CONFIG.AAFO.shieldIds[this.type.baseItem];
      else baseItemIdentifier = CONFIG.AAFO.armorIds[this.type.baseItem];
    }
    ...
    if ( baseItem ) {
      if ( fetchName ) {
        foundry.utils.setProperty(changed, "system.unidentified.name", game.i18n.format(
          "AAFO.Unidentified.DefaultName", { name: baseItem.name }
        ));
      }
      if ( fetchDesc ) {
    ...
    if ( fetchName ) foundry.utils.setProperty(changed, "system.unidentified.name", game.i18n.format(
      "AAFO.Unidentified.DefaultName", { name: game.i18n.localize(CONFIG.Item.typeLabels[this.parent.type]) }
    ));
  }
}
