...
export default class ConsumableData extends ItemDataModel.mixin(
  ...
  static defineSchema() {
    return this.mergeSchema(super.defineSchema(), {
      type: new ItemTypeField({value: "potion", baseItem: false}, {label: "AAFO.ItemConsumableType"}),
      magicalBonus: new NumberField({min: 0, integer: true, label: "AAFO.MagicalBonus"}),
      properties: new SetField(new StringField(), { label: "AAFO.ItemAmmoProperties" }),
      uses: new ActivatedEffectTemplate.ItemUsesField({
        autoDestroy: new BooleanField({required: true, label: "AAFO.ItemDestroyEmpty"})
      }, {label: "AAFO.LimitedUses"})
    });
  }
  ...
  prepareDerivedData() {
    ...
    const config = CONFIG.AAFO.consumableTypes[this.type.value];
    if ( config ) {
  ...
  get chatProperties() {
    return [
      this.type.label,
      this.hasLimitedUses ? `${this.uses.value}/${this.uses.max} ${game.i18n.localize("AAFO.Charges")}` : null,
      this.priceLabel
    ];
  }
  ...
  get proficiencyMultiplier() {
    const isProficient = this.parent?.actor?.getFlag("aafo", "tavernBrawlerFeat");
  ...
  get validProperties() {
    ...
    if ( this.type.value === "ammo" ) Object.entries(CONFIG.AAFO.itemProperties).forEach(([k, v]) => {
      if ( v.isPhysical ) valid.add(k);
    });
    else if ( this.type.value === "scroll" ) CONFIG.AAFO.validProperties.spell
