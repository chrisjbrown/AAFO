 ...
 * @property {string} capacity.type         Method for tracking max capacity as defined in `AAFO.itemCapacityTypes`.
...
export default class ContainerData extends ItemDataModel.mixin(
  ...
  static defineSchema() {
    return this.mergeSchema(super.defineSchema(), {
      ...
      properties: new foundry.data.fields.SetField(new foundry.data.fields.StringField(), {
        label: "AAFO.ItemContainerProperties"
      }),
      capacity: new foundry.data.fields.SchemaField({
        type: new foundry.data.fields.StringField({
          required: true, initial: "weight", blank: false, label: "AAFO.ItemContainerCapacityType"
        }),
        value: new foundry.data.fields.NumberField({
          required: true, min: 0, label: "AAFO.ItemContainerCapacityMax"
        })
      }, {label: "AAFO.ItemContainerCapacity"})
    });
  }
  ...
  static _migrateWeightlessData(source) {
    if ( foundry.utils.getProperty(source, "system.capacity.weightless") === true ) {
      foundry.utils.setProperty(source, "flags.aafo.migratedProperties", ["weightlessContents"]);
    }
  }
  ...
  async computeCapacity() {
    ...
    if ( type === "weight" ) {
      ...
      context.units = game.i18n.localize("AAFO.AbbreviationLbs");
    } else {
      context.value = await this.contentsCount;
      context.units = game.i18n.localize("AAFO.ItemContainerCapacityItems");
    }
