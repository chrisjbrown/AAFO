 ...
 * @property {number} proficient  Level of proficiency in this tool as defined in `AAFO.proficiencyLevels`.
...
export default class ToolData extends ItemDataModel.mixin(
  ...
  static defineSchema() {
    return this.mergeSchema(super.defineSchema(), {
      type: new ItemTypeField({subtype: false}, {label: "AAFO.ItemToolType"}),
      ability: new foundry.data.fields.StringField({
        required: true, blank: true, label: "AAFO.DefaultAbilityCheck"
      }),
      chatFlavor: new foundry.data.fields.StringField({required: true, label: "AAFO.ChatFlavor"}),
      proficient: new foundry.data.fields.NumberField({
        required: true, initial: null, min: 0, max: 2, step: 0.5, label: "AAFO.ItemToolProficiency"
      }),
      properties: new foundry.data.fields.SetField(new foundry.data.fields.StringField(), {
        label: "AAFO.ItemToolProperties"
      }),
      bonus: new FormulaField({required: true, label: "AAFO.ItemToolBonus"})
    });
  }
  ...
  prepareDerivedData() {
    ...
    this.type.label = CONFIG.AAFO.toolTypes[this.type.value] ?? game.i18n.localize(CONFIG.Item.typeLabels.tool);
  }
   ...
   */
  get chatProperties() {
    return [CONFIG.AAFO.abilities[this.ability]?.label];
  }
