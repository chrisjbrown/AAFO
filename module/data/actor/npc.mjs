...
export default class NPCData extends CreatureTemplate {
  ...
  static defineSchema() {
    return this.mergeSchema(super.defineSchema(), {
      attributes: new SchemaField({
        ...
        hp: new SchemaField({
          value: new NumberField({
            nullable: false, integer: true, min: 0, initial: 10, label: "AAFO.HitPointsCurrent"
          }),
          max: new NumberField({
            nullable: false, integer: true, min: 0, initial: 10, label: "AAFO.HitPointsMax"
          }),
          ...
          formula: new FormulaField({required: true, label: "AAFO.HPFormula"})
      ...
      details: new SchemaField({
        ...
        environment: new StringField({required: true, label: "AAFO.Environment"}),
        cr: new NumberField({
          required: true, nullable: false, min: 0, initial: 1, label: "AAFO.ChallengeRating"
        }),
        spellLevel: new NumberField({
          required: true, nullable: false, integer: true, min: 0, initial: 0, label: "AAFO.SpellcasterLevel"
        }),
      ...
      resources: new SchemaField({
        legact: new SchemaField({
          value: new NumberField({
            required: true, nullable: false, integer: true, min: 0, initial: 0, label: "AAFO.LegActRemaining"
          }),
          max: new NumberField({
            required: true, nullable: false, integer: true, min: 0, initial: 0, label: "AAFO.LegActMax"
          })
        }, {label: "AAFO.LegAct"}),
        legres: new SchemaField({
          value: new NumberField({
            required: true, nullable: false, integer: true, min: 0, initial: 0, label: "AAFO.LegResRemaining"
          }),
          max: new NumberField({
            required: true, nullable: false, integer: true, min: 0, initial: 0, label: "AAFO.LegResMax"
          })
        }, {label: "AAFO.LegRes"}),
        lair: new SchemaField({
          value: new BooleanField({required: true, label: "AAFO.LairAct"}),
          initiative: new NumberField({
            required: true, integer: true, label: "AAFO.LairActionInitiative"
          })
        }, {label: "AAFO.LairActionLabel"})
  ...
  static #migrateTypeData(source) {
    ...
    if ( match ) {
      ...
      const typeMatch = Object.entries(CONFIG.AAFO.creatureTypes).find(([k, v]) => {
        return (typeLc === k)
      ...
      if ( match.groups.size ) {
        ...
        const sizeMatch = Object.entries(CONFIG.AAFO.actorSizes).find(([k, v]) => {
  ...
  prepareBaseData() {
    ...
    this.attributes.hd.denomination = Number(denomination ?? CONFIG.AAFO.actorSizes[this.traits.size]?.hitDie ?? 4);
  ...
  prepareDerivedData() {
    ...
    const hpOptions = {
      ...
      mod: this.abilities[CONFIG.AAFO.defaultAbilities.hitPoints ?? "con"]?.mod ?? 0
    };
