...
/**
 * A data model and API layer which handles the schema and functionality of "group" type Actors in the aafo system.
 ...
 * const g = new aafo.documents.Actor5e({
 *  type: "group",
 *  name: "Test Group",
 *  system: {
...
export default class GroupActor extends ActorDataModel.mixin(CurrencyTemplate) {
  ...
  static defineSchema() {
    return this.mergeSchema(super.defineSchema(), {
      type: new SchemaField({
        value: new StringField({initial: "party", label: "AAFO.Group.Type"})
      }),
      description: new SchemaField({
        full: new HTMLField({label: "AAFO.Description"}),
        summary: new HTMLField({label: "AAFO.DescriptionSummary"})
      }),
      members: new ArrayField(new SchemaField({
        actor: new ForeignDocumentField(foundry.documents.BaseActor),
        quantity: new SchemaField({
          value: new NumberField({initial: 1, integer: true, min: 0, label: "AAFO.Quantity"}),
          formula: new FormulaField({label: "AAFO.QuantityFormula"})
        })
      }), {label: "AAFO.GroupMembers"}),
      attributes: new SchemaField({
        movement: new SchemaField({
          land: new NumberField({nullable: false, min: 0, step: 0.1, initial: 0, label: "AAFO.MovementLand"}),
          water: new NumberField({nullable: false, min: 0, step: 0.1, initial: 0, label: "AAFO.MovementWater"}),
          air: new NumberField({nullable: false, min: 0, step: 0.1, initial: 0, label: "AAFO.MovementAir"})
        })
      }, {label: "AAFO.Attributes"}),
      details: new SchemaField({
        xp: new foundry.data.fields.SchemaField({
          value: new foundry.data.fields.NumberField({
            integer: true, min: 0, label: "AAFO.ExperiencePointsCurrent"
          })
  ...
  async rest(config, result) {
     ...
     * @function aafo.groupRestCompleted
     * @memberof hookEvents
     ...
     */
    Hooks.callAll("aafo.groupRestCompleted", this.parent, results);
  ...
  _onUpdate(changed, options, userId) {
    if ( !foundry.utils.hasProperty(changed, "system.type.value") || (game.user !== game.users.activeGM)
      || (game.settings.get("aafo", "primaryParty")?.actor !== this.parent)
