...
export default class CreatureTemplate extends CommonTemplate {
  static defineSchema() {
    return this.mergeSchema(super.defineSchema(), {
      bonuses: new foundry.data.fields.SchemaField({
        mwak: makeAttackBonuses({label: "AAFO.BonusMWAttack"}),
        rwak: makeAttackBonuses({label: "AAFO.BonusRWAttack"}),
        msak: makeAttackBonuses({label: "AAFO.BonusMSAttack"}),
        rsak: makeAttackBonuses({label: "AAFO.BonusRSAttack"}),
        abilities: new foundry.data.fields.SchemaField({
          check: new FormulaField({required: true, label: "AAFO.BonusAbilityCheck"}),
          save: new FormulaField({required: true, label: "AAFO.BonusAbilitySave"}),
          skill: new FormulaField({required: true, label: "AAFO.BonusAbilitySkill"})
        }, {label: "AAFO.BonusAbility"}),
        spell: new foundry.data.fields.SchemaField({
          dc: new FormulaField({required: true, deterministic: true, label: "AAFO.BonusSpellDC"})
        }, {label: "AAFO.BonusSpell"})
      }, {label: "AAFO.Bonuses"}),
      skills: new MappingField(new RollConfigField({
        value: new foundry.data.fields.NumberField({
          required: true, nullable: false, min: 0, initial: 0, label: "AAFO.SkillLevel"
        }),
        ability: "dex",
        bonuses: new foundry.data.fields.SchemaField({
          check: new FormulaField({required: true, label: "AAFO.SkillBonusCheck"}),
          passive: new FormulaField({required: true, label: "AAFO.SkillBonusPassive"})
        }, {label: "AAFO.SkillBonuses"})
      }), {
        initialKeys: CONFIG.AAFO.skills, initialValue: this._initialSkillValue,
        initialKeysOnly: true, label: "AAFO.Skills"
      }),
      tools: new MappingField(new RollConfigField({
        value: new foundry.data.fields.NumberField({
          required: true, nullable: false, min: 0, max: 2, step: 0.5, initial: 1, label: "AAFO.ProficiencyLevel"
        }),
        ability: "int",
        bonuses: new foundry.data.fields.SchemaField({
          check: new FormulaField({required: true, label: "AAFO.CheckBonus"})
        }, {label: "AAFO.ToolBonuses"})
      })),
      spells: new MappingField(new foundry.data.fields.SchemaField({
        value: new foundry.data.fields.NumberField({
          nullable: false, integer: true, min: 0, initial: 0, label: "AAFO.SpellProgAvailable"
        }),
        override: new foundry.data.fields.NumberField({
          integer: true, min: 0, label: "AAFO.SpellProgOverride"
        })
      }), {initialKeys: this._spellLevels, label: "AAFO.SpellLevels"})
    });
  }
  ...
  static _initialSkillValue(key, initial) {
    if ( CONFIG.AAFO.skills[key]?.ability ) initial.ability = CONFIG.AAFO.skills[key].ability;
    return initial;
  }
  ...
  static get _spellLevels() {
    const levels = Object.keys(CONFIG.AAFO.spellLevels).filter(a => a !== "0").map(l => `spell${l}`);
    return [...levels, "pact"];
  }
  ...
  static #migrateSensesData(source) {
    ...
    for ( let s of original.split(",") ) {
      ...
      if ( (type in CONFIG.AAFO.senses) && !(type in source.attributes.senses) ) {
  ...
  static #migrateToolData(source) {
    ...
    for ( const prof of original.value ) {
      const validProf = (prof in CONFIG.AAFO.toolProficiencies) || (prof in CONFIG.AAFO.toolIds);
...
function makeAttackBonuses(schemaOptions={}) {
  return new foundry.data.fields.SchemaField({
    attack: new FormulaField({required: true, label: "AAFO.BonusAttack"}),
    damage: new FormulaField({required: true, label: "AAFO.BonusDamage"})
  }, schemaOptions);
}
