...
export default class CommonTemplate extends ActorDataModel.mixin(CurrencyTemplate) {
    ...
    return this.mergeSchema(super.defineSchema(), {
      abilities: new MappingField(new foundry.data.fields.SchemaField({
        value: new foundry.data.fields.NumberField({
          required: true, nullable: false, integer: true, min: 0, initial: 5, label: "AAFO.AbilityScore"
        }),
        proficient: new foundry.data.fields.NumberField({
          required: true, integer: true, min: 0, max: 1, initial: 0, label: "AAFO.ProficiencyLevel"
        }),
        max: new foundry.data.fields.NumberField({
          required: true, integer: true, nullable: true, min: 0, initial: null, label: "AAFO.AbilityScoreMax"
        }),
        bonuses: new foundry.data.fields.SchemaField({
          check: new FormulaField({required: true, label: "AAFO.AbilityCheckBonus"}),
          save: new FormulaField({required: true, label: "AAFO.SaveBonus"})
        }, {label: "AAFO.AbilityBonuses"})
      }), {
        initialKeys: CONFIG.AAFO.abilities, initialValue: this._initialAbilityValue.bind(this),
        initialKeysOnly: true, label: "AAFO.Abilities"
      })
    });
  }
  ...
  static _initialAbilityValue(key, initial, existing) {
    const config = CONFIG.AAFO.abilities[key];
    if ( config ) {
  ...
  prepareAbilities({ rollData={}, originalSaves }={}) {
    ...
    for ( const [id, abl] of Object.entries(this.abilities) ) {
      ...
      abl.checkProf = new Proficiency(prof, 0);
      ...
      if ( !Number.isFinite(abl.max) ) abl.max = CONFIG.AAFO.maxAbilityScore;
