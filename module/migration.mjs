...
export const migrateWorld = async function() {
  ...
  for ( const [actor, valid] of actors ) {
    try {
      ...
      err.message = `Failed aafo system migration for Actor ${actor.name}: ${err.message}`;
      console.error(err);
    }
  }
  ...
  for ( const [item, valid] of items ) {
    try {
      ...
      err.message = `Failed aafo system migration for Item ${item.name}: ${err.message}`;
      console.error(err);
    }
  }
  ...
  for ( const m of game.macros ) {
    try {
      ...
      err.message = `Failed aafo system migration for Macro ${m.name}: ${err.message}`;
      console.error(err);
    }
  }
  ...
  for ( const table of game.tables ) {
    try {
      ...
      err.message = `Failed aafo system migration for RollTable ${table.name}: ${err.message}`;
      console.error(err);
    }
  }
  ...
  for ( const s of game.scenes ) {
    try {
      ...
      err.message = `Failed aafo system migration for Scene ${s.name}: ${err.message}`;
      console.error(err);
    }
    ...
    for ( const token of s.tokens ) {
      ...
      try {
        ...
        err.message = `Failed aafo system migration for ActorDelta [${token.id}]: ${err.message}`;
        console.error(err);
      }
    }
  }
  ...
  game.settings.set("aafo", "systemMigrationVersion", game.system.version);
...
export const migrateCompendium = async function(pack) {
  ...
  aafo.moduleArt.suppressArt = true;
  ...
  for ( let doc of documents ) {
    ...
    // Handle migration failures
      ...
      err.message = `Failed aafo system migration for document ${doc.name} in pack ${pack.collection}: ${err.message}`;
      console.error(err);
    }
  }
  ...
  aafo.moduleArt.suppressArt = false;
...
export const migrateArmorClass = async function(pack) {
  ...
  const updates = [];
  const armor = new Set(Object.keys(CONFIG.AAFO.armorTypes));
...
export function migrateItemData(item, migrationData, flags={}) {
  ...
  // Migrate properties
  const migratedProperties = foundry.utils.getProperty(item, "flags.aafo.migratedProperties");
  if ( migratedProperties?.length ) {
    ...
    updateData["flags.aafo.-=migratedProperties"] = null;
  }

  if ( foundry.utils.getProperty(item, "flags.aafo.persistSourceMigration") ) {
    flags.persistSourceMigration = true;
    updateData["flags.aafo.-=persistSourceMigration"] = null;
  }
...
export const getMigrationData = async function() {
  const data = {};
  try {
    const icons = await fetch("systems/aafo/json/icon-migration.json");
    const spellIcons = await fetch("systems/aafo/json/spell-icon-migration.json");
...
function _migrateTokenImage(actorData, updateData) {
  const oldSystemPNG = /^systems\/aafo\/tokens\/([a-z]+)\/([A-z]+).png$/;
  for ( const path of ["texture.src", "prototypeToken.texture.src"] ) {
    ...
    if ( oldSystemPNG.test(v) ) {
      ...
      updateData[path] = `systems/aafo/tokens/${type}/${fileName}.webp`;
    }
  }
  return updateData;
}
...
/**
 * Migrate macros from the old 'aafo.rollItemMacro' and 'aafo.macros' commands to the new location.
 * @param {object} macro       Macro data to migrate.
 * @param {object} updateData  Existing update to expand upon.
 * @returns {object}           The updateData to apply.
 */
function _migrateMacroCommands(macro, updateData) {
  if ( macro.command.includes("game.aafo.rollItemMacro") ) {
    updateData.command = macro.command.replaceAll("game.aafo.rollItemMacro", "aafo.documents.macro.rollItem");
  } else if ( macro.command.includes("game.aafo.macros.") ) {
    updateData.command = macro.command.replaceAll("game.aafo.macros.", "aafo.documents.macro.");
  }
  return updateData;
}
...
export async function purgeFlags(pack) {
  const cleanFlags = flags => {
    const flags5e = flags.aafo || null;
    return flags5e ? {aafo: flags5e} : {};
  };
  await pack.configure({locked: false});
