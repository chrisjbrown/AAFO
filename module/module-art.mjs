...
export class ModuleArt {
  ...
  async #parseArtMapping(moduleId, mapping, credit) {
    let settings = game.settings.get("aafo", "moduleArtConfiguration")?.[moduleId];
  ...
  static getArtModules() {
    const settings = game.settings.get("aafo", "moduleArtConfiguration");
    const unsorted = [];
    const configs = [{
      id: game.system.id,
      label: game.system.title,
      mapping: "systems/aafo/json/fa-token-mapping.json",
      priority: settings.aafo?.priority ?? CONST.SORT_INTEGER_DENSITY,
      credit: `
        <em>
          Token artwork by
    ...
    for ( const module of game.modules ) {
      ...
      if ( !mapping ) continue;
      const config = { id: module.id, label: module.title, credit: flags?.["aafo-art-credit"], mapping };
      configs.push(config);
...
export class ModuleArtConfig extends FormApplication {
  ...
  constructor(object={}, options={}) {
    object = foundry.utils.mergeObject(game.settings.get("aafo", "moduleArtConfiguration"), object, {inplace: false});
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      title: game.i18n.localize("AAFO.ModuleArtConfigL"),
      id: "module-art-config",
      template: "systems/aafo/templates/apps/module-art-config.hbs",
      popOut: true,
      width: 600,
      height: "auto"
    });
  ...
  async _updateObject(event, formData) {
    await game.settings.set("aafo", "moduleArtConfiguration", foundry.utils.expandObject(formData));
